...
The CHERI Register Enable (CRE) bit controls whether explicit access to
CHERI register is permitted when V=1. When CRE=1, CHERI registers
can be read and written by VS and VU modes. When CRE=0, CHERI registers are
disabled in VS and VU modes.

* henvcfg.CRE is read-only zero if:
...
{cheri_base_ext_name} (but not {cheri_default_ext_name}), then <<vsstatus>>'s
...
VS-mode's version of supervisor register <<sscratch>>. This register is only
