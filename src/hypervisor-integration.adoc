...
The hypervisor extension defines several integer load and store instruction
(HLV.*x*, HSV.*x* and HLVX.*x*) that transfer the amount of integer data
...
When in pass:attributes,quotes[{cheri_cap_mode_name}], the hypervisor load and
store instructions behave as described in
xref:section_existing_riscv_insns[xrefstyle=short]. In
pass:attributes,quotes[{cheri_int_mode_name}], the instructions behave as
...
When in pass:attributes,quotes[{cheri_cap_mode_name}], the hypervisor
load and store capability instructions behave as described in
xref:section_existing_riscv_insns[xrefstyle=short]. In
pass:attributes,quotes[{cheri_int_mode_name}], the instructions behave as rely
