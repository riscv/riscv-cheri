...
* The <<m_bit>> is reset to be {cheri_default_ext_name}.
* The debugger can set the <<m_bit>> to {cheri_base_mode_ext} by reading the CSR, using <<SCMODE>> and then writing the CSR.
