...
IMPORTANT: This chapter is not part of the to-be ratified package.
...
When `{load_cap_name_lc}` transfers a capability with set {ctag} from memory
into a hart's register file,
...
We permit, but do not require, <<JALR_CHERI>> to raise a {cheri_excep_name_ld}
if the computed target <<pcc>> has its {ctag} set to 0.
The security of the system is not conditional on this behavior,
