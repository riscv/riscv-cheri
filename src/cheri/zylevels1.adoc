...
=== Interaction with <<CBLD>> and <<YSUNSEAL>>
...
Identical requirements hold for a <<YSUNSEAL>> instruction `{YSUNSEAL_LC} {cd}, {cs1}, {cs2}`.

#TODO: Or would we prefer that `{cd}` 's <<zylevels1_gl_perm>> is the AND of those of `{cs1}` and `{cs2}`?#
