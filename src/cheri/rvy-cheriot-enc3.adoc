...
== The {rvy_cheriot_encoding3_suffix} CHERI Capability Encoding Scheme
...
The only difference from {rvy_cheriot_encoding2_suffix} is that the top field, T, is replaced by a length, L.
...
// Decode base (same as <<rvy_cheriot_encoding_suffix>>)
c_b = (A[e + 8 : e]) < B ? -1 : 0
...
All other aspects of the capability encoding are identical to {rvy_cheriot_encoding2_suffix}.

==== Capability Encoding Parameter Summary

The encoding parameters for this encoding are identical to {rvy_cheriot_encoding2_suffix} except for the encoding variant parameter.

.{rvy_cheriot_encoding2_suffix} (Alternative Version) parameter summary
