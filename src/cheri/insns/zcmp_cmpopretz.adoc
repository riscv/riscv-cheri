...
`cm.popretz \{yreg_list\}, -stack_adj`
...
Load capability registers as specified in _yreg_list_. Deallocate stack frame. Move zero into `ca0`. Return by calling <<JALR_CHERI>> to `cra`. All data accesses are checked against `ysp`. The return destination is checked against `cra`.

{cheri_int_mode_name} Description::
Load integer registers as specified in _xreg_list_. Deallocate stack frame. Move zero into `a0`. Return by calling <<JALR_CHERI>> to `ra`. All data accesses are checked against <<ddc>>. The return destination is checked against <<pcc>>.
