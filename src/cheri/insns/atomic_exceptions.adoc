...
+
If the authorizing capability does not grant <<lm_perm>>, and the {ctag} of `cd` is 1 and `cd` is not sealed, then an implicit <<ACPERM>> clearing <<w_perm>> and <<lm_perm>> is performed to obtain the intermediate permissions on `cd` (see <<LC>>).
+
Extensions may further attenuate loaded or stored capabilities via "implicit <<ACPERM>>-s" and/or outright tag clearing;
see <<LC>> and <<SC>>.
+
endif::[]
ifndef::cap_atomic[]
