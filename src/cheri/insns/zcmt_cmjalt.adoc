<<<

[#CM_JALT_CHERI,reftext="CM.JALT ({cheri_base32_ext_name})"]
==== CM.JALT ({cheri_base32_ext_name})

Synopsis::
Jump via table with link (CM.JALT), 16-bit encodings

Mnemonic (RV32)::
`cm.jalt _index_`

Encoding::
[wavedrom, ,svg,subs=attributes+]
....
{reg:[
    { bits:  2, name: 0x2,      attr: ['C2'] },
    { bits:  8, name: 'index',  attr: [] },
    { bits:  3, name: 0x0,      attr: [] },
    { bits:  3, name: 0x5,      attr: ['FUNCT3'] },
],config:{bits:16}}
....

[NOTE]

  For this encoding to decode as <<CM_JALT_CHERI>>, _index{ge}32_, otherwise it decodes as <<CM_JT_CHERI>>.

Description ({cheri_base32_ext_name})::
Redirect instruction fetch via the jump table defined by the indexing via `jvt.address+ index*XLEN/8`, checking every byte of the jump table access against <<jvt_y>> bounds (not against <<pcc>>) and requiring <<x_perm>>. Link to `cra`.
+
The target <<pcc>> is calculated by copying the current <<pcc>> and setting its address to the value read from the jump table, using the semantics of the <<SCADDR>> instruction.
+
If the <<jvt_y>> check fails, then set the {ctag} of the target <<pcc>> to zero.

include::zcm_common.adoc[]

Permissions ({cheri_base32_ext_name})::
Requires <<jvt_y>> to have its {ctag} set to one, not be sealed, grant <<x_perm>>, and for the full XLEN-bit table access to be in <<jvt_y>> bounds.

include::pcrel_debug_warning.adoc[]

Prerequisites::
{c_cheri_base_ext_names}, Zcmt

Operation::
[source,SAIL,subs="verbatim,quotes"]
----
SAIL TO BE UPDATED
----
