...
+
. If the platform includes the <<section_rvy_sentry_jalr_ext>> extension,
  then impose any conditional behavior specified and,
  if the target <<pcc>> is a sentry (that is, sealed with an acceptable type),
  ensure that the `offset` immediate is zero and unseal the target <<pcc>>.
...
. Jump to the target <<pcc>>
  and, if the capability encoding entails the presence of <<section_rvy_sentry_jalr_ext>>
