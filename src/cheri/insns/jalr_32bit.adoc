...
+
NOTE: All such triples are acceptable to <<app_cap_description>>.
. If the target <<pcc>> is a <<sentry_cap>> (for <<app_cap_description>>: if it has <<sec_cap_type>> `1`), then unseal it.
. If the target <<pcc>> is (still) sealed, clear its {ctag}.
+
NOTE: This is not possible for <<app_cap_description>>, given its two <<sec_cap_type>> values.
...
+
NOTE: <<app_cap_description>> always seals return capabilities with <<sec_cap_type>> `1`.
...
+
NOTE: There are no such additional semantics defined by the capabilities of <<app_cap_description>>. +
...
+
NOTE: For <<app_cap_description>>, there are no conditions and all such triples are acceptable;
