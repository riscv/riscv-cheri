...
. HOOK 1: Clear the {ctag} of the target <<pcc>> if the triple of its <<sec_cap_type>> value, `{cd}`, and `{cs1}` is not acceptable, as defined by the capability encoding or an extension.
. If the target <<pcc>> is a <<sentry_cap>>, then unseal it.
...
.. HOOK 2: if the capability encoding or an extension defines <<sentry_cap>> <<sec_cap_type>> values for return pointers, then seal the return capability as specified.
. Jump to the target <<pcc>>.
.. NOTE: It is not possible for the target <<pcc>> to be sealed by the time it is jumped to.
. HOOK 3: Apply any capability encoding- or extension-specified architectural semantics for the type of `{cs1}`.
+
...
.. NOTE: For <<rv32y_cap_description>>/<<rv64y_cap_description>>, sentries have <<sec_cap_type>> set to `1` and sealed return capabilities also have type <<sec_cap_type>> set to `1`.
 CHERIoT formats support more <<sec_cap_type>> values, some of which are designated as <<sentry_cap,sentry>> types by the <<sec_zycheriot_unpriv>> extension.
...
.. NOTE: For example, CHERIoT offers sentries that update a hart's `mstatus.MIE` field when invoked; see the the {cheriot_priv_ext_name} extension.
+
[NOTE]
=====
