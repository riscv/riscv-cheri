...
=======
<<<<<<< HEAD
. HOOK 1: Clear the {ctag} of the target <<pcc>> if the triple of its <<sec_cap_type>> value, `{cd}`, and `{cs1}` is not acceptable, as defined by the capability encoding.
...
+
NOTE: This is not possible for <<rv32y_cap_description>>/<<rv64y_cap_description>>, given its two <<sec_cap_type>> values.
>>>>>>> 791a4b70 (Minor proofreading improvements (#879))
>>>>>>> ca87a56d (Minor proofreading improvements (#879))
=======
. HOOK 1: Clear the {ctag} of the target <<pcc>> if the triple of its <<sec_cap_type>> value, `{cd}`, and `{cs1}` is not acceptable, as defined by the capability encoding.
. If the target <<pcc>> is a <<sentry_cap>>, then unseal it.
. If the target <<pcc>> is (still) sealed, clear its {ctag}.
>>>>>>> 23464a0a (make JALR more readable)
...
[NOTE]
=====
When a sealed capability (i.e. a sentry) is passed as the input to <<JALR_CHERI>>, its address must have bit zero clear and the instruction must have a zero offset, or the target <<pcc>> will have its {ctag} set to zero, since updates to its address are interpreted with <<SCADDR>> semantics.
=====
