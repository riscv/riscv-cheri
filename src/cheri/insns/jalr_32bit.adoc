...
. Set bit zero of the target address to zero and update <<pcc>> with the new target address using semantics of the <<SCADDR>> instruction
. Jump to the target <<pcc>>
+
The <<pcc>> of the next instruction is calculated using semantics of the <<SCADDR>> instruction, and written to `{cd}` as a sentry.
+
