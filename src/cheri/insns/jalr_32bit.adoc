...
. If `{cd}` is the link register, `{cs1}` is _sealed_ and `{cs1}` is not a _forward-edge_ <<sentry_cap>>, then the behavior is _reserved_^1^.
...
. Unseal the target <<pcc>> if it is any <<sentry_cap>> and `{cs1}.address[0]`` is zero and the I-immediate is zero.
