...
. HOOK 1: Optionally set the {ctag} of the target <<pcc>> to zero depending on the <<sec_cap_type>> value and the numerical values of `{cd}` and `{cs1}`
. Unseal the target <<pcc>> if it is a <<sentry_cap>>, or set the {ctag} to zero if it is any other sealed type.
