...
Mnemonic::
`jalr {creg_pref}d, {creg_pref}s1, offset`
...
Description::
Indirect jump to the target capability in `{creg_pref}s1` with an address offset.
+
Copy `{creg_pref}s1` to the target <<pcc>>
+
...
+
The <<pcc>> of the next instruction is calculated using <<SCADDR>> semantics, and written to `{creg_pref}d` as a sentry.
+
