...
+
. If the target <<pcc>> is a <<sentry_cap>> and the `offset` is zero, then unseal the target <<pcc>>.
...
+
The <<pcc>> of the next instruction is calculated using the semantics of the <<SCADDR>> instruction, and written to `{cd}` as a <<sentry_cap>>.
+
