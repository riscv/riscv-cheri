...
+
NOTE: All such triples are acceptable to <<rvy64_cap_description>> and <<rvy32_cap_description>>.
. If the target <<pcc>> is a <<sentry_cap>> (for <<rvy32_cap_description>>/<<rvy64_cap_description>>: if it has <<sec_cap_type>> `1`), then unseal it.
. If the target <<pcc>> is (still) sealed, clear its {ctag}.
+
NOTE: This is not possible for <<rvy32_cap_description>>/<<rvy64_cap_description>>, given its two <<sec_cap_type>> values.
. If `{cd}≠0`, compute the return capability and install it to `{cd}`:
...
+
NOTE: <<rvy32_cap_description>>/<<rvy64_cap_description>> always seals return capabilities with <<sec_cap_type>> `1`.
...
+
NOTE: There are no such additional semantics defined by the capabilities of <<rvy32_cap_description>>/<<rvy64_cap_description>>. +
...
+
NOTE: For <<rvy32_cap_description>>/<<rvy64_cap_description>>, there are no conditions and all such triples are acceptable;
