...
.. If capability encoding format does not support the request set of permissions then <<CLRPERM>> will return a minimal sets of permissions, which may be no permissions.
...
NOTE: Any future extension that defines new permissions that are a refinement of existing permissions
(e.g., finer-grained <<asr_perm>> or those of <<section_zylevels1>>)
must be allocated to the bits that are currently reported as 1 to ensure forward-compatibility.
