...
. Encode the resulting architectural permissions as specified by the encoding in use.
  ...
  until a fixed point is reached -- specifically, capability metadata (such as
  a set of permissions or the <<M-bit>>) that cannot be further reduced by any rule.
  Note that encodings may specify other encoding procedures.
+
