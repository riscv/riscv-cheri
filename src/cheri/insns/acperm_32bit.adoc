...
NOTE: Future extensions may include hardwired permission bits, in which case they are not cleared by set bits in `rs2`.
 If they are hardwired to one then they must be set in the bit mask.
...
. Set `{cd}.tag=0` if any <<section_cap_integrity,integrity>> checks fail or if any bits are not set in the bit mask which map to reserved one bits in <<acperm_bit_field>>.
