...
. Convert the <<AP-field>>, <<m_bit>>, <<section_cap_level,CL-field>> and <<SDP-field>> of capability `cs1` into a bit field with the
...
. Copy `cs1` to `cd`, and update the <<AP-field>>, <<m_bit>>, <<section_cap_level,CL-field>> and <<SDP-field>> with the newly calculated versions.
. Set `cd.tag=0` if `cs1` is sealed and any bits in the <<AP-field>>, <<m_bit>> or <<SDP-field>> were affected by <<ACPERM>>.
