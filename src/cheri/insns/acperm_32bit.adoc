...
. Set `cd.tag=0` if `cs1` is sealed only if <<ACPERM>> has been issued as an instruction.
.. Do not do the sealed check if <<ACPERM>> is _implicitly_ used to process data from <<LC>> or for any other reason.
