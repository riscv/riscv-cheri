...
. Convert the AP and SDP fields of capability `ys1` into a bit field with the
...
. Copy `ys1` to `yd`, and update the AP and SDP fields with the newly calculated versions.
. Set `yd.tag=0` if `ys1` is sealed.
// TODO: reserved fields can only be set through a memory error, should all of this go into an appendix?
// or if any reserved fields of `ys1` are set.
