...
Mnemonics::
`{clrperm_lc} {creg_pref}d, {creg_pref}s1, rs2`
...
. Convert the <<AP-field>>, <<SDP-field>>, and any other extension-defined permission-like fields of capability `{creg_pref}s1` into a bit field with the
format shown in xref:acperm_bit_field[xrefstyle=short].
. The initial value in integer register `rs2` is treated as a bit mask that specifies bit positions to be cleared in the bit field. Any bit that is high in `rs2` will cause the corresponding bit to be cleared in the bit field.
+
...
. Copy `{creg_pref}s1` to `{creg_pref}d`, and update the <<AP-field>>, <<SDP-field>>, and any others therein with the newly calculated versions.
. Set `{creg_pref}d.tag=0` if `{creg_pref}s1` is sealed and any bits in the <<AP-field>> or <<SDP-field>> were affected by <<CLRPERM>>;
  extensions must describe whether such changes to their bits also necessitate tag clearing.
. Set `{creg_pref}d.tag=0` if any <<section_cap_integrity,integrity>> checks fail.
