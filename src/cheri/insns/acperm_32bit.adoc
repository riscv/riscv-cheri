...
+
.. Permission bits which are hardwired to one are not cleared by set bits in `rs2`.
...
. Set `{cd}.tag=0`, `{cd}.AP=0`, `{cd}.SDP=0`, and `{cd}.P=0`^1^ if `{cs1}` fails any <<section_cap_integrity,integrity>> checks.

^1^ If {cheri_default_ext_name} is implemented.
