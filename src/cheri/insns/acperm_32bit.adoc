...
NOTE: The <<el_perm,EL>>, <<sl_perm,SL>> and <<section_cap_level,CL>> fields are only modifiable if the implementation supports <<cheri_levels_ext,{cheri_levels_ext_name}>>, otherwise they are hardwired to 1.


NOTE: Even though it can be modified using <<ACPERM>>, <<section_cap_level,CL>> is not considered an architectural permission.
