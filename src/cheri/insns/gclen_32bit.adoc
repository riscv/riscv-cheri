...
Mnemonic::
`{gclen_lc} rd, {creg_pref}s1`

Encoding::
include::wavedrom/gclen.adoc[]

Description::
Calculate the length of `{creg_pref}s1` 's bounds and write the result in `rd`.
+
The length
is defined as the difference between the decoded bounds' top and base addresses,
i.e., `top - base`.
+
Return the maximum length, 2^MXLEN^-1, if the length of `{creg_pref}s1` is 2^MXLEN^.
+
