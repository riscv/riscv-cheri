...
Description::
Calculate the length of `ys1` 's bounds and write the result in `rd`. The length
is defined as the difference between the decoded bounds' top and base addresses
i.e. `top - base`. It is not required that the input capability `ys1`  has its
tag set to 1. <<GCLEN>> outputs 0 if `ys1` 's bounds are malformed (see
