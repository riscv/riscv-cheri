...
NOTE: The <<section_cap_integrity,integrity>> check on `{cs2}` is mandatory to prevent authorizing a capability with a lack of integrity.
 The <<section_cap_integrity,integrity>> check included in the subset check is optional, meaning that the check on `{cs1}` is optional.
ifndef::cheri_ratification_v1_only[]
