...
[NOTE]
=====
If software wishes to permit conditional reconstructions of sentries,
it suffices to conditionally execute a <<SENTRY>> instruction
on the result of a <<CBLD>>
depending on the result of <<GCTYPE>> on the input to said <<CBLD>>.
For example, to rebuild bits from `x3` into `x1`
using the superset in `x2` with scratch `x4`, software could use:
