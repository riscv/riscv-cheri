...
`hlvx.hu rd, {creg_pref}s1` +
`hlvx.wu rd, {creg_pref}s1`
...
either VS-mode or VU-mode. The effective address is `{creg_pref}s1.address`. The
authorizing capability for the operation is `{creg_pref}s1`. A copy of the loaded value
is written to `rd`.
+
