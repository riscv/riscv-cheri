...
Increment the address field of `ys2` by the unsigned word in `rs1` shifted left
by _n_ bit positions and write the result to `yd`. The valid tag bit of the output
capability is 0 if `ys2` did not have its tag set to 1, the incremented address
is outside `ys2` 's <<section_cap_representable_check>> or `ys2` is sealed.
