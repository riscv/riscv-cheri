...
`c.lh  rd', offset({creg_pref}s1')` +
`c.lhu rd', offset({creg_pref}s1')` +
`c.lbu rd', offset({creg_pref}s1')`

{cheri_cap_mode_name} Expansions::
`lh  rd, offset({creg_pref}s1)` +
`lhu rd, offset({creg_pref}s1)` +
`lbu rd, offset({creg_pref}s1)`
...
Subword load instructions, authorized by the capability in `{creg_pref}s1`.
