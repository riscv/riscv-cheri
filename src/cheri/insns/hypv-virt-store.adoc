...
`hsv.b rs2, {creg_pref}s1` +
`hsv.h rs2, {creg_pref}s1` +
`hsv.w rs2, {creg_pref}s1` +
`hsv.d rs2, {creg_pref}s1`
...
capability for the operation is `{creg_pref}s1`. A copy of `rs2` is written to memory at
