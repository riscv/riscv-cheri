...
The resulting capability from <<YSUNSEAL>> will be invalidated if the received capability
...
let tag = cs1_val.tag &
          ...
          cs2_val.tag &
          capIsSealed(cs2_val) &
          /* subset checks for malformed bounds, perms, reserved bits */
          capIsSubset(cs2_val, cs1_val);
