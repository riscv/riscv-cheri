...
`c.ld rd', offset({creg_pref}s1')` +
`c.lw rd', offset({creg_pref}s1')`

{cheri_cap_mode_name} Expansions::
`ld rd', offset({creg_pref}s1')` +
`lw rd', offset({creg_pref}s1')`
...
Standard load instructions, authorized by the capability in `{creg_pref}s1`.
