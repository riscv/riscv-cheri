...
Store the CLEN-bit value in `cs2'` to memory. The capability in `cs1/csp`
...
The capability written to memory has the valid tag set to 0 if the valid tag of `cs2'` is 0 or if the authorizing capability (`cs1/csp`) does not grant <<c_perm>>.
+
