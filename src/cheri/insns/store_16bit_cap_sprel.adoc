...
`{c_store_cap_sp_name_lc} cs2', offset(csp)`

{cheri_cap_mode_name} Expansions::
`{store_cap_name_lc} cs2', offset(cs1')` +
`{store_cap_name_lc} cs2', offset(csp)`

Encoding::
include::wavedrom/c-sp-store-cap.adoc[]

{cheri_cap_mode_name} Description::
Store the YLEN-bit value in `cs2'` to memory. The capability in `cs1/csp`
...
The capability written to memory has the {ctag} set to 0 if the {ctag} of `cs2'` is 0 or if the authorizing capability (`cs1/csp`) does not grant <<c_perm>>.
+
