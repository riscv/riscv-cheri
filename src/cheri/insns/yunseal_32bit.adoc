...
- If the <<section_zylevels1>> extension is implemented,
  and the capability in `{cs1}` does not grant <<zylevels1_gl_flag>>,
  use the semantics of the <<CLRPERM>> instruction
  to clear the <<zylevels1_gl_flag>> of the capability in `{cd}`.
+
(That is, the resulting capability in `{cd}` will grant <<zylevels1_gl_flag>>
if and only if the capabilities in `{cs1}` and `{cs2}` both grant <<zylevels1_gl_flag>>.)
