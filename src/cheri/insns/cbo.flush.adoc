...
Description::
A CBO.FLUSH instruction performs a flush operation on the cache block whose effective address is the base address specified in `{creg_pref}s1`.
The authorizing capability for this operation is `{creg_pref}s1`.
