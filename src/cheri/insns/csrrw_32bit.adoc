...
`csrrw {creg_pref}d, csr, {creg_pref}s1`
...
+
CSRRW accesses to YLEN-bit CSRs read YLEN bits into `{creg_pref}d` and write `{creg_pref}s1` into the CSR.
+
CSRRW accesses to extended CSRs in {cheri_cap_mode_name} read YLEN bits into `{creg_pref}d` and write `{creg_pref}s1` into the CSR.
+
...
+
In all cases, when writing `{creg_pref}s1`, if any <<section_cap_integrity,integrity>> check fails then set the {ctag} to zero before writing to the CSR.
