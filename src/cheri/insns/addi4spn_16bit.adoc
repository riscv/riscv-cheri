...
Add a zero-extended non-zero immediate, scaled by 4, to the stack pointer, `ysp`, and writes the result to `yd'`. This instruction is used to generate pointers to stack-allocated variables. Clear the valid tag if the resulting capability is unrepresentable or `ysp` is sealed.
