...
Add a zero-extended non-zero immediate, scaled by 4, to the stack pointer, `csp`, and writes the result to `cd'`. This instruction is used to generate pointers to stack-allocated variables. Clear the valid tag if the resulting capability is unrepresentable or `csp` is sealed.
