...
+
The target <<pcc>> is calculated by replacing the current <<pcc>> address with the value read from the jump table, and is updated using semantics of the <<SCADDR>> instruction.
+
