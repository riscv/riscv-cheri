...
The effective address of the store is obtained by adding the sign-extended 12-bit offset to the address of `ys1`.
The authorizing capability for the operation is `ys1`.
A copy of `rs2` is written to memory at the location indicated by the effective address and the valid tag bit of each block of memory naturally aligned to YLEN/8 is cleared.
+
...
// indicated by the effective address and the valid tag bit of each block of memory
