...
Copy `{cs1}` to `{cd}`.
+
Set `{cd}.tag=0` if `{cs1}` is sealed, or if `{cs1}` fails any <<section_cap_integrity,integrity>> check.
+
If `{cs1}` grants <<x_perm>> and `{cs1}` passes all <<section_cap_integrity,integrity>> checks, then update the <<m_bit>> of `{cd}` to:
+
