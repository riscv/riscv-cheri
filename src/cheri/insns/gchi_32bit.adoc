...
Mnemonic::
`{gchi_lc} rd, {creg_pref}s1`

Encoding::
`{gchi_lc} rd, {creg_pref}s1` is a pseudoinstruction for `{gchi_base_lc} rd, rs1, XLEN`

Description::
Copy the metadata (bits [YLEN-1:XLEN]) of capability `{creg_pref}s1` into `rd`.
+
