...
Description::
Logical right shift of Y register `rs1`, writing the lower YLEN bits of the result to `rd`.
...
Encoding::
{cheri_base64_ext_name}: {gchi_lc} rd, cs1` is a pseudoinstruction for `{gchi_base_lc} rd, rs1, 64` +
{cheri_base32_ext_name}: {gchi_lc} rd, cs1` is a pseudoinstruction for `{gchi_base_lc} rd, rs1, 32`

NOTE: #needs to be encoded as an XLEN right shift with this as a pseudo-op#
