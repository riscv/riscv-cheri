...
Load *c* registers as specified in _yreg_list_. Deallocate stack frame. Return by calling <<JALR_CHERI>> to `cra`. All data accesses are checked against `ysp`. The return destination is checked against `cra`.
