...
Load *c* registers as specified in _creg_list_. Deallocate stack frame. Return by calling <<JALR_CHERI>> to `cra`. All data accesses are checked against `csp`. The return destination is checked against `cra`.
