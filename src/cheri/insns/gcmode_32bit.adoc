...
. `cs1` does not grant <<x_perm>>, or,
. the AP field in `cs1` cannot be produced by <<ACPERM>>,

Otherwise set `rd` according to `cs1` 's CHERI execution mode (<<m_bit>>):

. Set `rd` to {CAP_MODE_VALUE} for {cheri_cap_mode_name}, or
. Set `rd` to {INT_MODE_VALUE} for {cheri_int_mode_name}.
