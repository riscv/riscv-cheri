...
`fld frd, offset({creg_pref}s1)` +
`flw frd, offset({creg_pref}s1)` +
`flh frd, offset({creg_pref}s1)`
...
Standard floating point load instructions, authorized by the capability in `{creg_pref}s1`.
+
