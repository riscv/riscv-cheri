...
+
For capability CSRs, the full capability is read into `yd` in {cheri_cap_mode_name}.
...
+
If `yd` is `c0` (or `rd` is `x0`), then <<CSRRWI_CHERI,CSRRWI>> shall not read the CSR and
...
`csrr yd, csr`, is encoded as `csrrs yd, csr, x0`.
+
