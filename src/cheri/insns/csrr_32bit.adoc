...
+
All writes are XLEN bits only and use the semantics of the <<SCADDR>> instruction to determine the final write data.
+
Read data from <<ylen_csrs>> is always YLEN bits.
+
Read data from <<extended_csrs>> is YLEN bits in {cheri_cap_mode_name} or, if {cheri_default_ext_name} is supported, XLEN bits in {cheri_int_mode_name}.
