...
+
Unlike <<CSRRW_CHERI>>, these instructions only update the address field and the valid tag as defined in xref:extended_CSR_writing[xrefstyle=short] when writing CLEN-wide CSRs regardless of the execution mode.
The CSR update is made using <<SCADDR>> semantics.
+
