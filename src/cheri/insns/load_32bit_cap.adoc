...
+
The {ctag} may also be set to 0 under platform specified conditions.
+
...
* The RVY privileged specification permits Physical Memory Attributes (PMAs) that cannot store {ctag}s, as might be the case with MMIO.
 The {ctag}s transported by <<STORE_CAP>> instructions to these regions are set to 0 and so will not read back.
* The MMU of the RVY privileged specification introduces page mappings that always set loaded {ctag}s to 0 even if the loaded location has a set {ctag}.
