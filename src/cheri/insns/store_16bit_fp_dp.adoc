...
`c.fsd fs2, offset(ys1')` +
`c.fsd fs2, offset(ysp)`
...
Standard floating point stack pointer relative store instructions, authorized by the capability in `ys1` or `ysp`.
...
In RV64 {cheri_cap_mode_name} they are remapped to <<C.SC>>/<<C.SCSP>>.
