<<<

[#GCPERM,reftext="GCPERM"]
==== GCPERM

include::new_encoding_note.adoc[]

ifdef::cheri_v9_annotations[]
NOTE: *CHERI v9 Note:* this instruction was called CGETPERM.
endif::[]

Synopsis::
Capability get permissions

Mnemonic::
`gcperm rd, cs1`

Encoding::
include::wavedrom/gcperm.adoc[]

Description::
Convert the unpacked <<AP-field>>, <<SDP-field>>, and any other extension-defined permission-like fields of capability `cs1`
into a bit field, with the same format as used by <<ACPERM>> (see <<acperm_bit_field>>),
and write the result to `rd`.
+
All bits in the `[23:0]` range that are reserved or assigned to extensions that are not implemented by the current hart always report 1.
+
If the <<AP-field>> could not have been produced by <<ACPERM>> then all architectural
permission bits in `rd` are set to 0.
+
include::no_tag_affect.adoc[]

Operation::
+
sail::execute[clause="GCPERM(_, _)",part=body,unindent]
