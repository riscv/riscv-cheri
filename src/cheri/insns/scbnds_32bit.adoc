...
address of its bounds replaced with the value of `ys1.address` and
the length of its bounds set to `rs2`  (or `imm`). If the resulting capability
cannot be <<section_cap_representable_check,represented exactly>> then set `yd.tag` to 0.
In all cases, `yd.tag` is set to 0 if its bounds exceed `ys1` 's bounds, `ys1` 's
tag is 0  or `ys1` is sealed.
+
