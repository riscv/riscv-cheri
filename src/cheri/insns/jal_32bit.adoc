...
+
. The target address is obtained by adding the sign-extended 20-bit J-immediate to `{cs1}.address`.
...
+
NOTE: If the only supported <<sec_cap_type>> is zero, then no <<sentry_cap, sentry capabilities>> exist, and so _sealing_ has no effect.
