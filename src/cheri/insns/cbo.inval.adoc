...
Description::
A CBO.INVAL instruction performs an invalidate operation on the cache block whose effective address is the base address specified in `{creg_pref}s1`.
The authorizing capability for this instruction is `{creg_pref}s1`.
