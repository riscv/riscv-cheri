...
Description::
Add the non-zero sign-extended 6-bit immediate to the value in the stack pointer (`ysp=y2`), where the immediate is scaled to represent multiples of 16 in the range (-512,496).
+
Set `ysp.tag=0` if `ysp` is sealed.
+
include::rep_range_check.adoc[]
+
include::malformed_ysp_clear_tag.adoc[]
