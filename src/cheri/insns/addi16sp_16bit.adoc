...
Add the non-zero sign-extended 6-bit immediate to the value in the stack pointer (`ysp=c2`), where the immediate is scaled to represent multiples of 16 in the range (-512,496). Clear the valid tag if the resulting capability is
unrepresentable or `ysp` is sealed.
