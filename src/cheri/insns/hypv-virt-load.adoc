...
`hlv.b[u] rd, {creg_pref}s1` +
`hlv.h[u] rd, {creg_pref}s1` +
`hlv.w[u] rd, {creg_pref}s1` +
`hlv.d    rd, {creg_pref}s1`
...
VU-mode. The effective address is `{creg_pref}s1.address`. The authorizing
capability for the operation is `{creg_pref}s1`. A copy of the loaded value is written to
`rd`.
+
