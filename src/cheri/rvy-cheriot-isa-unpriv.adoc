...
of <<section_cheri_hybrid_ext,{cheri_default_ext_name}>>.
...
[NOTE]
=====
{cheriot_unpriv_ext_name} is incompatible with
<<section_rvy_sentry_insn_ext>> as <<sec_cap_type>> of `1` is not a sentry type.
=====
...
* Two values, {cheriot_sentry_ot_bd} and {cheriot_sentry_ot_be},
  are used by <<JALR_CHERI>> and <<JAL_CHERI>> to seal the return capability when `rd` is `ra`.
