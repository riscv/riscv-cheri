...
* It defines a _root sealing_ capability that grants exactly all of
  <<zyseal_se_perm,SE>>, <<zyseal_us_perm,US>>, and <<zycheriot_u0_perm,U0>>.
...
[[perm_req:zycheriot:mem-nand-ct,mem-nand-ct]]mem-nand-ct::
  The <<zyseal_se_perm,SE>>, <<zyseal_us_perm,US>>, and <<zycheriot_u0_perm,U0>>
...
or uninterpreted integers (those granting <<zycheriot_u0_perm,U0>>).
...
(that is, one has its `LY` `p` parameter set to `pc`).
...
* Two values, {cheriot_sentry_ot_bd} and {cheriot_sentry_ot_be},
  are used by <<JALR_CHERI>> and <<JAL_CHERI>> to seal the return capability when `{cd}` is `ra`.
...
2+| other operands | 0, {cheriot_sentry_ot_fi} | Non-standard control flow (e.g., tail calls)
|===

[NOTE]
=====
All of the <<sentry_cap>> <<sec_cap_type>> values defined herein
need only be encodable if the capability grants <<x_perm>>,
...
and, dually, that a function pointer cannot be substituted for a return pointer.
=====
