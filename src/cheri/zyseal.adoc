...
the introduction and elimination of sealed capability forms,
...
"**type** capabilities", that do not authorize memory access.
Instead, their address and bounds identify a range of <<sec_cap_type>> values.
...
However, capability encodings have fewer than XLEN bits devoted to storing
<<sec_cap_type>> values.
...
Indeed, the encoding of <<rv32y_cap_description>>/<<rv64y_cap_description>> is one such encoding.
...
with type values that are within the bounds of this capability.
