...
|==============================================================================
| | {CRE}=0                            | {CRE}=1^1^, M-bit={int_mode_value} | {CRE}=1, M-bit={cap_mode_value}
...
^2^ The hart is fully compatible with standard RISC-V when {CRE}=0 provided that <<pcc>>, <<mtvec_y>>, <<mepc_y>>, <<stvec_y>>, <<sepc_y>>, <<vstvec_y>>, <<vsepc_y>> and <<ddc>> have not been changed from the default reset state (i.e., hold <<root-rx-cap>> and <<root-rw-cap>> capabilities).
