...
NOTE: {cheri_excep_name_pte_ld} faults are at present raised only if <<section_cheri_priv_crg_ext,{cheri_priv_crg_ext}>> and Svade are implemented.
...
[[pte_crw_summary]]
.Summary of memory access behavior depending on _pte.crw_.
