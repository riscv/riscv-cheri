...
| .>| *{cheri_excep_pc_list}* .<| *CHERI faults due to:* +
*PCC <<asr_perm>> clear* +
...
^2^ The higher priority {cheri_excep_name_pte_ld} covers capability loads ({cheri_priv_crg_ext} only) or atomics where the loaded valid tag _is not_ checked.

^3^ The lower priority {cheri_excep_name_pte_ld} covers capability loads ({cheri_priv_crg_ext} only) or atomics where the loaded valid tag _is_ checked.
...
Bits {cheri_excep_list} of <<medeleg>> now refers to a valid exception and so can be used to
...
| All | {cheri_excep_cause_pc_ytag} |<<pcc>> valid tag  | not(<<pcc>>.tag)
| All | {cheri_excep_cause_pc_seal} |<<pcc>> seal       | isCapSealed(<<pcc>>)
| All | {cheri_excep_cause_pc_perm} |<<pcc>> permission | not(<<pcc>>.<<x_perm>>)
| All | {cheri_excep_cause_pc_bnds} |<<pcc>> bounds     | Any byte of current instruction out of <<pcc>> bounds^1^
4+| *CSR/Xret additional exception check*
| CSR*, <<MRET_CHERI>>, <<SRET_CHERI>> | {cheri_excep_cause_pc_perm} | <<pcc>> permission | not(<<pcc>>.<<asr_perm>>) when required for CSR access or execution of <<MRET_CHERI>>/<<SRET_CHERI>>
4+| *Load additional exception checks*
| All loads        | {cheri_excep_cause_ls_ytag} | `cs1` valid tag            | not(`auth_cap.tag`)
| All loads        | {cheri_excep_cause_ls_seal} | `cs1` seal                 | isCapSealed(`auth_cap`)
| All loads        | {cheri_excep_cause_ls_perm} | `cs1` permission           | not(`auth_cap`.<<r_perm>>)
| all loads        | {cheri_excep_cause_ls_bnds} | `cs1` bounds               | Any byte of load access out of `auth_cap` bounds^1^
| capability loads | 4                           | load address misaligned    | Misaligned capability load
4+| *Store/atomic/cache-block-operation additional exception checks*
| all stores, all atomics, all CBOs              | {cheri_excep_cause_ls_ytag} |`cs1` valid tag           | not(`auth_cap.tag`)
| all stores, all atomics, all CBOs              | {cheri_excep_cause_ls_seal} |`cs1` seal                | isCapSealed(`auth_cap`)
| all atomics, CBO.INVAL*                        | {cheri_excep_cause_ls_perm} |`cs1` permission          | not(`auth_cap`.<<r_perm>>)
| all stores, all atomics, CBO.INVAL*, CBO.ZERO* | {cheri_excep_cause_ls_perm} |`cs1` permission          | not(`auth_cap`.<<w_perm>>)
| CBO.CLEAN*, CBO.FLUSH*                         | {cheri_excep_cause_ls_perm} |`cs1` permission          | not(`auth_cap`.<<r_perm>>) and not(`auth_cap`.<<w_perm>>)
| all stores, all atomics                        | {cheri_excep_cause_ls_bnds} |`cs1` bounds              | any byte of access out of `auth_cap` bounds^1^
| CBO.ZERO*, CBO.INVAL*                          | {cheri_excep_cause_ls_bnds} |`cs1` bounds              | any byte of cache block out of `auth_cap` bounds^1^
| CBO.CLEAN*, CBO.FLUSH*                         | {cheri_excep_cause_ls_bnds} |`cs1` bounds              | all bytes of cache block out of `auth_cap` bounds^1^
| CBO.INVAL*                                     | {cheri_excep_cause_ls_perm} |<<pcc>> permission        | not(<<pcc>>.<<asr_perm>>)
| capability stores                              | 6                           |capability alignment      | Misaligned capability store
|=========================================================================================

^1^ The bounds faults include the cases where the bounds could not be decoded.
...
{cheri_priv_vmem_ext} defines the Capability Write (CW) bit in Page Table Entries (PTEs) for Sv39, Sv48, and Sv57 virtual memory systems on {cheri_base64_ext_name} harts. The CW bit controls whether capabilities with their valid tag set can be written to a virtual page.
...
CHERI adds the concept of _CHERI PTE page faults_. They are split into :
...
Only harts which also implement a revocation scheme such as {cheri_priv_crg_ext} can raise _{cheri_excep_name_pte_ld}_.
...
* A _{cheri_excep_name_pte_st}_ exception is raised when a capability store or AMO instruction is executed and the valid tag bit of the capability being written is set.
...
| 0     |Capability load      | Set loaded valid tag to zero
| 0     |Capability store/AMO | Raise a _{cheri_excep_name_pte_st}_ if the stored valid tag is set
...
If the bounds cannot be decoded due to the address being invalid, and the bounds not being <<infinite-cap>>, then a _{cheri_excep_name_pc_ytag}_ or _{cheri_excep_name_ls_ytag}_ exception is raised as appropriate.
...
NOTE: Replacing an address of a valid capability normally requires an <<SCADDR>>. This specification does not require an <<SCADDR>> when compressing an invalid address to reduce the implementation cost.
...
* If the capability's address is invalid and the capability does not have <<infinite-cap>> bounds, then set the valid tag to zero before writing to the CSR.

NOTE: When the capability's address is invalid and happens to match an invalid address which the CSR
can hold, then it is implementation defined whether to set the valid tag to zero.

==== Branches and Jumps

If the effective target address of the jump or branch is invalid, and the authorizing capability does not have <<infinite-cap>> bounds, then set the valid tag of the target <<pcc>> to zero.
This will cause a {cheri_excep_name_pc_ytag} exception when executing the target instruction.

==== Memory Accesses

If the effective address of the memory access is invalid, and the authorizing capability does not have <<infinite-cap>> bounds, then raise a {cheri_excep_name_ls_bnds} exception because the bounds cannot be reliably decoded.
