...
The {cheri_priv_m_reg_enable_ext} extension makes the {CRE} bit of <<misa>>, <<menvcfg>>, and <<senvcfg>> writable.
...
* the effective {CRE}-enable for the current privilege is 0.

The effective {CRE}-enable for the current privilege is:

* Machine: `<<misa>>.{CRE}`
* Supervisor: `<<misa>>.{CRE} & <<menvcfg>>.{CRE}`
* User: `<<misa>>.{CRE} & <<menvcfg>>.{CRE} & <<senvcfg>>.{CRE}`

NOTE: The effective {CRE}-enable is always 1 in debug mode.

NOTE: On reset {CRE} access is disabled (<<misa>>.{CRE} resets to zero).
...
Disabling {CRE} access allows harts supporting CHERI to be fully
