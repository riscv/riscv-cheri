...
<<mtvec_y>> is always updated using semantics of the <<SCADDR>> instruction and so writing a sealed capability will cause the {ctag} to be set to zero.
...
NOTE: On reset CHERI is always disabled for backwards compatibility (<<misa>>.{CRE} resets to zero, <<ddc>> and <<pcc>> bounds are infinite).
...
// FIXME: there should be no way to observe the M bit, do we need to mention this?
...
do not change the state of CHERI CSRs raise exceptions.
...
| Compressed Instructions Remapped     | No                | No                    | Yes
| Summary                              | **_Fully RISC-V compatible_**^2^ | **{cheri_int_mode_name}** | **{cheri_cap_mode_name}**
|==============================================================================

^1^ M-bit is irrelevant when {CRE}=0. +
instructions. +
^2^ The hart is fully compatible with standard RISC-V when {CRE}=0 provided that <<pcc>>, <<mtvec_y>>, <<mepc_y>>, <<stvec_y>>, <<sepc_y>>, <<vstvec_y>>, <<vsepc_y>> and <<ddc>> have not been changed from the default reset state (i.e., hold the <<infinite-cap>> capability).
