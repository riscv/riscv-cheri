...
* A <<cheri_pte_fault,_{cheri_excep_name_pte_st}_>> exception is raised when a capability store or AMO instruction is executed and the valid tag bit of the capability being written is set.
...
NOTE: Capability addresses cannot generally be updated without using <<SCADDR>> to check that the new address doesn't change the bounds.
 This check is avoided when discarding address bits of invalid addresses to reduce hardware cost, as a CHERI exception is guaranteed on dereferencing the capability.
...
* If the new address is invalid and the capability does not have <<infinite-cap>> bounds, then set the valid tag to zero before writing to the CSR.
