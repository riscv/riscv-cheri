...
| CBO.CLEAN, CBO.FLUSH                           | {cheri_excep_cause_st}     | {cheri_excep_name_st} | All bytes of the cache block is out of `auth_cap` bounds^2^
...
* Machine: `<<misa_y>>.{CRE}`
* Supervisor: `<<misa_y>>.{CRE} & <<menvcfg_y>>.{CRE}`
* User: `<<misa_y>>.{CRE} & <<menvcfg_y>>.{CRE} & <<senvcfg_y>>.{CRE}`
