...
{cheri_default_ext_name} adds <<ddc>> to the registers that must have the capability metadata set to the reset value for `<<misa_y,misa>>.{CRE}` to be programmed to zero.

The effective CHERI-enable for the current privilege is:

* M-mode:    always enabled
* S-mode: `<<menvcfg_y,menvcfg>>.{CRE}`
* U-mode:       `<<menvcfg_y,menvcfg>>.{CRE} & <<senvcfg_y,senvcfg>>.{CRE}`
