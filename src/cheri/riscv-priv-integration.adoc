...
*CHERI fault due to {pcc} checks ({ctag}, execute permission, bounds^1^)*
...
*CHERI fault due to {pcc} <<asr_perm>> clear* +
...
*CHERI fault due to capability checks ({ctag}, sealed, permissions, bounds)*
...
hold capabilities or with other new functions. <<asr_perm>> in the <<pcc>> required for access to all privileged CSRs.
...
In this case all {CRE} bits are hardwired to 1 and CHERI is always enabled.
...
NOTE: On reset CHERI is always disabled for backwards compatibility (<<misa>>.{CRE} resets to zero, <<ddc>> and <<pcc>> bounds are nominally infinite).
...
|==============================================================================
| | {CRE}=0                            | {CRE}=1, M-bit={int_mode_value} | {CRE}=1, M-bit={cap_mode_value}
...
| Summary                              | **_Fully RISC-V compatible_**^1^ | **{cheri_int_mode_name}** | **{cheri_cap_mode_name}**
|==============================================================================

^1^ The hart is fully compatible with standard RISC-V when {CRE}=0 provided that <<pcc>>, <<mtvec_y>>, <<mepc_y>>, <<stvec_y>>, <<sepc_y>>, <<vstvec_y>>, <<vsepc_y>> and <<ddc>> have not been changed from the default reset state (i.e., hold <<root-rx-cap>> and <<root-rw-cap>> capabilities).
...
NOTE: Due to links to the unpriv manual being broken, this improves readability.
...
[#DRET_CHERI, reftext="DRET (RVY)"]
DRET (RVY)::
   See the unprivileged manual for the behavior of DRET for RVY. #should be in debug manual#

[#MRET_CHERI, reftext="MRET (RVY)"]
MRET (RVY)::
   See the unprivileged manual for the behavior of MRET for RVY. #should be in priv manual#

[#SRET_CHERI, reftext="SRET (RVY)"]
SRET (RVY)::
   See the unprivileged manual for the behavior of SRET for RVY. #should be in priv manual#
...
[#HLV_CAP, reftext="HLV.Y"]
HLV.Y::
   See the unprivileged manual for the definition of HLV.Y.

[#HSV_CAP, reftext="HSV.Y"]
HSV.Y::
   See the unprivileged manual for the definition of HSV.Y.
...
[#CSR_exevectors, reftext="YLEN-wide CSRs storing code pointers or data pointers"]
YLEN-wide CSRs storing code pointers or data pointers::
  See the unprivileged manual for the table of these CSRs.

[#dcsrnames-renamed, reftext="Extended debug mode CSRs"]
Extended debug mode CSRs::
  See the unprivileged manual for the table of these CSRs. #clearly in the wrong place#
