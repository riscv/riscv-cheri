...
When the hart attempts to store data with the valid tag set to memory regions that are not taggable, the implementation ignores the stored valid tag.

NOTE: A PMA from a future extension could raise an access fault exception when attempting to store a valid tag to non taggable memory.
