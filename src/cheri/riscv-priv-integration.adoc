...
<<mtvec_y>> need not be capable of holding all possible invalid metadata values.
Prior to writing <<mtvec_y>>, implementations may convert invalid metadata values into some other invalid metadata value that <<mtvec_y>> is capable of holding.
...
<<mepc_y>> need not be capable of holding all possible invalid metadata values.
Prior to writing <<mepc_y>>, implementations may convert invalid metadata values into some other invalid metadata value that <<mepc_y>> is capable of holding.

NOTE: <<JALR_CHERI>> may jump to a target <<pcc>> value with invalid metadata, and the target raises the exception.
 The WARL behavior means that the full invalid metadata does not have to be maintained before being reported.
