...
When the hart attempts to store data with the valid tag set to memory regions that do not have the _CHERI taggable_ PMA set, the implementation ignores the stored valid tag.

NOTE: A future extension could raise an access fault exception when attempting to store a valid tag to non taggable memory.
