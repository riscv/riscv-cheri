...
NOTE: {cheri_base_ext_name} does not modify the behavior of SFENCE.\*, or SINVAL.*.
Address operands passed to them are received as XLEN-bit values, and so the metadata and {ctag} are ignored.
A future extension may add CHERI checks to these instructions.
