...
| All | {cheri_excep_cause_pc} |<<pcc>> valid tag  | not(<<pcc>>.tag)
| All | {cheri_excep_cause_pc} |<<pcc>> seal       | isCapSealed(<<pcc>>)
| All | {cheri_excep_cause_pc} |<<pcc>> permission | not(<<pcc>>.<<x_perm>>)
| All | {cheri_excep_cause_pc} |<<pcc>> bounds     | Any byte of current instruction out of <<pcc>> bounds^1^
4+| *CSR/Xret additional exception check*
| CSR*, <<MRET_CHERI>>, <<SRET_CHERI>> | {cheri_excep_cause_pc} | <<pcc>> permission | not(<<pcc>>.<<asr_perm>>) when required for CSR access or execution of <<MRET_CHERI>>/<<SRET_CHERI>>
4+| *Load additional exception checks*
| All loads        | {cheri_excep_cause_ld}      | `cs1` valid tag            | not(`auth_cap.tag`)
| All loads        | {cheri_excep_cause_ld}      | `cs1` seal                 | isCapSealed(`auth_cap`)
| All loads        | {cheri_excep_cause_ld}      | `cs1` permission           | not(`auth_cap`.<<r_perm>>)
| all loads        | {cheri_excep_cause_ld}      | `cs1` bounds               | Any byte of load access out of `auth_cap` bounds^1^
...
| all stores, all atomics, all CBOs              | {cheri_excep_cause_st}     |`cs1` valid tag           | not(`auth_cap.tag`)
| all stores, all atomics, all CBOs              | {cheri_excep_cause_st}     |`cs1` seal                | isCapSealed(`auth_cap`)
| all atomics, CBO.INVAL*                        | {cheri_excep_cause_st}     |`cs1` permission          | not(`auth_cap`.<<r_perm>>)
| all stores, all atomics, CBO.INVAL*, CBO.ZERO* | {cheri_excep_cause_st}     |`cs1` permission          | not(`auth_cap`.<<w_perm>>)
| CBO.CLEAN*, CBO.FLUSH*                         | {cheri_excep_cause_st}     |`cs1` permission          | not(`auth_cap`.<<r_perm>>) and not(`auth_cap`.<<w_perm>>)
| all stores, all atomics                        | {cheri_excep_cause_st}     |`cs1` bounds              | any byte of access out of `auth_cap` bounds^1^
| CBO.ZERO*, CBO.INVAL*                          | {cheri_excep_cause_st}     |`cs1` bounds              | any byte of cache block out of `auth_cap` bounds^1^
| CBO.CLEAN*, CBO.FLUSH*                         | {cheri_excep_cause_st}     |`cs1` bounds              | all bytes of cache block out of `auth_cap` bounds^1^
| CBO.INVAL*                                     | {cheri_excep_cause_st}     |<<pcc>> permission        | not(<<pcc>>.<<asr_perm>>)
...
If the bounds cannot be decoded due to the address being invalid, and the bounds not being <<infinite-cap>>, then a _{cheri_excep_name_pc}_, _{cheri_excep_name_ld}_ or _{cheri_excep_name_st}_ exception is raised as appropriate.
