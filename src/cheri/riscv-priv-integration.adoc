...
However, in case {cheri_base_ext_name} is the only supported base ISA and there is no option to switch to RV32I/RV64I (i.e. {cheri_default_ext_name} is not supported), then the CRE bits are hardwired to 1.
...
<<stvecc>>, <<sepcc>>, <<vstvecc>>, <<vsepcc>> and <<ddc>> have not been changed from the default reset state (i.e. hold the
<<infinite-cap>> capability).
