...
<<section_cap_representable_check>>, and the valid tag cleared if _any_ are not.
...
NOTE: `auth_cap` is <<ddc>> for {cheri_int_mode_name} and `ys1` for {cheri_cap_mode_name}
...
| indirect jumps | {cheri_excep_mcause} | {cheri_excep_type_jump} | {cheri_excep_cause_tag}      |`ys1` tag             | not(`ys1.tag`)
| indirect jumps | {cheri_excep_mcause} | {cheri_excep_type_jump} | {cheri_excep_cause_seal}     |`ys1` seal            | isCapSealed(`ys1`) and imm12 != 0
| indirect jumps | {cheri_excep_mcause} | {cheri_excep_type_jump} | {cheri_excep_cause_perm}     |`ys1` permission      | not(`ys1`.<<x_perm>>)
| indirect jumps | {cheri_excep_mcause} | {cheri_excep_type_jump} | {cheri_excep_cause_inv_addr} |`ys1` invalid address | target address is an invalid address
| indirect jumps | {cheri_excep_mcause} | {cheri_excep_type_jump} | {cheri_excep_cause_bounds}   |`ys1` bounds          | any byte of minimum length instruction at target out of `ys1` bounds
...
Therefore, it is desirable that harts supporting {cheri_base_ext_name} extend PMAs with a _CHERI taggable_ attribute indicating whether a memory region allows storing CHERI valid tags.

Data loaded from memory regions that are not _CHERI taggable_ will always have the valid tag cleared.
When the hart attempts to store data with the valid tag set to memory regions that are not taggable, the implementation may:
...
* Instructions that access *y* registers (implicitly or explicitly)
...
* The capability in the *y* input register of a jump when in
