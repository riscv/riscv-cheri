...
^1^ These exceptions are defined by the {cheri_priv_crg_ext} extension.
...
The exception codes in <<exception-cause-cheri>> can be reported in <<scause>>.
...
^2^ The load/store bounds check includes the case where the bounds could not be decoded as they are <<section_cap_malformed,malformed>>.
...
This includes allowing custom encodings reallocated to CHERI to revert to being custom.
{cheri_default_ext_name} includes functions to disable explicit access to CHERI
registers and instructions (hereafter referred to as _disabling CHERI_).
...
* Custom encoding spaces reallocated as standard encoding spaces for {cheri_base_ext_name} revert to being custom encoding spaces, and so the encodings in <<rvy_insn_table>> and <<rvy_hybrid_insn_table>> are no longer available.
* Natively YLEN CSRs are no longer available, and may have been mapped to different standard CSRs.
...
compatible with standard RV32[IE]/RV64[IE] base architectures,
including any instructions mapped into the reallocated custom encoding spaces.
...
^2^ The compressed instructions operating on capability data are _unavailable_ as their encodings will revert to non-CHERI standard behavior.

^3^ The hart is fully compatible with standard RISC-V when {CRE}=0 provided that <<pcc>>, Xtvec, Xepc and <<ddc>> have not been changed from the default reset state (i.e., hold <<root-rx-cap>> and <<root-rw-cap>> capabilities).
...
{cheri_base64_ext_name} adds the the following new fault type:

* {cheri_excep_name_pte_st} (cause value {cheri_excep_cause_pte_st})

It is prioritized against other fault types as shown in <<exception-priority-cheri>>.
