...
{cheri_sv39_ext}, {cheri_sv48_ext} and {cheri_sv57_ext} imply Sv39, Sv48 and Sv57 respectively on {cheri_base_ext_name} architectures.
...
[NOTE]
====
As RV32 has no field available to be allocated in the PTE, capability loads and stores operate as normal, i.e there is no {cheri_base_ext_name} version of Sv32. +
Therefore, the {cheri_base_ext_name} field is allocated in the RV64 PTE formats only (e.g., <<sv39pte>>) for use by {cheri_base64_ext_name} extensions. +
...
{cheri_default_mext_name} is implied by {cheri_default_ext_name} and includes the ability to disable CHERI by making the {CRE} bit of <<menvcfg_y>> writable.

{cheri_default_sext_name} is implied by {cheri_default_ext_name} if the implementation supports supervisor mode and includes the ability to disable CHERI by making the {CRE} bit of <<senvcfg_y>> writable.
...
* Supervisor-mode^1^: `<<menvcfg_y,menvcfg>>.{CRE}`
* User-mode:       `<<menvcfg_y,menvcfg>>.{CRE} & <<senvcfg_y,senvcfg>>.{CRE}`

^1^ This is also the condition for user-mode for implemenations which don't support supervisor mode.
