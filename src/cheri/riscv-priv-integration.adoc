[#section_priv_cheri]
== "{cheri_priv_m_ext}" Extension for Machine-Level Architecture, Version 1.0

This chapter describes integration of {cheri_base_ext_name} with the RISC-V privileged architecture.
{cheri_priv_m_ext} is mandatory for all {cheri_base_ext_name} implementations.
...
as it holds a read-only address solely for use by _low-level system software_
and is not interpreted by hardware.
Such software can be expected to hold suitable capabilities
from which it can derive a capability to access the data structures at the target address.
=====

[#mtvec_y,reftext="mtvec ({cheri_base_ext_name})"]
==== Machine Trap Vector Base Address Capability Register (mtvec)

The WARL `mtvec` register is widened to a capability.
The suggested reset value is a <<root-rx-cap>> capability.

NOTE: <<mtvec_y>> exists in all CHERI implementations, and so may be used as a source of a <<root-rx-cap>> capability after reset.

.Machine trap-vector base-capability register
include::img/mtveccreg.edn[]

NOTE: Implementations may use the WARL behavior to restrict the metadata to only values valid for code pointers.
...
When a trap is taken into M-mode, the capability in <<mtvec_y>> is installed in the <<pcc>>, with the address determined by the standard `mtvec` behavior.
...
NOTE: Implementations may use the WARL behavior to restrict the metadata to values only valid for code pointers.

NOTE: <<JALR_CHERI>> may jump to a target <<pcc>> value with invalid metadata.
 The exception is raised at the target <<pcc>>.
 The WARL behavior means that the metadata reported can be restricted to valid <<pcc>> values.
 This allows invalid metadata fields not to need to be maintained within <<pcc>> values within the microarchitecture. +
 As _xepc_ CSRs are WARL, implementations should take care not to fix too many fields so that debug becomes difficult.
 For example, if the <<x_perm>> field is hardwired to one, then it may be hard to diagnose a fault caused by jumping to a <<pcc>> without <<x_perm>>, as the bit will read as one.
...
Invalidate the capability on writing if:
...
The <<mtidc>> register is used to identify the current software thread in M-mode, using the method defined in the section for the unprivileged <<utidc>> CSR.
...
Zero is only a legal value for <<misa_y,misa>>.{CRE} if the capability metadata of <<pcc>>, `__x__tvec`, and `__x__epc`  are currently configured as their respective reset values.
...
[#mstatusreg_pte]
.Machine status (*mstatus*) register for {cheri_base64_ext_name}
...
* The MXL field, and the SXL and UXL fields that control the
value of XLEN for S-mode and U-mode, must be read-only when the base architecture is {cheri_base_ext_name}, with SXL and UXL equal to MXL.
  Only 1 and 2 are supported values.

* The MBE, SBE, and UBE fields that control the memory system endianness for M-mode, S-mode, and U-mode must be read-only when the base architecture is {cheri_base_ext_name}.
...
delegate CHERI exceptions to S-mode.
...
^1^ The access fault is triggered on all capability stores or atomics such as <<STORE_CAP>> or <<AMOSWAP_CAP>> when <<c_perm>> and <<w_perm>> are granted and the to-be-stored capability is valid.
 For <<STORE_COND_CAP>>, the access fault is raised if the to-be-stored capability in `{cs2}` is valid, independent of the reservation outcome.
...
=== M-mode YLEN CSR Summary

<<all_capability_CSRs_m>> shows all YLEN M-mode CSRs added or modified by {cheri_base_ext_name}.

.All {cheri_base_ext_name} M-mode CSRs.
...
The platform may reset those CSRs to a smaller memory range, or to grant fewer permissions.

.Action taken on writing to YLEN-bit M-mode CSRs
...
.YLEN-bit M-mode CSRs storing code pointers or data pointers
[#CSR_exevectors_m]
...
== "{cheri_priv_s_ext}" Extension for Supervisor-Level Architecture, Version 1.0

{cheri_priv_s_ext} is mandatory for all {cheri_base_ext_name} implementations that support S-mode.
...
The WARL `stvec` register is widened to a capability.
...
The handling of <<stvec_y>> is identical to <<mtvec_y>>, but in S-mode.

[#sstatus_y, reftext="sstatus ({cheri_base_ext_name})"]
==== Supervisor Status Registers (sstatus)

[#sstatusreg_pte]
.Supervisor status (*sstatus*) register when SXLEN=64
[wavedrom, ,svg,subs=attributes+]
....
{reg: [
  {bits:  1, name: 'WPRI'},
...
The WARL `sepc` register is widened to a capability.

{ROOT_RX_RESET_SCSR}

The handling of <<sepc_y>> is identical to <<mepc_y>>, but in S-mode (i.e. applying to <<SRET_CHERI>>).
...
The <<stidc>> register is used to identify the current software thread in S-mode, using the method defined in the section for the unprivileged <<utidc>> CSR.
...
=== S-mode YLEN CSR Summary

<<all_capability_CSRs_s>> shows all YLEN S-mode CSRs added or modified by {cheri_base_ext_name}.

.All {cheri_base_ext_name} S-mode CSRs.
...
.Action taken on writing to YLEN-bit S-mode CSRs
...
.YLEN-bit S-mode CSRs storing code pointers or data pointers
[#CSR_exevectors_s]
[width="100%",options=header,cols="1,1,1,1,1"]
|==============================================================================
...
== "{cheri_priv_vmem_ext}" Page-Based Virtual-Memory Extensions, Version 1.0

When the base architecture is {cheri_base64_ext_name}, the Sv39, Sv48, and Sv57 page-based virtual-memory extensions allocate the `{cheri_base_ext_name}` PTE field to control capability stores.
...
* All capability stores of valid capabilities raise a {cheri_excep_name_pte_st}.
...
However, the bounds encoding of capabilities depends on the address value if the bounds are not <<infinite_bounds,infinite>>.

Therefore, if the bounds are not <<infinite_bounds,infinite>>, implementations must not convert an invalid address to a different invalid address, except as permitted by the rules in the following subsections.
...
* If the new address is invalid and the capability bounds are not <<infinite_bounds,infinite>> then invalidate the capability before writing to the CSR.

NOTE: When the new address is invalid and happens to match an invalid address which the CSR
can hold, then it is implementation-defined whether to invalidate the capability.

==== Branches and Jumps

If the effective target address of the jump or branch is invalid, and the authorizing capability's bounds are not <<infinite_bounds,infinite>>, then set the {ctag} of the target <<pcc>> to zero.
...
If the effective address of the memory access is invalid, and the authorizing capability's bounds are not <<infinite_bounds,infinite>>, then raise a {cheri_excep_name_ld} or {cheri_excep_name_st} exception because the bounds cannot be reliably decoded.
...
[#unpriv-csrnames-added, reftext="Unprivileged YLEN-bit CSRs added in {cheri_default_ext_name}"]
Unprivileged YLEN-bit CSRs added in {cheri_default_ext_name}::
...
== "{cheri_priv_hybrid_exts}" Extensions, Version 1.0
...
When using a system with {cheri_default_ext_name}, it may be desirable to configure privilege levels below M-mode such that they operate as a standard RV32[IE]/RV64[IE] base architecture.

{cheri_default_mext_name} is mandatory for all {cheri_default_ext_name} implementations.
{cheri_default_mext_name} adds the ability to disable CHERI in lower privilege modes by making the `{CRE}` bit of <<menvcfg_y>> and `<<misa_y,misa>>.{CRE}` writable, and adds <<ddc>> to the registers that must have the capability metadata set to the reset value for `<<misa_y,misa>>.{CRE}` to be programmed to zero.

{cheri_default_sext_name} is mandatory for all {cheri_default_ext_name} implementations that support S-mode, and adds the ability to disable CHERI in U-mode by making the `{CRE}` bit of <<senvcfg_y>> writable.

The effective CHERI-enable for the current privilege is:

* M-mode: always enabled
* S-mode: `<<menvcfg_y,menvcfg>>.{CRE}`
* U-mode: `<<menvcfg_y,menvcfg>>.{CRE} & <<senvcfg_y,senvcfg>>.{CRE}` (or `<<menvcfg_y,menvcfg>>.{CRE}` if S-mode is not implemented)

To allow standard RVI/RVE code to run at privilege levels below M-mode the registers are initialized as follows:

* `<<menvcfg_y,menvcfg>>.{CRE}=0`
* `<<senvcfg_y,senvcfg>>.{CRE}=0`
* <<pcc>>.<<p_bit>>={INT_MODE_VALUE} (for {cheri_int_mode_name}).
* <<ddc>> is at most the <<root-rw-cap>> capability.
* <<pcc>>, `__x__tvec`, `__x__epc` are at most <<root-rx-cap>> capabilities.
...
* No CSRs added by {cheri_base_ext_name} are available.
...
* Exceptions and interrupts use the full capability held in `__x__tvec` and `__x__epc`.
...
| {cheri_base_ext_name} CSRs available | ✘                 | ✔                     | ✔
| Widened CSR access width             | XLEN              | XLEN                  | YLEN
