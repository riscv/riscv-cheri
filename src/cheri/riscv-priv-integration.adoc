...
#Changed since last ARC review#

When the {cheri_base_ext_name} base ISA is implemented
and the platform incorporates {rvy_sentry_jalr_ext_name},
the <<otherpriv,trap-return instructions>> (`MRET` and `SRET`) read the full YLEN bits of the <<mepc_y>>/<<sepc_y>> register and unseal it prior to exception return,
if the read capability's type
is defined as acceptable for <<JALR_CHERI>> to unseal.
(If there are conditions on <<JALR_CHERI>>'s ability
to unseal particular types, those do not apply here;
that is, if there is any circumstance
under which <<JALR_CHERI>> would unseal the type in question,
`MRET` and `SRET` unseal the capability in question.)
