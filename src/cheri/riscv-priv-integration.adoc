...
capability read or written has its valid tag bit cleared if the legalized address is
...
^1^ This check is architecturally required, but is impossible to encounter so may not be required in an implementation.
...
* Implicitly set the stored valid tag to 0
