...
For all CHERI PC faults ({cheri_excep_pc_list}), <<mtval>> is written with the
MXLEN-bit effective PC address which caused the fault.

For all CHERI Data faults ({cheri_excep_ls_list}), <<mtval>> is written with the
MXLEN-bit effective address which caused the fault.
...
The <<mtval2>> register is required for CHERI machine mode when virtual memory is supported.

When a <<section_priv_cheri_vmem,CHERI PTE page fault>>, is taken into M-mode, <<mtval2>> is written
...
| All loads        | {cheri_excep_cause_ls_ytag} | `auth_cap` valid tag       | not(`auth_cap.tag`)
| All loads        | {cheri_excep_cause_ls_seal} | `auth_cap` seal            | isCapSealed(`auth_cap`)
| All loads        | {cheri_excep_cause_ls_perm} | `auth_cap` permission      | not(`auth_cap`.<<r_perm>>)
| all loads        | {cheri_excep_cause_ls_bnds} | `auth_cap` bounds          | Any byte of load access out of `auth_cap` bounds^1^
...
| all stores, all atomics, all CBOs              | {cheri_excep_cause_ls_ytag} |`auth_cap` valid tag      | not(`auth_cap.tag`)
| all stores, all atomics, all CBOs              | {cheri_excep_cause_ls_seal} |`auth_cap` seal           | isCapSealed(`auth_cap`)
| all atomics, CBO.INVAL*                        | {cheri_excep_cause_ls_perm} |`auth_cap` permission     | not(`auth_cap`.<<r_perm>>)
| all stores, all atomics, CBO.INVAL*, CBO.ZERO* | {cheri_excep_cause_ls_perm} |`auth_cap` permission     | not(`auth_cap`.<<w_perm>>)
| CBO.CLEAN*, CBO.FLUSH*                         | {cheri_excep_cause_ls_perm} |`auth_cap` permission     | not(`auth_cap`.<<r_perm>>) and not(`auth_cap`.<<w_perm>>)
| all stores, all atomics, all CBOs              | {cheri_excep_cause_ls_bnds} |`auth_cap` invalid address| Address is invalid (see <<section_invalid_addr_conv>>)
| all stores, all atomics                        | {cheri_excep_cause_ls_bnds} |`auth_cap` bounds         | any byte of access out of `auth_cap` bounds^1^
| CBO.ZERO*, CBO.INVAL*                          | {cheri_excep_cause_ls_bnds} |`auth_cap` bounds         | any byte of cache block out of `auth_cap` bounds^1^
| CBO.CLEAN*, CBO.FLUSH*                         | {cheri_excep_cause_ls_bnds} |`auth_cap` bounds         | all bytes of cache block out of `auth_cap` bounds^1^
...
However, the bounds encoding of capabilities in {cheri_base_ext_name} depends on the address value if the bounds are not of the <<infinite-cap>> capability.
...
NOTE: Replacing an address of a valid capability normally requires an <<SCADDR>>. This specification does not require an <<SCADDR>> when compressing an invalid address to reduce the implentation cost.

NOTE: Not requiring to decode the bounds for invalid addresses reduces the size of bounds comparators to the supported virtual address width, not the maximum address width.
