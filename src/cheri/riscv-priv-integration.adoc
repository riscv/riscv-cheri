...
[#supervisor-level-widended-csrs-section]
...
When the {cheri_base_ext_name} base ISA is implemented, the <<otherpriv,trap-return instructions>> (`MRET` and `SRET`) read the full YLEN bits of the <<mepc_y>>/<<sepc_y>> register and unseal it prior to exception return.
...
The CSRs shown in xref:CSR_exevectors[xrefstyle=short], as well as the <<pcc>>, need not hold all possible invalid addresses.
