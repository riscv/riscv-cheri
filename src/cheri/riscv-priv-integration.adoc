...
^2^ The higher priority {cheri_excep_name_pte_ld} covers capability loads or atomics where the loaded valid tag _is not_ checked ({cheri_priv_crg_ext} is implemented) .

^3^ The lower priority {cheri_excep_name_pte_ld} covers capability loads or atomics where the loaded valid tag _is_ checked ({cheri_priv_crg_load_tag_ext} is implemented).
...
| CBO.INVAL                                      | {cheri_excep_cause_st}     | {cheri_excep_name_st} | <<pcc>> does not have <<asr_perm>>^2^
...
^2^ The <<asr_perm>> check is only required if CBO.INVAL is not executing as CBO.FLUSH, and the PMA is _CHERI Valid Tag_.
 This is to guard against exposing stale valid tags in the memory system.
