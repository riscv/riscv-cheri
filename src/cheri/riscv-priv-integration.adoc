...
As listed above for <<mtvec_y>>, this means that `mepcc.address` can represent multiple different values.
...
^1^ The access fault is triggered on all capability stores or atomics such as <<STORE_CAP>> or <<AMOSWAP_C>> when <<c_perm>> and <<w_perm>> are granted and the {ctag} is set to one.
