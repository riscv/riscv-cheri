...
NOTE: Examples of WARL behavior include always setting <<x_perm>> to 1 and setting the reserved fields to zero, otherwise the capability is unusable.
 Another example is to set the bounds to a constant value.
...
. `mepcc.address[1]=1` and IALIGN=16 when writing the CSR, and
. IALIGN=32 when reading the CSR, or executing <<MRET_CHERI>>, cause the valid tag to be observed as zero
...
On reset the valid tag of <<stidc>> will be set to 0 and the remainder of the data is UNSPECIFIED.
...
| indirect jumps | {cheri_excep_mcause} | {cheri_excep_type_jump} | {cheri_excep_cause_tag}      |`cs1` valid tag       | not(`cs1.tag`)
...
^1^ The CHERI architecture requires that the <<pcc>> is valid and unsealed.
 It is not currently possible to write a valid sealed capability to the <<pcc>>.
 All methods of writing a new <<pcc>> value either unseal or invalidate the capability.
 If future extensions ever allow the <<pcc>> to be valid and sealed then the architecture would require this check.
...
Therefore, it is desirable that harts supporting {cheri_base_ext_name} extend PMAs with a _CHERI taggable_ Physical Memory Attribute indicating whether a memory region allows storing CHERI valid tags.
...
#ARC question do we want both? should they be different PMAs?#

* Cause an access fault exception
* Ignore the stored valid tag

[#section_vm]
=== Virtual Memory

CHERI checks are made on the effective address according to the current translation scheme.
I.e., on the virtual address if translation is enabled or the physical address if transaltion is disabled.
...
xref:section_cap_encoding[xrefstyle=short]), then A's valid tag is set to 0.
