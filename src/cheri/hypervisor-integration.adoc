...
The handling of <<vsepc_y>> is identical to <<sepc_y>>, but in when V=1 (i.e. applying to <<SRET_CHERI>>).
...
The <<vstidc>> register is used to identify the current software thread in VS-mode, substituting for <<stidc>>.
