...
IMPORTANT: {not_v1_ratification_package_stable}
...
VSXL must be read-only.
When the implementation supports both {cheri_base_ext_name}
and {cheri_default_ext_name}, then VSXL behaves as mstatus.SXL.

The VSBE field controls the endianness of explicit memory accesses
...
registers is permitted when V=1. When <<henvcfg_cheri>>.{CRE}=1 and <<menvcfg_y>>.{CRE}=1
and <<misa_y>>.{CRE}=1, CHERI can be enabled by VS-mode and
...
The {CRE} bit is reset to 0 for compatibility, so that non-CHERI aware code can run unmodified.

[#hedeleg,reftext="hedeleg"]
=== Hypervisor Exception Delegation Register (hedeleg)

Bits {cheri_excep_cause_list} of <<hedeleg>> refer to valid CHERI exceptions and so can be used to
...
must be read-only. When the implementation supports both {cheri_base_ext_name}
...
<<vsepc_y>> is an executable capability, so it does not need to be able to hold all possible invalid addresses (see <<section_invalid_addr_conv>>).
Additionally, the capability in <<vsepc_y>> is unsealed if it is a <<sentry_cap>> when it is written to <<pcc>> on execution of an <<SRET_CHERI>> instruction when V=1.
...
As with other Virtual Supervisor registers when V=1, <<vstidc>> substitutes
...
instructions change behavior depending on the {cheri_mode_name} although
