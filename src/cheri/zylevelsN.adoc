...
* If this field is zero, then any capability with a <<zylevelsN_cl_field>> of zero (i.e., _local_), will be invalidated before being stored.
...
Elevate Level Permission (EL):: Any valid unsealed capability that is loaded from memory has its <<zylevelsN_el_perm>> cleared and its <<zylevelsN_cl_field>> restricted to the authorizing capability's <<zylevelsN_cl_field>> if the authorizing capability does not grant <<zylevelsN_el_perm>>.
+
...
If `{cd}.tag=1`, `{cd}` is not sealed and `{cs1}` does not grant <<zylevelsN_el_perm>>, then an implicit <<CLRPERM>> is performed restricting the <<zylevelsN_cl_field>> of `{cd}` to the level of `{cs1}` and to remove <<zylevelsN_el_perm>>.
+
If `{cd}.tag=1`, `{cd}` is sealed and `{cs1}` does not grant <<zylevelsN_el_perm>>, then an implicit <<CLRPERM>> is performed restricting the <<zylevelsN_cl_field>> of `{cd}` to the level of `{cs1}`
+
...
.3+.^|1  .3+.^| 1   | 1    | X    | Store unmodified data capability
               .2+.^| 0    | 1    | Store unmodified data capability (`CL ≥ ~SL`)
                           | 0    | Store invalidated data capability (`CL < ~SL`)
|==============================================================================
...
|==============================================================================
   4+|Auth cap field                  2+| Data cap field |
    h|*R*   h|*C* h|*EL*      h|*CL*   h| {ctag_table} h| Sealed h|Action
...
When storing capabilities, the <<zylevelsN_sl_perm>> checks need to perform a `<<cheri_lvlbits>>`-wide integer comparison instead of just testing a single bit.
Considering for an example `<<cheri_lvlbits>>=2`:

[options=header,grid=rows,cols="2,3,6"]
|===
|<<zylevelsN_sl_perm>> | Permitted for levels| Resulting semantics
...
NOTE: While this extra negation is non-intuitive, it is required such that <<CLRPERM>> can use a monotonically decreasing operation for both <<zylevelsN_cl_field,CL>> <<zylevelsN_sl_perm>>.

NOTE: The layout of the <<CLRPERM>> input / <<GCPERM>> result is not yet defined, but existing bits will not be moved around so the <<zylevelsN_sl_perm,SL>>/<<zylevelsN_cl_field,CL>> fields will be non-contiguous.
