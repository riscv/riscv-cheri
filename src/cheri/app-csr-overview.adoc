...
CLEN-bits wide, as the *x* registers are renamed to *c* registers.
Even though the CSR is extended, the CSR index remains identical.

NOTE: If {cheri_default_ext_name} is supported then the <<cheri_execution_mode>> determines whether CLEN or XLEN-bits are returned (see <<CSRRW_CHERI>>).
