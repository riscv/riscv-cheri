...
NOTE: {SCHI_BASE} and {GCHI_BASE} are actual instructions. {SCHI} and {GCHI} are pseudoinstructions.

NOTE: ACPERM was replaced by {CLRPERM}. The difference being that the mask is used to clear, not retain, permission bits.
...
NOTE: 0.9.5 had SH[123]ADD, and the .UW forms, replaced by capability versions. This is no longer the case, so now the capability versions have new encodings.

NOTE: There is no longer an SH4ADD instruction (i.e. the integer version).
