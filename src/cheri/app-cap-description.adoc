...
ifdef::cheri_nolevels[]
.Encoding of architectural permissions for MXLEN=32 when {cheri_0levels_ext_name} is implemented
[#cap_perms_encoding_nolevels32,width="100%",options=header,cols="^2,^1,^1,^1,^1,^1,^1,^2,4",align="center"]
|==============================================================================
...
NOTE: Even though valid capabilities which fail the integrity check could not have been legally generated,
 defining the handling in the architecture allows the behavior to be precisely specified for all 2^(CLEN+1)^ input values.

Implementing these checks is optional, as all detected cases are theoretically unreachable.

IMPORTANT: The default is to _include_ integrity checks, and they are _excluded_ in all cases to save logic area if {cheri_nointegrity_ext} is implemented.
...
|==============================================================================
| Field    | Value  | Comment
| Valid Tag                | zero   | Capability is not valid
...
| <<section_cap_level,CL>> | zero   | _Local_
| CT       | zero   | Unsealed
