...
* used as the sealing type for <<SENTRY>> instructions,
  if <<section_rvy_sentry_insn_ext>> is present in the platform.

Additionally, if the platform includes the <<section_zysentry>> extension, then
<<JALR_CHERI>> both

* unseals input capabilities of type 1 and

* seals its return capabilities at type 1.
