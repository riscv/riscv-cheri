[appendix]
[#app_cap_description, reftext="Default capability encoding ({rvy_default_encoding_name})"]
== {rvy_default_encoding_name} - The Default Capability Encoding in {cheri_base_ext_name}
...
If no capability encoding format extension is specified then the selected format is always <<uycfg>>.base=0, <<uycfg>>.features=1.
