...
|===
| Rule             | Permission   | Only valid if
| RV32-Zylevels1-1 | <<zylevels1_sl_perm>>  | (<<lm_perm>> and (<<x_perm>> or <<w_perm>>))
| RV32-Zylevels1-2 | <<lm_perm>>  | (<<w_perm>> or <<zylevels1_lg_perm>>)
| RV32-Zylevels1-3 | <<x_perm>>   | (<<c_perm>> and <<lm_perm>> and <<zylevels1_lg_perm>> and <<zylevels1_sl_perm>>) or +
...
along with any added by extensions or the chosen capability format,
...
| 10  (RV32 only) | <<lm_perm>>  | (<<w_perm>> or <<zylevels1_lg_perm>>)
| 14              | <<asr_perm>> | <<x_perm>>
| 15^1^           | <<m_bit>>    | <<x_perm>> and {cheri_default_ext_name} is implemented
|===
