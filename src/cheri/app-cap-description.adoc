...
MXLEN without including the {ctag} bit. The value of YLEN is always calculated
...
The base definition of capabilities in {cheri_base_ext_name} does not have capability-mediated Information Flow Control (IFC) mechanisms.
However, we anticipate that extensions may introduce such schemes;
see <<section_zylevels1>> for one possibility.
...
[NOTE]
=====
These are sketches and subject to revision before this PR is merged.
=====

This encoding may be instantiated with "minimal, implicit sealing support".
This entails the presence of <<section_zyseal>> and <<section_zysentry>>
and it specifies a one bit wide CT field.
This field encodes <<sec_cap_type>> `0` (unsealed capabilities) with value `0`
and type `1` with value `1`.
_All_ capabilities are presumed to bear <<zyseal_se_perm>>;
it is not possible to clear this permission without clearing the tag.
_No_ capability grants <<zyseal_us_perm>>,
even the <<root-cap>> capability of <<section_infinite_cap_encoding>>;
the sole mechanisms available for unsealing are <<JALR_CHERI>> and <<CBLD>>.
<<JALR_CHERI>> is defined to seal its return capabilities as sentries at type 1
and to accept (and unseal) input sentry capabilities sealed at type 1;
no further conditioning on its behavior is imposed
(so sentries are accepted from any register
and jumps to sentries may themselves give rise to return capabilities).

This encoding may be instantiated with "minimal, explicit sealing support".
This too entails the presence of <<section_zyseal>> and <<section_zysentry>>
and shares the same one bit wide CT field,
designation of <<sec_cap_type>> `1`,
and <<JALR_CHERI>> behavior
with the "minimal, ambient sealing support" above.
However, in this case, <<zyseal_se_perm>> is manifest in the (encoded) AP field.

Other instantiations of <<section_zyseal>> and <<section_zysentry>>
are certainly possible and are left for future versions of this encoding.

[NOTE]
=====
The CHERIoT encoding (to be defined in a separate section!) will instantiate
<<section_zyseal>> and <<section_zysentry>> with a 3-bit CT field with
<<x_perm>>-dependent encoding, explicit <<zyseal_se_perm>> and
<<zyseal_us_perm>> bits in a separate root, and a plethora of sentry types with
additional semantics.  All of this should be compatible with the extensions as
defined.
=====
...
When <<section_zylevels1>> _is not_ present, the following new rows are added to those of <<sec_permission_transitions>>:
...
When <<section_zylevels1>> _is_ present, the following new rows are added to those of <<sec_permission_transitions>>:
...
===== RV32 Permission Transitions

Executing <<CLRPERM>> can result in sets of permissions which cannot be represented when MXLEN=32
(see <<cap_perms_encoding32>>).
These cases are defined to return useful minimal sets of permissions.

When removing permissions with <<CLRPERM>> the following rules are included
(along with those of <<sec_permission_transitions>> and others added by extensions)
in the set iterated to fixed-point:

.<<CLRPERM>> RV32 rules
[float="center",align="center",cols="2,2,4",options="header"]
|===
| Rule            | Permission   | Valid only if
| RV32-1 | <<asr_perm>> | All other permissions are set.
| RV32-2 | <<c_perm>>   | <<r_perm>>
| RV32-3 | <<x_perm>>   | <<r_perm>>
| RV32-4 | <<w_perm>>   | not(<<c_perm>>) or <<lm_perm>>
| RV32-5 | <<x_perm>>   | <<w_perm>> or <<c_perm>>
| RV32-6 | <<lm_perm>>  | (<<w_perm>> or <<zylevels1_lg_perm>>)
|===
