[appendix]
[#app_cap_description, reftext="Default capability encoding"]
== Default Capability Encoding in {cheri_base_ext_name}

This chapter describes the default capability encoding for both {cheri_base64_ext_name} and {cheri_base32_ext_name}.
It covers the following configurations:

* <<uycfg>>.base=0, and
* <<uycfg>>.features=0 (no levels support) or 1 (<<section_zylevels1>> support)

NOTE: No levels support is specified as an option to allow flexibility for future extensions to reuse the architectural permissions allocated to <<section_zylevels1>> in {cheri_base32_ext_name}. It is not recommended to exclude <<section_zylevels1>> until a better alternative exists.

If no capability encoding format extension is specified then the selected format is always <<uycfg>>.base=0, <<uycfg>>.features=1. The explicit name for this is `Zydefaultcap`.
...
When a valid source capability is read, a new address is applied, and then the resulting capability is written to the destination register, a check is applied to the new address to ensure that the decoding of the bounds is the same for the source and destination capabilities.

This is the _representable range_ check that checks the new address is valid for use with the source capability metadata.

This check is the fundamental building block for combining addresses and capability metadata, and is used by instructions such as <<SCADDR>> and <<CADD>>/<<CADDI>>.

Given:

* A source capability with address _a_, metadata _m_ that decodes to give the bounds _b_ and _t_.
* An arbitrary address _a'_ with the same metadata _m_ decodes to give the bounds _b'_ and _t'_.

The address _a'_ is within the source capability's _representable range_ if `b == b' && t == t'`.

If the address _a'_ is outside the _representable range_, then the destination capability has the {ctag} set to zero.
