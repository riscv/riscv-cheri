...
Similarly, if {cheri_levels_ext_name} is _not_ implemented then <<el_perm>> is initialized to 1, and <<sl_perm>> is initialized to the maximum value, but the resulting <<el_perm>> and <<sl_perm>> is discarded at the end.
