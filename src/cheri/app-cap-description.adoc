...
Whenever a capability with a new different address is derived from a valid source capability, a check is applied to the new address to ensure that the decoding of the bounds is the same for the source and destination capabilities.
...
* An arbitrary address _a'_ with the same metadata _m_ that decodes to give the bounds _b'_ and _t'_.
