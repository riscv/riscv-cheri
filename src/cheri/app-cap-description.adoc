...
MXLEN without including the validity tag bit. The value of CLEN is always calculated
...
The components of a capability, except the validity tag, are encoded as shown in
...
or register able to hold a capability must also store the validity tag as out of band
...
when the validity tag is set (see xref:section_cap_malformed[xrefstyle=short]).
...
. Cause certain manipulation instructions like <<CADDI>> to always clear the validity tag of the result.
