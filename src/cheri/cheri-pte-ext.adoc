...
This extension specifies use of the {cheri_base_ext_name} field from the PTE formats (e.g., see <<sv39pte>>).
{cheri_priv_crg_ext} implies Sv39.

The {cheri_priv_crg_ext} extension is enabled when the `sstatus.CRGE` bit is set.

NOTE: The enable bit is required to redefine the usage of the _pte_._{cheri_base_ext_name_lc}_ field.
...
Additionally, the extension allows tracking of kernel space pages for guarding against use-after-reallocation within the OS kernel.

The 4-bit _pte_._{cheri_base_ext_name_lc}_  field is sub-divided into four 1-bit fields, two controlling capability loads (including the capability loaded during an AMO) and two controlling capability stores (again, including the capability stored as part of an AMO).

[[svycrg_rvy_field_breakdown]]
.Breakdown of the 4-bit _pte_._{cheri_base_ext_name_lc}_ field.
[%autowidth,float="center",align="center",cols="<,<,<",options="header"]
|===
| Bit                                | Name           | Comment
3+| *Capability load/AMO fields*
|_pte_._{cheri_base_ext_name_lc}[0]_ | _pte_._lye_    | Load capability enable
|_pte_._{cheri_base_ext_name_lc}[1]_ | _pte_._lycrg_  | Load capability read generation
3+| *Capability store/AMO fields*
|_pte_._{cheri_base_ext_name_lc}[2]_ | _pte_._sye_    | Store capability enable
|_pte_._{cheri_base_ext_name_lc}[3]_ | _pte_._yd_     | Capability dirty bit
|===

NOTE: {cheri_priv_crg_ext} is not a mandatory part of the {cheri_base64_ext_name} Privileged ISA, as a future extension may specify an alternative usage for the _pte_._{cheri_base_ext_name_lc}_ field allowing a different mechanism giving improved software efficiency.
...
_pte_._lye_ is used to enable capability loads or AMOs to write a {ctag} to {cd}.

When _pte_._lye_ is set, _pte_._lycrg_ can be used to trap on capability loads or AMOs when it does not match the Capability Read Generation (CRG) value which is represented by the value of `sstatus.UCRG` for userspace pages (_pte_._u=1_) or `sstatus.SCRG` for kernel pages (_pte_._u=0_).
...
. _pte_._lye_ is set.
. if _pte_._u=1_ _pte_._lycrg_ does not equal `sstatus.UCRG`.
. if _pte_._u=0_ _pte_._lycrg_ does not equal `sstatus.SCRG`.
...
As a result, the software is required to be tolerant of raising the trap when the {ctag} is not set, potentially resulting in spurious traps from pages which have _pte_._lye=1_, and so are likely to store valid capabilities.
...
[[pte_lye_lycrg_load_summary]]
.Summary of capability load _pte_._lye_ and _pte_._lycrg_ behavior in the PTEs
[%autowidth,float="center",align="center",cols="<,<,<,<",options="header"]
|===
|_pte_._lye_| _pte_._lycrg_          | tag^1^| Load Capability Behavior
| 0         | 0                      |   X   | Clear loaded {ctag}
| 0         | 1                      |   X   | Normal operation
| 1         | {ne} `sstatus.xCRG`^2^ |   0   | Implementation defined choice of +
{cheri_excep_name_pte_ld} or normal operation
| 1         | {ne} `sstatus.xCRG`^2^ |   1   | {cheri_excep_name_pte_ld}
| 1         | = `sstatus.xCRG`^2^    |   X   | Normal operation
|===

^1^ The loaded {ctag}.

^2^ if _pte_._u=1_ `sstatus.UCRG` else `sstatus.SCRG`.
...
When _pte_._sye_ is clear, capability stores or AMOs where the to-be-stored {ctag} is set will raise a {cheri_excep_name_pte_st} fault.

When _pte_._sye_ is set, capability stores to the virtual page are permitted.
In addition, the _pte_._yd_ bit indicates that a capability was stored to the virtual page since the last time the _pte_._yd_ bit was cleared.
...
* For _Svadu_, do a hardware update which sets _pte_._yd=1_, following the same rules as setting _pte_._d_.
** When setting _pte_._yd_, the hardware update also necessarily sets (or leaves set) _pte_._a_ and _pte_._d_.


[[pte_sye_yd_store_summary]]
.Summary of capability store _pte_._sye_ and _pte_._yd_ behavior in the PTEs
[%autowidth,float="center",align="center",cols="<,<,<,<",options="header"]
|===
|_pte_._sye_|_pte_._yd_|tag^1^| Store Capability Behavior
| 0        | 0      |  0   | Normal operation
| 0        | 1      |  X   | {cheri_excep_name_pte_st}
| 1        | 0      |  0   | Normal operation
| 1        | 0      |  1   | {cheri_excep_name_pte_st} (_Svade_) or +
hardware _pte_._yd_ update (_Svadu_)
...
Capability dirty tracking _must_ be triggered when _pte_._sye=1_ and _pte_._yd=0_ and the to-be-stored {ctag} is set, and _may_ be triggered in the following case where it is not known if there will be a stored {ctag} during translation:
...
_pte_._yd_ must always be set when there is a stored {ctag} to a virtual page so that software knows which pages have had capabilities stored to them.
...
In this case the _pte_._yd_ bit will still be set.
...
The minimum level of PTE support is to set _pte_._lye=1_ and _pte_._sye=1_ in all PTEs intended for storing
capabilities (e.g., private anonymous mappings) and leave
`sstatus.UCRG`, `sstatus.SCRG` on all harts and _pte_._lycrg_ in all PTEs set to 0, which will enable
...
The `mstatus`, `sstatus` and `vsstatus` CSRs include the Userspace Capability Read Generation (UCRG) bit, the Supervisor version (SCRG) and the enable bit (CRGE).
