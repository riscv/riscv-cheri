...
* the authorizing capability (rs2) grants <<c_perm>>,
...
|===
|PTE. CRW| PTE. CD| PTE.CRG                               |rs2.C  | PTE.U | Load/AMO
| 0      | 0      | 0                                     | X     | X     | Clear loaded tag
| 0      | 0      | 1                                     | X     | X     | Reserved
| 0      | 1      | X                                     | X     | X     | Reserved
| 1      | X      | {ne} <<sstatusreg_pte,sstatus>>.UCRG  | 1     | 1     | {cheri_excep_name_pte_ld}, or {cheri_excep_name_pte_ld} if tag is set for _{cheri_priv_crg_load_tag_ext}_^1^
| 1      | X      | X                                     | 0     | 1     | Normal operation
| 1      | X      | = <<sstatusreg_pte,sstatus>>.UCRG     | X     | 1     | Normal operation
| 1      | X      | X                                     | X     | 0     | Normal operation^2^
|===

^1^ The choice here is whether to take data dependent exceptions on capability load data
