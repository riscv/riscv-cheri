...
The minimum level of PTE support is to set CW and CD to 1 in all PTEs intended for storing
...
If the CW bit is clear, the "no capability state", then the existing rules from <<section_priv_cheri_vmem>> are followed:
...
When the CW bit is set, the "capability state", then the behaviour of
capability loads is controlled by CRG and the behaviour of capability stores
is controlled by CD.
...
** the virtual page's CRG bit does not equal <<sstatusreg_pte,sstatus>>.UCRG, and
** PTE.U is set, and
** the capability read from memory has its {ctag} set.

The second option is enabled by the _{cheri_priv_crg_load_tag_ext}_ extension.

When CW is set, the CD bit indicates that a capability was stored to the
...
* The same behavior as when CW is clear, allowing software interpretation
 of this state.
* The implementation sets the CD bit. The PTE update must be
 ...
 hart must not perform explicit memory accesses before the PTE update is globally
 visible.

NOTE: _{cheri_priv_crg_ext}_ requires that when CW, CD and CRG are all clear,
the implementation clears loaded tags and raises {cheri_excep_name_pte_st} when
the stored {ctag} is set. Other bit combinations when CW=0 are reserved
...
[[pte_cw_cd_store_summary]]
.Summary of Store CW and CD behavior in the PTEs
[%autowidth,float="center",align="center",cols="<,<,<",options="header"]
|===
|PTE.CW |PTE.CD|Store/AMO
| 0     | X    | {cheri_excep_name_pte_st} if stored tag is set
| 1     | 0    | {cheri_excep_name_pte_st} if stored tag is set, or <<section_hardware_pte_updates, hardware CD update>>
...
The decision about whether to take exceptions on capability stores with the {ctag} set to a page with PTE.CW=1 and PTE.CD=0 is determined by whether the _Svade_ and _Svadu_ extensions are enabled.
