...
NOTE: When {cheri_priv_crg_ext} is enabled (<<sstatusreg_pte,sstatus>>.CRGE is set), the semantics of PTE.CRW bit is redefined relative to the definition in <<section_priv_cheri_vmem>>.
...
Alternatively, the CRGE bit could be made exclusively accessible from M-mode (e.g. in <<menvcfg>>); the S-mode software could use an extension of the SBI interface to opt in to the revocation extensions.
...
The CRGE bit also has the benefit or maintaining matching polarity of the PTE.CD bit with respect to the data Dirty bit (PTE.D), and permit alternative uses by future extensions of the PTE.CD and PTE.CRG reserved bits (e.g. by an alternative revocation extension that re-defines the semantics of CD and CRG).
