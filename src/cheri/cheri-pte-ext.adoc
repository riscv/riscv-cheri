...
The principle is that future extensions, may redefine the usage of the {cheri_base_ext_name} field by adding an enable bit into `Xstatus`.
====

The _pte_._{cheri_base_ext_name_lc}_ field adds the ability for supervisor-mode software to quickly enable trapping loads of capabilities from all pages of memory, incrementally allow loads of capabilities from such pages, and track stores of capabilities to all pages of memory.
...
The 4-bit _pte_._{rv64y_pte4_field_name_lc}_  field is subdivided into four 1-bit fields, two controlling capability loads (including the capability loaded during an AMO) and two controlling capability stores (again, including the capability stored as part of an AMO).
...
When both of the following are true, the _pte_._{rv64y_pte4_field_name_lc}_  field adds two related architectural features, {cheri_excep_name_pte_ld}s and Capability Dirty Tracking:
...
When _pte_._{pte_cap_r_name_lc}=1_, capability loads may raise a {cheri_excep_name_pte_ld}.

In the case of AMOs that could trigger both a {cheri_excep_name_pte_st}, due to storing a valid {ctag}, and a {cheri_excep_name_pte_ld}, the {cheri_excep_name_pte_st} takes priority as shown in <<exception-priority-cheri>>
...
When clear {ctag}s cannot be written to {cd}, and when set, they can.
...
* For _Svadu_, do a hardware update that sets _pte_._{pte_cap_dirty_name_lc}=1_, following the same rules as setting _pte_._d_.
...
_pte_._{pte_cap_dirty_name_lc}_ must always be set when a capability with a valid {ctag} is written to a virtual page so that software knows which pages have had capabilities stored to them.
...
In this case the _pte_._{pte_cap_dirty_name_lc}_ bit would still be set.
...
The `mstatus`, `sstatus` and `vsstatus` CSRs include the Userspace Capability Read Generation (U{pte_cap_crg_pte_name}) bit and the Supervisor version (S{pte_cap_crg_pte_name}).
