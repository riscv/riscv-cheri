...
NOTE: Checking the value of the {ctag} requires taking data dependent exceptions on loaded data for loads or AMOs.
 ...
 The same software is tolerant of raising the trap when the {ctag} is not set, but checking the {ctag} is expected to result in fewer spurious traps.
