...
NOTE: Diagram included in the standalone specification for reference only.
...
The extension adds the ability for S-mode software to quickly enable trapping loads of capabilities from all pages of memory, incrementally allow loads of capabilities from such pages, and track stores of capabilities to all pages of memory.
...
. The loaded capability is valid, unless the implementation traps conservatively^1^.
...
When _pte_._{pte_cap_w_name_lc}_ is set to zero, capability stores or AMOs where the to-be-stored capability is valid will raise a {cheri_excep_name_pte_st}.
...
. The to-be-stored capability is valid.
...
Capability dirty tracking _must_ be triggered when _pte_._{pte_cap_w_name_lc}_=1 and _pte_._{pte_cap_dirty_name_lc}_=0 and the to-be-stored capability is valid, and _may_ be triggered in the following case where it is not known if there will be a stored {ctag} during translation:

* SC{LD_ST_DOT_CAP} triggers capability dirty tracking if `{cs2}` is a valid capability, even if the store fails.
...
* For future AMOCAS{LD_ST_DOT_CAP}, it is not known whether the store will happen until the load has executed, and the compare has been done.
  Therefore, AMOCAS{LD_ST_DOT_CAP} is likely to trigger capability dirty tracking if `{cs2}` is a valid capability.
