...
This extension specifies use of the {rv64y_pte4_field_name_lc} field from the PTE formats
...
The {cheri_priv_crg_ext} extension is enabled when the `sstatus.{revocation_enable}` bit is set.
...
|_pte_._{rv64y_pte4_field_name_lc}[0]_ | _pte_._{pte_cap_r_name_lc}_         | Capability read enable
...
|_pte_._{rv64y_pte4_field_name_lc}[2]_ | _pte_._{pte_cap_w_name_lc}_         | Capability write enable
|_pte_._{rv64y_pte4_field_name_lc}[3]_ | _pte_._{pte_cap_dirty_name_lc}_     | Capability dirty bit
|===

NOTE: {cheri_priv_crg_ext} is not a mandatory part of the {cheri_base64_ext_name} Privileged ISA, as a future extension may specify an alternative usage for the _pte_._{rv64y_pte4_field_name_lc}_ field allowing a different mechanism giving improved software efficiency.

=== Software Primitives

When all of the following are true, {cheri_priv_crg_ext} adds two primitives for use by software:

. `sstatus.{revocation_enable}` is set.
...
. if _pte_._u=1_, _pte_._{pte_cap_crg_pte_name_lc}_ does not equal `sstatus.{revocation_u}`.
. if _pte_._u=0_, _pte_._{pte_cap_crg_pte_name_lc}_ does not equal `sstatus.{revocation_s}`.
...
^1^ Allowing flexibility about checking the loaded {ctag} gives a range of valid implementations.
...
| 1                         | {ne} `sstatus.{revocation_x}`^2^ |   0   | Implementation defined choice of +
{cheri_excep_name_pte_ld} or normal operation
| 1                         | {ne} `sstatus.{revocation_x}`^2^ |   1   | {cheri_excep_name_pte_ld}
| 1                         | = `sstatus.{revocation_x}`^2^    |   X   | Normal operation
|===

^1^ The loaded {ctag}.

^2^ if _pte_._u=1_, `sstatus.{revocation_u}` else `sstatus.{revocation_s}`.
...
The minimum level of PTE support is to set _pte_._{pte_cap_r_name_lc}_=1, _pte_._{pte_cap_dirty_name_lc}_=1, _pte_._{pte_cap_w_name_lc}_=1 and _pte_._{pte_cap_crg_pte_name_lc}_=0 in all PTEs intended for storing capabilities (e.g., private anonymous mappings) and set `sstatus.{revocation_u}`=0 and `sstatus.{revocation_s}`=0 on all harts, which will enable capabilities to be loaded and stored successfully.

ifdef::cheri_standalone_spec[]

[#xstatus_pte]
=== {revocation_u} CSR field

The `mstatus`, `sstatus` and `vsstatus` CSRs include the Userspace Capability Read Generation ({revocation_u}) bit, the Supervisor version ({revocation_s}) and the enable bit ({revocation_enable}).
...
{reg: [
  ...
  {bits:  1, name: 'MDT'},
  {bits: 16, name: 'WPRI'},
  {bits:  1, name: '{revocation_enable}'},
  {bits:  1, name: '{revocation_s}'},
  {bits:  1, name: '{revocation_u}'},
  {bits:  1, name: 'WPRI'},
  {bits:  1, name: 'SD'},
