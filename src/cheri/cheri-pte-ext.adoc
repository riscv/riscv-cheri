...
* A {cheri_excep_name_pte_ld} exception is raised when a capability load or AMO instruction is executed
...
^1^ The choice here is whether to take data dependent exceptions on load data for loads or atomic operations.
 The default is to take the trap without checking the value of the loaded valid tag.
 Taking a trap when the valid tag is not set will introduce additional traps during revocation sweeps.
 If _{cheri_priv_crg_load_tag_ext}_ is implemented then the trap is only taken if the loaded valid tag is set, to reduce software overhead from revocation sweeps.
 Checking the loaded tag affects the exception priority, see <<exception-priority>>.

NOTE: _{cheri_priv_crg_load_tag_ext}_ is an optimization for software, and as such implementations are allowed to conservatively fault under certain conditions even if the valid tag is not set.
