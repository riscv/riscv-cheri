[#section_sv_cheri]
[#cheri_pte_ext]
== "{cheri_pte_ext_name}" Extension, Version 1.0
...
The {cheri_pte_ext_name} extension adds the ability to perform capability revocation of user mode pages (see <<cap_revocation>>) by adding the PTE.CRG bit, and <<sstatusreg_pte,sstatus>>.UCRG as described below.

NOTE: {cheri_pte_ext_name} depends on {cheri_priv_vmem_ext} also being supported.
