...
The extension adds the ability for supervisor-mode software to quickly enable trapping loads of capabilities from all pages of memory, incrementally allow loads of capabilities from such pages, and track stores of capabilities to all pages of memory.egalize
...
NOTE: It may make sense to subdivide this into two extensions as _Capability Dirty Tracking_ is more generally useful than {cheri_excep_name_pte_ld}s.
...
CSRs include the Userspace Capability Read Generation (U{pte_cap_crg_pte_name}) bit, the Supervisor version (S{pte_cap_crg_pte_name}) and the enable bit ({pte_cap_fc_enable}).
