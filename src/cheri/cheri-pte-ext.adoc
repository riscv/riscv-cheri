...
When enabled, the extension adds the ability for supervisor-mode software to quickly front-run loads of capabilities from userspace pages.
Such a facility has been shown to be a useful primitive for system-level implementation of certain forms of capability revocation which, in turn, allow userspace heap allocator software to deterministically guard against use-after-reallocation cite:[cornucopia-reloaded].

It achieves this by adding by adding the _pte.crg_, _pte.cd_ and `sstatus.UCRG` bits as described below`.
When the extension is disabled the _pte.cd_, _pte.crg_ and `sstatus.UCRG` bits remain reserved.
...
When _pte.crw_ and `sstatus.CRGE` are both set then {cheri_priv_crg_ext} adds two primitives for use by software which are enabled when all of the following are true.
...
NOTE: These primitives may be used to implement the load-barrier and store-barrier primitives from cite:[cornucopia-reloaded].
...
When _pte.crw_ is set, the local _pte.crg_ bit can be used to trap on capability loads or AMOs when it does not match the value of `sstatus.UCRG`.
...
. _pte.crg_ bit does not equal `sstatus.UCRG`.
. _pte.u_ is set.
...
NOTE: Checking the value of the {ctag} requires taking data dependent exceptions on load data for loads or AMOs, and also affects the exception priority, see xref:exception-priority[xrefstyle=short].
...
If the _{cheri_priv_crg_load_tag_ext}_ extension is implemented, implementations are allowed to conservatively fault under certain conditions even if the {ctag} is not set.
...
. The {ctag} bit of the capability being stored to memory is set.
. The _pte.cd_ bit is clear.
...
** This allows software interpretation of this state.
* For _Svadu_, do a hardware update which sets _pte.cd=1_, following the same rules as setting _pte.a_/_pte.d_.
** When setting _pte.cd_, the hardware update also sets _pte.a_ and _pte.d_.
** _pte.cd_ cannot be set speculatively, following the same rule as _pte.d_.
...
NOTE: Capability dirty tracking _always_ checks the {ctag} on stored capabilities, it is not optional as the value of the {ctag} is always available to the microarchitecture.
...
`sstatus.UCRG` and _pte.crg_ in all PTEs set to 0, which will allow
capabilities to be loaded and stored successfully.

[#xstatus_pte]
=== UCRG, CRGE CSR fields

The `mstatus`, `sstatus` and `vsstatus` CSRs are extended to include the new Capability Read Generation (CRG) and Capability Read Generation Enable (CRGE) bits.
