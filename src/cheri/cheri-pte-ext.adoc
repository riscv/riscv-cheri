...
The {cheri_priv_crg_ext} extension adds the ability to perform capability revocation of user mode pages (see <<cap_revocation>>)
by extending the PTE.CRW field to 3 bits and adding an <<sstatusreg_pte,sstatus>>.UCRG bit as described below.
...
and a PTE.CRW bit pattern to disable load-side generation faults for a page.

The minimum level of PTE support is to set CRW field to `0b100` in all PTEs intended for storing
capabilities (i.e., private anonymous mappings) and leave <<sstatusreg_pte,sstatus>>.UCRG set to 0,
which will allow capabilities with their tags set to be loaded and stored successfully.
...
Leaf PTEs have the Capability Read-Write (CRW) field extended to 3 bits
in Sv39, Sv48 and Sv57 as shown in xref:sv39pte_crg[xrefstyle=short],
xref:sv48pte_crg[xrefstyle=short] and xref:sv57pte_crg[xrefstyle=short] respectively.
For non-leaf PTEs these bits remain reserved and must be cleared by software for
forward compatibility, or else a page-fault exception is raised. Additionally,
if the hypervisor extension is enabled these bits remain reserved for leaf and
non-leaf PTEs used in guest address translation.
...
The CRW field encodes the PTE behavior for capability load and store operations
to the page. The extended CRW field encodes the Capability Dirty (CD) and the
Capability Read Generation (CRG) state for the page.
The logical CD bit indicates that a capability was stored to the
virtual page since the last time the CD bit was cleared.
The logical CRG bit indicates the current generation of the virtual
memory page with regards to the ongoing capability revocation cycle.

The CRW field encodes logical states that determine the behaviour for capability
load and store operations to the page.

[[pte_crw_encoding]]
.Summary of CRW configurations in the PTEs
[%autowidth,float="center",align="center",cols="<,<,<",options="header"]
|===
|PTE.CRW  | PTE.U=1 CHERI load/store semantic | PTE.U=0 CHERI load/store semantic
| `0b000` | No capability state               | No capability state
| `0b001` | Reserved                          | Reserved
| `0b010` | Reserved                          | Reserved
| `0b011` | Reserved                          | Reserved
| `0b100` | Capability state, CD=1, CRG=0     | Normal operation^2^
| `0b101` | Capability state, CD=1, CRG=1     | Reserved
| `0b110` | Capability state, CD=0, CRG=0     | Reserved
| `0b111` | Capability state, CD=0, CRG=1     | Reserved
|===
...
The CRW bit (defined by {cheri_priv_vmem_ext}) indicates whether reading or
writing capabilities with the {ctag} set to the virtual page is permitted.
Software unaware of the {cheri_priv_crg_ext} can operate without modification
by leaving `PTE.CRW[1:0]=0` and using `PTE.CRW[2]` with the same semantic as
the single-bit CRW as defined by {cheri_priv_vmem_ext}.

NOTE: The {ctag} bit of the stored capability is checked _after_ it is potentially
cleared due to lack of <<c_perm>>.

If the CRW field is in the "no capability state" (`0b000`), then the existing rules
from <<section_priv_cheri_vmem>> are followed:
...
When the CRW field is in one of the "capability states", the behaviour of
capability loads is controlled by CRG and the behavior of capability stores
is controlled by CD.

When a PTE is in the "capability state", the CRG bit indicates the current
generation of the virtual memory page with regards to the ongoing capability
revocation cycle.
...
When a PTE is in the "capability state", the CD bit indicates that a capability
was stored to the virtual page since the last time the CD bit was cleared.
...
* The same behavior as the "no capability state", allowing software interpretation
 of this state.
...
When CRW is in the "no capability state" (`0b000`), the implementation is required
to clear loaded tags and raise {cheri_excep_name_pte_st} when the stored {ctag} is set.

NOTE: The value of CD and CRG when CRW is in a reserved state are undefined and may be
defined by future extensions.
The reserved PTE states behave as the "no capability state", unless a future extension
defines an alternative function.

[[pte_crw_crg_load_summary]]
.Summary of Load CRW and CRG behavior in the PTEs (PTE.U=1)
[%autowidth,float="center",align="center",cols="<,<,<,<",options="header"]
|===
|PTE.CRW           | CD     | CRG                                     | Load/AMO
| `0b000`          | -      | -                                       | Clear loaded tag
| `0b001`          | -      | -                                       | Reserved
| `0b010`          | -      | -                                       | Reserved
| `0b011`          | -      | -                                       | Reserved
| `0b100`, `0b110` | X      | &#8800; <<sstatusreg_pte,sstatus>>.UCRG | {cheri_excep_name_pte_ld}, or {cheri_excep_name_pte_ld} if tag is set for _{cheri_priv_crg_load_tag_ext}_^1^
| `0b101`, `0b111` | X      | = <<sstatusreg_pte,sstatus>>.UCRG       | Normal operation
|===
...
.Summary of Store CRW and CD behavior in the PTEs (PTE.U=1)
[%autowidth,float="center",align="center",cols="<,<,<,<",options="header"]
|===
|PTE.CRW           | CD     | CRG | Store/AMO
| `0b000`          | -      | -   | {cheri_excep_name_pte_st} if stored tag is set
| `0b001`          | -      | -   | Reserved
| `0b010`          | -      | -   | Reserved
| `0b011`          | -      | -   | Reserved
| `0b110`, `0b111` | 0      | X   | {cheri_excep_name_pte_st} if stored tag is set, or <<section_hardware_pte_updates, hardware CD update>>
| `0b100`, `0b101` | 1      | X   | Normal operation
|===

[#section_hardware_pte_updates]
=== Enabling Software or Hardware PTE updates

The decision about whether to take exceptions on capability stores with the {ctag} set to a page with CD=0 is determined by whether the _Svade_ and _Svadu_ extensions are enabled.
...
* If only _Svadu_ is implemented, or enabled through henvcfg.ADUE or menvcfg.ADUE, then do the hardware update of setting CD=1 as described in <<section_extending_pte>>.

NOTE: The CD update must retain the same CRG state. For example, PTE.CRW is updated from `0b110` to `0b100` to enter the Capability Dirty state when CRG=0.
