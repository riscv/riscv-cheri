...
* A {cheri_excep_name_pte_ld} is raised when a capability load or AMO instruction is executed
 wherein:
** the authorizing capability grants <<c_perm>>,
** the virtual page's CRG bit does not equal <<sstatusreg_pte,sstatus>>.UCRG,
** the virtual page's PTE.U is set, and
...
When CW, CD and CRG are all clear, the implementation is required to clear
loaded tags and raise {cheri_excp_name_pte_st} when the stored {ctag} is set.

NOTE: Other CD and CRG combinations when CW=0 are reserved for future extension.
The reserved PTE states behave as the CW=0 CD=0 CRG=0, unless a future extension
...
|===
|PTE.CW | PTE.CD | PTE.CRG                                 | PTE.U | Load/AMO
...
[%autowidth,float="center",align="center",cols="<,<,<,<",options="header"]
|===
|PTE.CW | PTE.CD | PTE.CRG | Store/AMO
