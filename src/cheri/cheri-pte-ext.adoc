...
The minimum level of PTE support is to set _pte_._{pte_cap_r_name_lc}_=1 and _pte_._{pte_cap_w_name_lc}_=1 in all PTEs intended for storing
capabilities (e.g., private anonymous mappings) and leave
`sstatus.{revocation_u}`, `sstatus.{revocation_s}` on all harts and _pte_._{pte_cap_crg_pte_name_lc}_ in all PTEs set to 0, which will enable
