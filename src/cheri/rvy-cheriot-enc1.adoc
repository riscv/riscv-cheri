...
| Field      | Description
| R          | Reserved bit
| G          | Global bit, as in <<zylevels1_gl_flag,GL>>.
...
Its <<zylevels1_gl_flag,GL>> flag is encoded in the G bit,
...
* Quadrant 0 is used to encode permissions that authorize sealing (see <<section_zyseal>>) and also the single <<SDP-field,software-defined permission>>, <<zycheriot_u0_perm,U0>>. It can also encode 'no permissions'.
...
** Octant 4 encodes combinations of <<r_perm,R>> and <<w_perm,W>> without <<c_perm,C>> with the redundant not-<<r_perm,R>> and not-<<w_perm,W>> used to encode write-only with <<c_perm,C>>.
...
.Decoding of the XLEN-bit base (_b_) bound
...
<<section_zylevels1>> requires root capabilities to be <<zylevels1_gl_flag,Global>>,
