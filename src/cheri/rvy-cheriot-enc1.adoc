...
It is specifically designed to be a suitable substrate for {cheri_base32_ext_name}{cheriot_unpriv_ext_name} systems.
...
Similarly to {rv32y_uni_base_name}, capability permisisons are encoded in a compressed form.
...
Quadrant 0 is used to encode permissions that authorise sealing (see <<section_zyseal>>) and also the single <<sec_cap_sdp,software-defined permission>>, `U0`. It can also encode 'no permissions'.
...
Quadrant 2 is subdivided into octants, with octant 4 encoding combinations of <<r_perm,R>> and <<w_perm,W>> without <<c_perm,C>>
(with the redundant not-<<r_perm,R>> and not-<<w_perm,W>> used to encode write-only with <<c_perm,C>>).
Octant 5 encodes read-only capabilities with <<c_perm,C>> and the dependent permissions <<lm_perm,LM>> and <<zylevels1_lg_perm,LG>>.
...
| P[4:3] | P[2] | P[1] | P[0]
  ...
  |<<x_perm,X>>|<<asr_perm,ASR>>|   `U0`     | <<zyseal_se_perm,SE>>  |<<zyseal_us_perm,US>>| Notes
| 00     | `U0` |<<zyseal_se_perm,SE>>|<<zyseal_us_perm,US>>
  |            |                |            |                        |                     |
  |            |                | P[2]       | P[1]                   | P[0]                | Sealing
...
6. Encode `U0`, <<zyseal_se_perm,SE>> and <<zyseal_us_perm,US>> using the sealing format (P[4:3] = 00).
...
[#tab_cheriot_enc1_root_caps,width="100%",options=header,cols="1,1,2",align="center"]
|===
