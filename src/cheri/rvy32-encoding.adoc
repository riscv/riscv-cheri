...
[#AP-field-encoding32, reftext="AP-field encoding"]
==== Architectural Permissions (AP) Encoding

The permissions field is {cap_rv32_perms_width} bits wide and is encoded using one a compressed representation as shown below.
...
Therefore, it is only legal to encode a subset of all combinations, and this redundancy is used to reduce the size of the permissions field compared to {rvy32_uni_base_name}.
...
[#M-field-encoding32, reftext="M-field encoding"]
...
Capabilities in this encoding have a 1-bit field for <<sec_cap_type>> values which behaves in the same way as the {rvy64_uni_base_name} <<section_cap_sealed>>.
...
Compared to {rvy64_uni_base_name}, this encoding uses an additional `L8` bit as described in xref:section_cap_bounds[xrefstyle=short].

===== <<AUIPC_CHERI>> Shift

For the capabilities of this encoding, the <<section_auipc_shift>> value is 12.
The bounds encoding has sufficient <<section_rep_check_concept,representability>> that no <<AUIPC_CHERI>> instruction used to reach an in-bounds offset
will clear the {ctag}.
...
The representable range check behaves in exactly the same way as in {rvy64_uni_base_name}, just with the {cap_rv32_mw_width}-bit mantissa width of this encoding.
