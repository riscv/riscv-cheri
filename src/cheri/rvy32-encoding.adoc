...
== The {rv32y_uni_base_name} Capability Base Extension for {cheri_base32_ext_name}, Version 1.0

This chapter describes an in-memory format and properties of a capability encoding intended for {cheri_base32_ext_name}.
This format is heavily based upon the {cheri_base64_ext_name} format <<rv64y_cap_description,{rv64y_uni_base_name}>> with the following changes:

* The parameters used to set the width, or presence, of fields in the encoding are defined in <<rvy32_uni_base_name_param_summary>>.
...
| <<section_zyseal>>           | Will be compatible if new permissions are encoded
endif::[]
| All RVY versions of other standard extensions | Compatible if the extension is compatible with RV32I
|==============================================================================

.{rv32y_uni_base_name} Feature summary
...
[#AP-field-encoding32, reftext="M-bit encoding in the AP field"]
...
Therefore, it is only legal to encode a subset of all combinations, and this redundancy is used to reduce the size of the permissions field compared to <<rv64y_cap_description,{rv64y_uni_base_name}>>.
...
==== AP encoding and rules without <<section_zylevels1>> for {rv32y_uni_base_name}

.Encoding of architectural permissions for {rv32y_uni_base_name} without <<section_zylevels1>>
...
NOTE: Mode is encoded with permissions for {rv32y_uni_base_name}, but is not a permission. It is
...
.{rv32y_uni_base_name} <<CLRPERM>> rules if <<section_zylevels1>> is absent.
...
==== AP encoding and rules with <<section_zylevels1>> for {rv32y_uni_base_name}

.Encoding of architectural permissions for {rv32y_uni_base_name} with <<section_zylevels1>>
...
.{rv32y_uni_base_name} <<CLRPERM>> rules if <<section_zylevels1>> is present.
[float="center",align="center",cols="2,2,4",options="header"]
|===
| <<CLRPERM>> Rule | Permission            | Valid only if
...
Capabilities in this encoding have a 1-bit field for <<sec_cap_type>> values which behaves in the same way as the <<rv64y_cap_description,{rv64y_uni_base_name}>> <<section_cap_sealed>>.

[#section_cap_bounds32]
==== Bounds (EF, T, TE, B, BE, L~8~) Encoding

The bounds are encoding in the same way as in <<section_cap_bounds,{rv64y_uni_base_name}>>, with the appropriate values for mantissa width and maximum exponent substituted.
Compared to <<rv64y_cap_description,{rv64y_uni_base_name}>>, this encoding uses an additional `L~8~` bit as described in xref:section_cap_bounds[xrefstyle=short].
...
The representable range check behaves in exactly the same way as in <<section_cap_representable_check,{rv64y_uni_base_name}>>, just with the {cap_rv32_mw_width}-bit mantissa width of this encoding.
