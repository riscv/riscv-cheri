...
This format is heavily based upon the <<rv64y_cap_description,{rv64y_uni_base_name}>> format with the following changes:
...
<<section_zylevels1>>::
If <<section_zylevels1>> is available, additional values of the `AP, P` field are allocated, otherwise they are reserved for any valid capability.
...
[#AP-field-encoding32, reftext="P-bit encoding in the AP field"]
==== Architectural Permissions and Mode (AP, P) Encoding
...
==== AP encoding and rules without {cheri_levels1_ext_name} for {rv32y_uni_base_name}

.Encoding of architectural permissions for {rv32y_uni_base_name} without {cheri_levels1_ext_name}
...
.{rv32y_uni_base_name} <<CLRPERM>> rules if {cheri_levels1_ext_name} is absent.
...
==== AP encoding and rules with {cheri_levels1_ext_name} for {rv32y_uni_base_name}
...
.{rv32y_uni_base_name} <<CLRPERM>> rules if {cheri_levels1_ext_name} is present.
[float="center",align="center",cols="2,2,4",options="header"]
|===
| <<CLRPERM>> Rule | Permission            | Valid only if
...
The bounds are encoded in the same way as in <<section_cap_bounds,{rv64y_uni_base_name}>>, with the appropriate values for mantissa width and maximum exponent substituted.
