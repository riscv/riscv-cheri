...
However, the precise in-memory representation of capabilities is not defined in this chapter since multiple legal (and generally software-compatible) encodings exist.
...
NOTE: {cheri_base_ext_name} si designed to be highly extensible allowing multiple encoding formats to be created.
 Multiple formats are most likely for {cheri_base32_ext_name} due to the limited number of bits available in the metadata.
...
[IMPORTANT]
====
For XLEN=32 there are very limited numbers of bits available in the capability metadata but depending on the target domain different choices must be made.
For example, a small embedded controller with limited address space of a few megabytes may need to make different tradeoffs than an auxiliary core within a larger SoC with many gigabytes of address space, and a accelerator using the {cheri_base_ext_name} ISA may need yet another set of tradeoffs.
