...
. The capability metadata does not have any bits or fields set to a value reserved by the implemented extensions.
...
Integrity checks are therefore _optional_ by default: performing an integrity check on a valid capability never alters execution on a correctly functioning system.
...
(integrity failure) |1 |No |Metadata has a reserved value or <<section_cap_malformed,malformed>> bounds, which is detected by optional integrity checks.
|Corrupted valid capability +
