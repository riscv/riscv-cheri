...
_Root_ capabilities are the capability encodings for the maximum set of bounds, covering the whole 2^XLEN^ address space, and that grant all encodable permissions.

Some capability-encoding formats like {rv64y_uni_base_name} provide the encoding for a single <<section_infinite_cap_encoding,Infinite>> capability which provides all the characteristics of root capabilities.

Other capability-encoding formats may have a _set_ of root capabilities distinguishing between at least data and executable capabilities.

NOTE: Future extensions may add additional root capabilties for purposes such as sealing and unsealing.
To accommodate this <<root-rx-cap>> and <<root-rw-cap>> are defined below.
...
Upon initialization, the execution environment defines how capabilities are made available to software.

NOTE: Implementations are not required to provide access to the entire space to a single hart.
The provisioned capabilities may be restricted to subset of the entire address space, or even to remove certain permissions.
Provisioned capabilities may be the maximum that the hart can ever have access to, or may allow access to capabilities in memory with greater bounds or permissions.
