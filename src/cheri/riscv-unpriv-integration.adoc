[[rv64y]]
== {cheri_base64_ext_name} Base Capability Instruction Set, Version 1.0
...
The {cheri_base64_ext_name} base can be composed with other standard options to bases such as E (16-registers), Zfinx and endianness.
...
{cheri_base64_ext_name} specifies the minimum required fields that the capability format must support, and their semantics.
...
The name {cheri_base64_ext_name} refers generically to CHERI base ISAs, which share common semantics but may have different metadata encodings.
...
For example, the CHERIoT capabilty encoding formats use  `e14` since it uses a four-bit exponent field that can encode only the values 0 through 14 and 24 (which, with its 9-bit mantissas, is sufficient to provide an encoding of bounds that cover the whole address space).
...
The currently defined {cheri_base64_ext_name} base capability encoding format is:
...
<<rv64y>> introduced the CHERI base ISA and the {rv64y_uni_base_name} encoding format, as the only defined {cheri_base64_ext_name} format.

{cheri_base32_ext_name} inherits all details from {cheri_base64_ext_name}, and, sets XLEN=32, YLEN=64.

{cheri_base32_ext_name} has multiple encoding formats however, due to the limit metadata space.

{cheri_base32_ext_name} has insufficient bits available to enable features for all target domains, and so multiple capability encoding formats are available.
...
Further {cheri_base32_ext_name}-extending capability formats exist, which currently are the CHERIoT formats.
