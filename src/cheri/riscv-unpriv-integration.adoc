...
==== Zca HINT and _reserved_ Instructions

For {cheri_base_ext_name} _all_ arithmetic HINT instructions require _rd_=`x0`.

This is because the metadata and {ctag} are set to zero by arithmetic instuctions which do not affect the integer data value.

As a result, many encodings in Zca declared as HINTs in RVI/RVE are _reserved_ for {cheri_base_ext_name}.

Specifically the following encodings are _reserved_:

* C.ADDI with __rd__{ne}x0 and __imm__=0.
* C.SLLI with __rd__{ne}x0 and __shamt__=0.
* C.SRLI with __shamt__=0.
* C.SRAI with __shamt__=0.

NOTE: 32-bit arithmetic HINT instructions are unaffected by this rule as all have _rd_=`x0`.
