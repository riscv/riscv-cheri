...
The {ctag} is the only architectural state that distinguishes a capability from data, so every YLEN-bit value is either a _valid_ or an _invalid_ capability.
xref:table_cap_states[xrefstyle=short] also names the corrupted states a valid capability can be in after a memory error, physical glitch, or interaction with incompatible IP.

.Summary of capability states
[#table_cap_states,width="100%",options=header,cols="3,^1,^1,5"]
|=======================
