...
== "{rvy_sentry_ext_name}" Extension for Creation of Sentry Capabilities

In the {cheri_base_ext_name} base ISA the only way to create a <<sentry_cap>> is through the <<JALR_CHERI>> instruction as that writes back a sentry capability to the link register.
The {rvy_sentry_ext_name} extension adds the <<SENTRY>> instruction to allow sealing arbitrary capabilities as a <<sentry_cap>> (<<sec_cap_type>>=1).
