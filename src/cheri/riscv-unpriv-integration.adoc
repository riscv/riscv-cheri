...
NOTE: The _length_ and _top_ are (XLEN+1)-bit values, so <<GCLEN>> and <<GCTOP>> saturate their result to the maximum XLEN-bit value (2^XLEN^-1) when it does not fit in an XLEN-bit register.
...
However, the decoded top address of a capability is XLEN + 1 bits wide and does *not* wrap.
...
A capability whose {ctag} is set can always be assumed to satisfy these integrity rules, because it could only have been produced by a valid derivation.
Consequently, an integrity check can only change the result of an operation when it is performed on an input capability whose {ctag} is zero.
...
As stated above, all state which can hold addresses is extended from XLEN to YLEN bits.
