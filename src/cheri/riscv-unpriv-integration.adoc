...
[NOTE]
====
For non-{cheri_base_ext_name} bases, C.SRLI, C.SRAI and C.SLLI all have code points which are HINTs if _shamt_=0.

As a result, for {cheri_base_ext_name} bases:

 * No code points of C.SRLI and C.SRAI are HINTs.
