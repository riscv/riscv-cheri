...
Modifying any of its fields clears the valid tag of the output capability.
...
IMPORTANT: Permissions can only be removed, and any attempt to add permissions zeroes the valid tag.
...
Write Permission (W):: Allow writing integer data to memory. Valid tags are always
written as zero when writing integer data. Every CLEN aligned word in memory
has a tag, if any byte is overwritten with integer data then the valid tag for all
...
The valid tags are also tracked through the memory subsystem: every aligned CLEN-bit wide region has a non-addressable one-bit valid tag, which the hardware manages atomically with the data.
The valid tag is cleared if the memory region is ever written using an operation other than a store of a full capability register with <<c_perm>> granted.
...
Non-capability load instructions always zero the valid tag and metadata of the result register.
...
Therefore, misaligned stores may clear up to two valid tag bits in memory.
