...
[NOTE]
=====
Each thread (as scheduled by the kernel) can have its own context stored in privileged data structures managed by the compartmentalisaiton TCB, for example a stack of user-level compartment entries and exits.
<<tid>> allows compartment-switching code to determine the currently running thread without a call into the operating system.
...
Extending <<tid>> to a capability allows a data structure to be shared (guarded by e.g. the subset sealing mechanism), which allows for example efficient recovery of a trusted stack, without requiring additional indirection.
=====
