...
Valid capabilities are guaranteed to satisfy all integrity rules unless hardware faults or incompatible IP are present, as valid capability derivation preserves integrity.
Integrity checks are therefore _optional_ by default: checking a valid capability never alters execution on a correctly functioning system.
Only instructions that can take an arbitrary bit pattern which represents an invalid capability as an input and validate it _must_ perform an integrity check on the invalid capability.

NOTE: In the {cheri_base_ext_name} ISA, this mandatory check only applies to the `{cs2}` input to <<CBLD>>.
...
.Architecturally Valid Capability States
[#table_valid_cap_states,%autowidth,options=header,align="center",cols="2,1,4"]
|=======================
|State              |{ctag_table} | Description
|Valid capability   | 1  | All integrity checks have passed.
|Invalid capability | 0  | The YLEN-bit payload is not considered to be a capability unless authorized by <<CBLD>>.
|=======================

.Architecturally Invalid Capability States
[#table_invalid_cap_states,%autowidth,options=header,align="center",cols="2,1,4"]
|=======================
|State              |{ctag_table} | Description
|Top overflow       | 1  | All integrity checks have passed, but a logic fault has caused an undetected top overflow.
|Integrity fail     | 1  | At least one metadata field is in a reserved state, or the bounds are <<section_cap_malformed,malformed>>, or both.
|=======================
