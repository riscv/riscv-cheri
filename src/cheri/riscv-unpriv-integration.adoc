...
(_integrity_), and that derived capabilities must not grant permissions or bounds
...
The lower XLEN bits of a capability encode the address to which the capability points.
This is also referred to as the integer part of the capability.
When an instruction writes non-capability data to an extended register, the metadata fields and {ctag} of the destination are set to zero unless that instruction's definition explicitly states otherwise.
...
* Valid capabilities have the {ctag} set to one.
* Invalid capabilities have the {ctag} set to zero.
** The term _invalidating a capability_ means deriving a new capability with identical metadata and address but the {ctag} set to zero.

Using an invalid capability to dereference memory or authorize any operation raises an exception.
All capabilities derived from invalid capabilities are themselves invalid.
...
The implementation must set the {ctag} to zero if any byte in the naturally aligned YLEN-bit memory region is ever written using an operation other than the successful store of a capability operand.

For example, the <<STORE_CAP>> instruction sets the {ctag} to one if all the instruction's prerequisites are met, including `{cs2}` being a valid capability, but <<ldst,SW>> will always set the stored {ctag} to zero.

NOTE: A failed SC will leave the {ctag} in memory unmodified.
...
NOTE: Future CHERI bases may use capability-encoding formats with different bounds encoding schemes (see <<sec_cap_encoding_formats>>).
...
NOTE: The _length_ and _top_ are (XLEN+1)-bit values, so <<GCLEN>> and <<GCTOP>> saturate their result to the maximum XLEN-bit value (2^XLEN^-1) when it overflows an XLEN-bit register.

anchor:infinite_bounds[]

Infinite bounds::
Capabilities which allow access to every byte in memory have _infinite bounds_.

==== Deriving New Bounds

Upon initialization, the execution environment defines how capabilities are made available to the execution context.
All capabilities with smaller bounds are derived from these, or from capabilities in memory accessible by these.
...
Address arithmetic is modulo 2^XLEN^, so the byte at address 2^XLEN^ - 1 is adjacent to the byte at address zero.
A capability's <<section_cap_representable_check>> is also circular, so address 0 is within the <<section_cap_representable_check>> of a capability where address 2^XLEN^ - 1 is within the bounds.

However, the decoded top bound address of a capability is XLEN + 1 bits wide and does *not* wrap.
For example, a hypothetical capability with base 2^XLEN^ - 1 and top 2^XLEN^ + 1 authorizes access to the last byte of the address space only, and does not authorize access to the byte at address 0.
A capability with architectural top > 2^XLEN^ cannot be generated by any valid capability derivation and is only possible due to a logic or memory fault.

Unlike <<section_cap_malformed,malformed bounds>>, an architectural top > 2^XLEN^ is not special-cased in bounds decoding to return zero base and top; normal bounds check rules apply.
...
Instructions that operate on capabilities will produce an invalid capability if the source capability is sealed and the operation would result in altered address, bounds, or permissions.
...
Permissions grant access subject to the capability being valid, the capability being unsealed, and bounds checks passing.
...
+
The permission stripping behavior _only_ applies to valid loaded capabilities and that are not sealed after all other checks.
...
* **CSRs:** The privileged `__x__tidc` CSRs require <<asr_perm>> for writing, but not for reading (if access is permitted in the current privilege mode).
...
NOTE: Extensions may add additional unprivileged CSRs that require <<asr_perm>>.
...
NOTE: The rules are referenced by name and extended by {cheri_base32_ext_name} capability-encoding formats which cannot represent all combinations of permissions.

Extensions that define new permission bits may also introduce new dependency constraints.
Currently defined examples are:

. <<section_cheri_hybrid_ext,{cheri_default_ext_name}>>, see <<zyhybrid_clrperm_table>>
...
_Root_ capabilities are the set of capability encodings for the maximum set of bounds, covering the whole 2^XLEN^ address space, and that grant all encodable permissions.

[#root-rx-cap,reftext="Root Executable"]
Root Executable Capability::
An unsealed capability that has <<infinite_bounds,infinite bounds>> and grants at least all of <<x_perm>>, <<r_perm>>, <<c_perm>>, <<lm_perm>>, and <<asr_perm>>.

[#root-rw-cap,reftext="Root Data"]
Root Data Capability::
An unsealed capability that has <<infinite_bounds,infinite bounds>> and grants at least all of <<r_perm>>, <<w_perm>>, <<c_perm>>, and <<lm_perm>>.

NOTE: Extensions introducing new permissions may require these to be provided by <<root-rx-cap>> and/or <<root-rw-cap>> capabilities.

Upon initialization, the execution environment defines how the initial set of capabilities are made available to software.

The definitions of <<root-rx-cap>> and <<root-rw-cap>> are the maximum possible encodable values.

Implementations are not required to provide access to the entire address space to a single hart or even to provide all permissions.
As such, where a root capability value is specified, harts may specify implementation defined values instead with reduced bounds and/or permissions as required.

NOTE: The hart specific initial capabilities may be the maximum that the hart can ever have access to, but there could also be capabilities in memory with greater bounds or permissions.

NOTE: Some capability-encoding formats like {rv64y_uni_base_name} provide an <<section_infinite_cap_encoding,Infinite>> capability which provides all the characteristics of the full set of root capabilities in a single encoding.

NOTE: Future extensions may add additional root capabilities for purposes such as sealing and unsealing.

[#null-cap,reftext="NULL"]
===== NULL Capability
An invalid capability with all-zero metadata and address is referred to as the <<section_null_cap_encoding,NULL>> capability.
...
It is possible to generate code for {cheri_base_ext_name} that is compatible with all valid capability-encoding formats of the same address width.
...
Each new CHERI base ISA defines a capability-encoding format.
...
.{cheri_base_ext_name} base ISA parameters
[#rvy_base_isa_param_default_summary,width="100%",options=header,cols="1,1,1,4"]
|==============================================================================
| Parameter   | Legal values | Default Value        | Comment
...
. The bounds are not malformed (a capability encoding format-specific check; see xref:section_cap_malformed[xrefstyle=short] for {rv64y_uni_base_name}).
. The capability metadata does not have any bits, or fields, set to a value that is not valid for the implemented extensions.

NOTE: An example of a value that is not valid for {cheri_base_ext_name} is an illegal set of permissions such as <<asr_perm>> without <<x_perm>>.
Future extensions may allocate values that are reserved in {cheri_base_ext_name}; capabilities using those values will fail integrity checks on harts that do not implement that extension.

If any of the above rules do not hold for a valid capability, it indicates either:

* State corruption due to memory errors or logic glitches, or
* The presence of incompatible or defective CHERI IP within the system.

NOTE: These checks are much less rigorous than parity or ECC protection, and are only used to detect simple problems with the capability metadata.

NOTE: While a capability whose decoded bounds exceed 2^XLEN^ also represents a loss of integrity resulting from state corruption or incompatible IP, detecting this condition requires full bounds decoding and is not included in the capability integrity checks due to implementation cost.
...
Valid capabilities are guaranteed to satisfy all integrity rules unless state corruption or incompatible IP is present, as valid capability derivation preserves integrity.
Integrity checks on operands that are expected to be valid are therefore _optional_: performing an integrity check on a valid capability never alters execution on a correctly functioning system.
However, instructions that can produce a valid capability from an invalid input capability _must_ perform an integrity check on that input, since an invalid capability can hold arbitrary bit patterns.
Therefore, integrity checks are _optional_ except for instructions that are designed to operate on potentially invalid capabilities.

NOTE: In the {cheri_base_ext_name} ISA, this mandatory integrity check only applies to the `{cs2}` input to <<CBLD>>.

[NOTE]
====
Implementations may choose to enforce optional integrity checks for a number of reasons:

. Instruction-level verification is simpler as deterministic behavior is specified for all 2^(YLEN+1)^ input values (similar to how the D extension defines deterministic behavior for improperly NaN-boxed register values).
...
xref:table_cap_states[xrefstyle=short] summarizes the possible states of a capability register or memory location, including the corrupted states a valid capability can only reach through memory errors, logic glitches, or incompatible IP.

.Summary of capability states
[#table_cap_states,%unbreakable,width="100%",options=header,cols="3,^1,^1,5"]
|=======================
|State |{ctag_table} |Legal? |Description
|Valid capability |1 |Yes |Passes all <<section_cap_integrity,integrity checks>> and has `top ≤ 2^XLEN^`.
|Invalid capability |0 |Yes |Holds an arbitrary YLEN-bit payload and cannot authorize operations unless used as the `{cs2}` input to <<CBLD>>.
|Corrupted valid capability +
(integrity failure) |1 |No |Metadata has a value that is not valid for the implemented extensions or <<section_cap_malformed,malformed>> bounds, which is detected by optional integrity checks.
|Corrupted valid capability +
(`top > 2^XLEN^`) |1 |No |Not detected by integrity checks, as decoded `top > 2^XLEN^` is safely handled by existing bounds-checking logic.
|=======================
...
NOTE: The privileged `mconfigptr` CSR is the only documented exception to the rule; see <<mconfigptr>> for details.

NOTE: A reminder that _all_ YLEN state also has a hardware-managed {ctag}.

==== General-Purpose Registers

The <<gprs,XLEN-bit integer registers>> (e.g., `x1`, `x2`) are all widened to YLEN bits, each gaining an associated {ctag}, as shown in <<base_cap_registers>>.
...
ifdef::cheri_ratification_v1_only[]
e.g., _xtvec_ and _xepc_ from the privileged specification.
endif::[]
ifndef::cheri_ratification_v1_only[]
...
+
When a WARL CSR is widened the metadata and {ctag} also become WARL.
If the final written capability value cannot be written unmodified to the CSR due to WARL behavior then the capability value is considered to be _invalid_ and so must be written with its {ctag} set to zero.
+
NOTE: Examples of WARL behavior for metadata are implementations requiring all fields to be in a valid (non-reserved) state, and also to fix specific permissions to certain values.

[[ylen_csrs, YLEN-bit CSRs]]YLEN-bit CSRs::
This category includes:

* XLEN-bit CSRs widened to YLEN bits such as _mtvec_ from the privileged specification which contain pointers
* Natively YLEN-bit CSRs added by {cheri_base_ext_name} which contain pointers (e.g., <<utidc>>).

When accessing CSRs these rules are followed:

. Accesses to <<xlen_csrs>> are as specified by Zicsr
. Accesses to <<ylen_csrs>> using CSRRW will:
.. Read YLEN bits
.. Write YLEN bits, and will write the {ctag} to zero if the written value fails any <<section_cap_integrity, integrity>> checks.
. Accesses to <<ylen_csrs>> using instructions other than CSRRW will:
.. Read YLEN bits
...
NOTE: <<ylen_csrs,YLEN-bit>> CSRS are summarized in <<all_capability_CSRs_u>>, <<all_capability_CSRs_s>>, and <<all_capability_CSRs_m>>.
endif::[]
ifndef::cheri_standalone_spec[]
NOTE: <<ylen_csrs,YLEN-bit>> CSRS are summarized in <<all_capability_CSRs_u>>, and there are further references in the _Privileged Specification_.
endif::[]
...
.<<ylen_csrs, YLEN-bit CSR>> access summary for {cheri_base_ext_name}
...
|CSRR[C\|S]I uimm==0  |YLEN         |
|CSRR[C\|S]I uimm!=0  |YLEN         | XLEN
|=======================
...
* All bytes of the access must be in <<sec_cap_bounds_overview,bounds>>.
...
NOTE: A valid capability is assumed to satisfy the <<section_cap_integrity,integrity>> checks, so this rule can only affect the result for an operand whose {ctag} is set to zero.
...
ifdef::cheri_ratification_v1_only[]
Zcd (RV64) is incompatible with the {cheri_base_ext_name} base ISA, as its encodings are reused for the capability loads and stores below.
On harts that also implement {cheri_default_ext_name}, these encodings revert to Zcd when executing in {cheri_int_mode_name} (see xref:cheri_execution_mode[xrefstyle=full]).
endif::[]
...
In RVI/RVE, Zca arithmetic encodings with _rd_{ne}``x0`` and a zero immediate or shift amount are defined as HINTs because they leave the integer value in _rd_ unchanged.
In {cheri_base_ext_name}, executing an integer arithmetic instruction that writes _rd_{ne}``x0`` sets the destination register's metadata and {ctag} to zero, so the following encodings are instead _reserved_ (leaving only Zca HINT encodings with _rd_=`x0` as HINTs):

* `C.ADDI` with _rd_{ne}``x0`` and _imm_=0
* `C.SLLI` with _rd_{ne}``x0`` and _shamt_=0
* `C.SRLI` with _shamt_=0
* `C.SRAI` with _shamt_=0

NOTE: Marking these encodings as _reserved_ allows future extensions to allocate these code points consistently across RVI/RVE and {cheri_base_ext_name}.
...
== U-mode YLEN CSR Summary

<<all_capability_CSRs_u>> shows all YLEN U-mode CSRs added or modified by {cheri_base_ext_name}.

.All {cheri_base_ext_name} U-mode CSRs.
[#all_capability_CSRs_u]
[width="100%",options=header,cols="3,3,2,2,3,3,5"]
|==============================================================================
include::{generated_dir}/csr_permission_u_table_body.adoc[]
|==============================================================================

NOTE: Where initial values are specified as a <<root-rx-cap>> or <<root-rw-cap>> they are the maximum possible value.
The EEI may initialize those CSRs to cover a smaller memory range, or to grant fewer permissions.

.Action taken on writing to YLEN-bit and extended U-mode CSRs
...
.YLEN-bit U-mode CSRs storing code pointers or data pointers
[#CSR_exevectors_u]
