...
The memory representation of capabilities is always `2*XLEN` (`YLEN`) bits wide and holds the address in the first `XLEN` bits of memory followed by the metadata.
...
When an operation fails a check, the operation raises an exception or sets the resulting {ctag} to zero, as defined by each instruction.
...
For example, the <<STORE_CAP>> instruction sets the {ctag} to one if all the instruction's prerequisites are met, but <<ldst,SW>> will always set the {ctag} to zero.
...
If the bounds _do_ change then the {ctag} of the derived capability is set to zero, so that it is invalid for use.
