...
// NOTE: Instructions like <<SCBNDS>> can be reused to reduce the bounds of an existing capability while retaining the valid tag, or they can be used to construct a new capability which is then validated using <<CBLD>>.
...
Instructions that decode bounds will typically set the output valid tag to zero if the input is found to be <<section_cap_malformed,malformed>>.

NOTE: CHERI requires that the capability is correctly formed if the valid tag is set.
Defining the handling of malformed bounds and invalid settings of reserved bits in the architecture allows the behavior to be precisely specified for all 2^(CLEN+1)^ input values (even though some of these are impossible to observe in a correctly operating environment).
...
#ARC note: I think this chapter needs to be after Zicsr, otherwise it's painful to add ASR and utid/utidc later as we'll end up with a fragmented description of ACPERM#
...
When beginning execution <<pcc>> bounds and permissions must have been set such that the program can run successfully (e.g., by setting it to <<infinite-cap>> to ensure _all_ instructions are in bounds).
...
NOTE: An access fault is raised instead of a misaligned exceptions since these instructions cannot be emulated since there is one hidden valid tag per CLEN-aligned memory region.
...
//
//<<JALR_CHERI>> raises a CHERI exception when:
//
