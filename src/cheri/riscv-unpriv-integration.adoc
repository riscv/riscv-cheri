...
In RVI/RVE, several Zca arithmetic encodings with _rd_{ne}``x0`` and a zero immediate (`C.ADDI` with _imm_=0, and `C.SLLI`, `C.SRLI`, and `C.SRAI` with _shamt_=0) are defined as HINTs because they do not modify the integer value in _rd_.
In {cheri_base_ext_name}, however, integer arithmetic instructions that write _rd_{ne}``x0`` clear the capability metadata and {ctag}, so these encodings would have an architectural side effect.
Therefore, for {cheri_base_ext_name}, only Zca HINT encodings with _rd_=`x0` remain HINTs, and the following encodings with _rd_{ne}``x0`` are _reserved_:
