...
.. Read YLEN bits
.. Write an XLEN-bit value to the address field, and use semantics of the <<SCADDR>> instruction to determine the final written value
...
* Whenever a capability with a new address is returned, the result is _always_ created using semantics of the <<SCADDR>> instruction.
...
* Whenever the address field of the <<pcc>> is modified, it is _always_ updated using semantics of the <<SCADDR>> instruction.
