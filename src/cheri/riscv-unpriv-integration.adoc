...
** The bounds may be adjusted to the next representable size if they cannot be encoded exactly but must be no larger than the initial bounds.
...
deriving a capability with `CT{ne}0` is termed _sealing_
...
.. Read YLEN bits
.. Write YLEN bits, and will write the {ctag} to zero if any <<section_cap_integrity,integrity>> checks fail
