...
However, to allow compilers and/or software library authors to make assumptions about encoding-dependent properties such as bounds precisions, these are encoded via an `_Ly<params>` suffix to the base ISA name such as `{rv32y_uni_base_name}_IMAF_Zicsr`.
