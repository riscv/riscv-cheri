...
Zca HINT encodings with _rd_{ne}``x0`` are _reserved_ instead of HINTs, because executing an integer arithmetic instruction that writes _rd_{ne}``x0`` sets the destination register's metadata and {ctag} to zero (leaving only the 32 (RV32) or 64 (RV64) C.SLLI code points with _rd_=`x0` as HINTs).

Therefore the following encodings are _reserved_ instead of HINTs:
