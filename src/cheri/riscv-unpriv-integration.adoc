...
The documentation for each capability encoding format specifies at least the parameters in <<cap_encoding_param_summary>> below where the default is not used.

.{cheri_base_ext_name} capability encoding format parameters
[#cap_encoding_param_summary,width="100%",options=header,cols="1,1,1,4"]
|==============================================================================
...
AP_ENC::
 ...
 Each scheme has a suffix to show which exact version is in use, as `ps` scheme may not always store permissions in the same order and `pc` schemes may use different forms of compression.
REP_RANGE::
...
Any implementation with matching parameters is binary compatible (assuming the same extensions are implemented).
