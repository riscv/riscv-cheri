...
The {cheri_base_ext_name} base ISA can be composed with other standard options to bases such as E (16-registers) and Zfinx.
...
The full specification of a base ISA includes a capability encoding format.

E.g., in {rv64y_uni_base_name} `XLEN=64`, and in {rv64y_uni_encoding_suffix} `L` means that it's a parameterized base, `Y` means that it's a CHERI base and `A` is the specific capability encoding format.
...
Also see xref:sec_cap_encoding_formats[].
...
Therefore a permission bit being set in the result of <<GCPERM>>
...
Capability encoding formats with `AP_ENC=pc<suffix>` may impose additional constraints to reduce the number of bits necessary to represent permissions.
