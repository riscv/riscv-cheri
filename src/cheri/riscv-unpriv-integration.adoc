...
code is referred to as a _trampoline_. Sentries can
...
The <<utidc>> register is designed to hold a capability, which can only be accessed by
trusted code, which can be ensured through making the capability a sentry.
Trusted code can access memory through this capability and use it for implementing compartment
switches, e.g., by implementing a trusted stack.
...
that facilitates a trusted source for the thread ID.
