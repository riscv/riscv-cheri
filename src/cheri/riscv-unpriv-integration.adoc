...
{cheri_base_ext_name} introduce new instructions to the base RISC-V integer ISA to inspect and operate on capabilities held in registers.
...
These instructions all manipulate or copy capabilities.
...
These instructions all extract information from a capability.
They do not cause exceptions.
...
NOTE: There is no intention to allow emulation of misaligned capability memory accesses.
