...
NOTE: #the following notes should move to a programmers guide#
...
* The result written to the target register *x* is _always_ unchanged. I.e., the lower XLEN-bits are always written with the value from RV32I/RV64I.
...
** The base address is unchanged, using the value from `rs1`.
...
NOTE: Exceptions are raised _on the branch or jump_ which cause an exception, not at the target, primarily to reduce the attack surface but also to aid with debugging.
