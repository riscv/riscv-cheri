...
{cheri_base_ext_name} extends the general purpose registers to `2*XLEN-bit` (hereafter referred to as YLEN), adding metadata to protect its integrity, limit how it is manipulated, and control its use.
In addition to widening to YLEN, each register also gains an out-of-band (hidden / hardware managed) one-bit valid tag.
...
The valid tag is an additional hardware managed bit added to addressable memory and all *y* registers.
It is stored separately and may be referred to as _out of band_ or _hidden_.
It indicates whether a *y* register or YLEN-aligned memory location contains a valid capability.
If the valid tag is set, the capability is valid and can be dereferenced (contingent on checks such as permissions or bounds).
...
All locations in registers or memory able to hold a capability are YLEN-bits wide including the valid tag bit.
...
IMPORTANT: Bounds can only be reduced, and any attempt to increase them zeroes the valid tag.
...
written as zero when writing integer data. Every YLEN aligned word in memory
has a tag, if any byte is overwritten with integer data then the valid tag for all
YLEN-bits is cleared.
...
.CHERI *y* registers extending existing *x* registers in {cheri_base_ext_name}
[#base_cap_registers]
[bytefield]
----
...
For the ABI register names the `c` prefix is added, i.e. `ysp`, `ca0`.
The zero register is extended with zero metadata and a cleared valid tag: this is called the <<null-cap>> capability.

NOTE: Instruction encodings refer to *y* registers when they use `ys1`, `yd`, etc. instead of `rs1`, `rd` (which reads/writes the XLEN-bit subset of the register).
...
NOTE: All CSRs that hold pointers are extended to YLEN bits, but since no CSRs are defined in the RV32I/RV64I base instruction set, any such change is described in the corresponding chapters.
...
The valid tags are also tracked through the memory subsystem: every aligned YLEN-bit wide region has a non-addressable one-bit valid tag, which the hardware manages atomically with the data.
...
For example, `lw t0, 16(ysp)` loads a word from memory, getting the address, bounds, and permissions from the `ysp` (capability stack pointer) register.
...
|<<CBLD>>     |Set yd to ys2 with its tag set after checking that ys2 is a subset of ys1
|=======================
...
|<<SCSS>>   |Set result true if ys1 and ys1 tags match and ys2 bounds and permissions are a subset of ys1
...
They all operate in the same manner as the base ISA but access YLEN bits and also the valid tag.

All capability memory accesses requires <<c_perm>> in the authorizing capability.
If <<c_perm>> is not granted then:

* All capability loads will return YLEN-bits from memory, and a zero valid tag.
* All capability stores will write YLEN-bits to memory, and write a zero valid tag.
...
All capability memory access instructions require YLEN aligned addresses, and will
take the appropriate misaligned exception if this requirement is not met.

All capability memory accesses require permission from the authorizing capability in *ys1*.
...
NOTE: Misaligned capability memory accesses cannot be emulated.
  To transfer YLEN misaligned bits without a valid tag, use integer loads and stores.
...
The affected operands change type from *x* to *y* to represent this, and/or the PC is replaced by the <<pcc>>
...
* <<JALR_CHERI>> copies _all_ of `ys1` into the <<pcc>>. If `ys1` is not suitable for execution then an exception is taken (see <<pcc>> for the definition).
...
|<<JAL_CHERI>>              |Jump to PC+offset, bounds check minimum size target instruction, link to yd
...
|<<section_int_load_store_insns,LD, LW[U], LH[U], LB[U]>>|Integer loads (authorized by the capability in `ys1`)
|<<section_int_load_store_insns,SD, SW, SH, SB>>|Integer stores (authorized by the capability in `ys1`)
|=======================
...
//
//For indirect jumps, the authorizing capability is not <<pcc>> but the base capability register in *ys1*.
//
...
* For the base operand (`rs1`), the full capability register `ys1` is used to authorize the resulting memory access.

All load and store instructions authorized by `ys1` cause CHERI exceptions if any of these checks fails:

* `ys1` must not be `c0`^1^
* The valid tag (`ys1.tag`) must be set, and there must be no reserved bits set in `ys1`.
* `ys1` must be unsealed
* The virtual address in `ys1` must not be invalid if virtual memory is enabled.
* All bytes of accessed memory must be inside `ys1's` bounds, and the bounds can't be <<section_cap_malformed,malformed>>.
* For loads, the <<r_perm,read permission>> must be set in `ys1`
* For stores, the <<w_perm,write permission>> must be set in `ys1`

^1^ All load/store encodings are _reserved_ if `ys1=c0` (since dereferencing <<null-cap>> always faults).
 This principle is extended to _all_ loads and stores authorized by `ys1`.

Non-capability load instructions always zero the valid tag and metadata of the result register.

Non-capability stores write zero to the valid tag associated with the memory locations that are naturally aligned to YLEN.
...
NOTE: The changed interpretation of the base register also applies to all loads and stores defined in later chapters of this specification (including floating-point and vector loads/stores).
 Under {cheri_base_ext_name} _all_ loads and stores are authorized by `ys1`.
