...
>>>>>>> 107bfa2b (final merge issues resolved)
...
No other exception paths are added by {cheri_base_ext_name}: in particular, capability manipulation instructions do not except, but may clear the valid tag on the result capability if the operation is not permitted.
