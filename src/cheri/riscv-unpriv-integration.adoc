...
However, control-flow instruction can be used to jump to a sentry capability to begin executing the instructions it references.
+
...
.New CHERI CSR added in {cheri_base_ext_name}
[#base_csr_registers]
[bytefield]
----
(defattrs :plain [:plain {:font-family "M+ 1p Fallback" :font-size 40}])
...
(draw-box "" [:box-first {:span 1}]) (draw-box "utidc" [:box-related {:span 32}]) (draw-box "utidc" [:box-last {:span 32 }])
(draw-box "V" {:span 1 :borders {}}) (draw-box "Metadata" {:span 32 :borders {}}) (draw-box "Address" {:span 32 :borders {}})
----
...
NOTE: Compartmentalization seeks to separate the privileges between different
...
NOTE: The <<utidc>> register is designed to hold a capability, which can only be accessed by
...
taken over by more privileged code, e.g., the kernel running on a different privilege level. The privileged
code switches software threads and writes the <<utidc>> register.

NOTE: Every piece of code in the user space (and more privileged levels) can read the content of the <<utidc>>
...
NOTE: Trusted code can use the sentry in <<utidc>> to implement secure compartment switches. Often, the sentry is
use to implement a trusted stack. Whenever a compartment switch happens, the trusted code can pass arguments between
...
that facilitates a trusted source for identifying software threads.
