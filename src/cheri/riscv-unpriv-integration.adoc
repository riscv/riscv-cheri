...
NOTE: Future RV64 CHERI capability encoding format could be RV64LYB, RVY64LYC etc. or they may have more descriptive names after the RV64LY prefix.
...
All registers or memory locations able to hold a valid capability are YLEN bits wide with an additional hidden {ctag} bit.
These are referred to as being _YLEN-bit_ in this specification.

NOTE: Not all memory in a real system will support {ctag}s.
 The execution environment is able to configure the regions {ctag} supporting memory which are visible to user mode.
...
If they _do_ change then the {ctag} of the derived capability is set to zero, so that it is invalid for use.
...
The capability encoding specifies a mapping between the CT-field within the capability format and the architectural capability types.
...
Whilst {cheri_base64_ext_name} has enough bits in the encoding format to prevent such trade-offs in general, the concepts are introduced in here and {cheri_base32_ext_name} formats will make extensive use of them.
...
| AP_ENC      | ps<suffix>/ pc<suffix>| ps1                  | Encoding scheme for the <<AP-field>>
4+| *Other parameters*
| AUIPC_SHIFT | integer values up to 12 | 12                   | Shift distance of the immediate in AUIPC
| REP_REGION  | rc1/r0               | rc1                  | Whether out of bounds addresses are representable
|==============================================================================

Two parameters used non-numeric values:

AP_ENC::
 Whether permissions in the <<AP-field>> are encoded using a _simple_ representation of one bit per architectural permission (`ps`) or a _compressed_ encoding that leverages dependencies between permissions and does not allow encoding all possible combinations (`pc`).
 Each scheme has a suffix to show which exact version is in use, as `ps` schemes may not always store permissions in the same order and `pc` schemes may use different forms of compression.
REP_RANGE::
...
NOTE: For {cheri_base64_ext_name}, the number of spare bits in the capability encoding ensures sufficient future extensibility and therefore few custom formats are expected to exist.
 {cheri_base32_ext_name}, however, has insufficient bits available to enable features for all target domains, so multiple {cheri_base32_ext_name}-extending capability formats will exist.
...
Capability subset::
Some instructions used the concept of `{cs2}` being a _capability subset_ of `{cs1}`.
The definition is:
+
...
All load instructions, except for <<LOAD_CAP>>, always zero the {ctag} and metadata of the result register.
...
NOTE: These encodings are redundant and may be used by future extensions, suggestions include branching on {ctag} values only, or YLEN-bit compares.

NOTE: A future extension may raise an exception on branch instructions if fetching a minimum sized instruction at the target <<pcc>> will raise a _{cheri_excep_name_pc}_.
...
Zcf (RV32) and Zcd (RV64) are incompatible with {cheri_base_ext_name}.
...
An {cheri_base_ext_name} core which supports Zca also supports {rvy_zca_mod_ext_name} which _modifies_ the behavior of some instructions.
