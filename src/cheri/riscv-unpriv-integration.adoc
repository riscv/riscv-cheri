...
The {cheri_base_ext_name} ISA do not allow a capability to be written to the pcc which doesn't meet the following rules:

1. The valid tag is set.
2. <<x_perm>> is granted.
3. Is not sealed.
4. Has no reserved bits set.

NOTE: Future extensions should respect these rules so that these bits do not need storing in the implementation.
