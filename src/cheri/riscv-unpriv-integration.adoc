...
This chapter introduces several new bases that are informally collectively called {cheri_base_ext_name} (e.g., {cheri_base32_ext_name}, {cheri_base64_ext_name}), that extend the RV32 and RV64 instruction sets with CHERI.
The CHERI bases can be composed with other standard options to bases such as E (16-registers), Zfinx and endianness.
...
However, the precise in-memory representation of the capability metadata is not defined in this chapter since multiple legal (and generally software-compatible) encodings exist.
On little-endian systems, the memory representation of capabilities always holds the address in the first `XLEN` bytes of memory followed by the metadata.

NOTE: So far, no big-endian implementations of {cheri_base_ext_name} exist, but it is likely that such systems would switch the order of metadata and address in memory for consistency (even though having the address first is better for software compatibility).
...
For more details on the currently available formats see <<sec_cap_encoding_formats>>.
...
However, to allow compilers and/or software library authors to make assumptions about encoding-dependent properties such as bounds precisions, these are encoded via an `_Ly<params>` suffix to the base ISA name such as:

* `{rv32y_uni_base_name}_IMAF_Zicsr`, which refers to the encoding format defined in xref:rv32y_cap_description[xrefstyle=full] for {cheri_base32_ext_name}, or
* `{rv64y_uni_base_name}_IMAF_Zicsr`, which refers to the encoding format defined in xref:rv64y_cap_description[xrefstyle=full] for {cheri_base64_ext_name}
...
NOTE: Writing non-capability data into a register or memory location causes the {ctag} to be cleared.
...
* The _top_ is returned by the <<GCTOP>> instruction (if <<section_rvy_topr_insn_ext>> is implemented).
** Otherwise the _top_ can be calculated using a saturating addition of the <<GCBASE>> and <<GCLEN>> results.
...
NOTE: {cheri_base_ext_name} implementations that use a different encoding scheme to <<rv32y_cap_description>>/<<rv64y_cap_description>> (e.g., for accelerators or specific micro-controllers) may specify an alternative to the representable region check, and may never allow the address to be out of bounds.
...
Which capability types a given CHERI platform supports is a function of the extensions and capability encoding format in use.
...
Standard capability type mapping is shown in <<table_capability_types>>.

[#table_capability_types]
.Capability type mapping in the {rv32y_uni_base_name}/{rv64y_uni_base_name} bases
...
(<<rv32y_cap_description>>/<<rv64y_cap_description>> does not avail itself of this option,
but, for example, the <<rv32y_cheriot_encoding1_description>> capability encoding does.)
...
(With the capabilities of <<rv32y_cap_description>>/<<rv64y_cap_description>>, that would be type `1`.)
...
The <<JALR_CHERI>> instruction may unseal capabilities of particular, encoding-specified types before installing them to the program counter.
...
// For some reason I get `possible invalid reference: section_zylevels1` if I use a plain <<section_zylevels1>> here... (the link works in the pdf but the reftext is not expanded).
an example of such an extension is <<section_zylevels1,{cheri_levels1_ext_name}>>.
...
Capability _encodings_ may impose additional constraints to reduce the number of bits necessary to represent permissions.
...
The CHERI base ISA name is extended with a compact description of these decisions,
and the definition of all CHERI base ISAs includes sufficient details of a capability encoding format precisely define the execution behavior of all base ISA instructions.

As a result, the {cheri_base32_ext_name} and {cheri_base64_ext_name} names on their own do not describe the complete base ISAs, but define all the _semantics_ of a {cheri_base_ext_name} machine, where certain behavioral parameters must be filled in by the capability encoding.
...
NOTE: These observable differences in behaviour are similar to floating point numbers where the supported operations and their semantics are the same for all formats, but the exact result depends on the representation (e.g., 16/32/64-bit IEEE-754 or other floating point formats).

The encoding format parameters are specified in the architecture string using a `_Ly<params>` syntax, which can be used by software and/or the compiler to optimize for that specific in-memory format.

NOTE: The base `_L<encoding_parameters>` encoding format scheme is designed to separate the aspects of capability encoding that influence the behavior of {cheri_base_ext_name} instructions from the rest.
...
For example, the <<rv32y_cheriot_encoding1_description>> format uses  `e14` since it uses a four-bit exponent field that can encode only the values 0 through 14 and 24 (which, with its 9-bit mantissas, is sufficient to provide an encoding of bounds that cover the whole address space).
...
Vendors may define alternate formats where all other parameters are otherwise identical, so this parameter can be used to differentiate such formats.
...
NOTE: To allow arbitrary characters in the suffix of this parameter it must be the last component of the `_Ly<params>` to avoid ambiguity with other parameters.
...
Other parameters::
These other parameters are not used by the formats currently set forward for ratification but are being used in vendor extensions on top of {cheri_base_ext_name}.
* **<<AUIPC_CHERI>> shift (as<N>)**: The shift amount used in the {cheri_base_ext_name} variant of <<AUIPC_CHERI>>. This is 12 (as in RVI) unless specified otherwise.

As with other extensions, a version number suffix may be added to the `_Ly` string to identify a new version with the same parameters but different in-memory representation.
Any newer version must be forwards compatible with any existing software using the same parameters.
Custom capability encoding formats may be implemented for vendor-specific non-standard implementations (their name suffix should start with `_Lxy`).
...
IMPORTANT: The behavior of all {cheri_base_ext_name} instructions is fully defined by the parameters in the `_Ly` string.
Therefore, any implementation with matching parameters is binary compatible (assuming the same extensions are implemented).

The currently known {cheri_base_ext_name} base capability encoding formats are listed below:

* For {cheri_base64_ext_name}, the capability encoding format based on University of Cambridge CHERI v9 cite:[cheri-v9-spec]: <<rv64y_cap_description, {rv64y_uni_base_name}>>
* For {cheri_base32_ext_name}, the capability encoding format based on University of Cambridge CHERI v9 cite:[cheri-v9-spec]: <<rv32y_cap_description, {rv32y_uni_base_name}>>
...
For {cheri_base64_ext_name}, the number of spare bits in the capability encoding ensure sufficient future extensibility and therefore no vendor-defined formats exist.
However, {cheri_base32_ext_name} has insufficient bits available to enable features for all target domains, so in addition to the <<rv32y_cap_description, {rv32y_uni_base_name}>> further {cheri_base32_ext_name}-extending capability formats exist:

* The CHERIoT encodings (with `{rvy_cheriot_common_base_suffix}` parameters) optimized for smaller devices: <<rv32y_cheriot_encoding1_description>>, <<rv32y_cheriot_encoding2_description>>, and <<rv32y_cheriot_encoding3_description>>.

// Can uncomment this once CHERIoTv2 format is included in this spec?
//
// TODO: what does E have to do with anything?  Is that just a shorthand for
// "a sufficiently small microcontroller"?
ifndef::cheri_ratification_v1_only[]
NOTE: RV32YE (16-register) implementations are recommended to use one of these CHERIoT encodings.
endif::[]
...
For all other instructions, a malformed input capability can only be triggered by glitches in the system and therefore performing these checks is optional unless the hart implements additional reliability features.
...
{cheri_base_ext_name} adds the YLEN-bit CSR shown in
xref:csrnames-added-y[xrefstyle=short].
ifdef::cheri_xycfg_csr[]
It also adds the XLEN-bit CSR shown in xref:csrnames-added-x[xrefstyle=short]
endif::[]
...
.2+|0x1       | 0x0    | <<rv32y_cap_description>>/<<rv64y_cap_description>> capabilities, without <<section_zylevels1>> features
              | 0x1   <| <<rv32y_cap_description>>/<<rv64y_cap_description>> capabilities, with <<section_zylevels1>> features
...
Extensions that define encodings derived from <<rv32y_cap_description>>/<<rv64y_cap_description>>
...
[#section_rvy_topr_insn_ext,reftext="{rvy_topr_insn_ext_name}"]
== "{rvy_topr_insn_ext_name}" Extension for Extracting the Top Bound for Memory Allocators

The {rvy_topr_insn_ext_name} extension adds the <<GCTOP>> instruction to return the top bound of a capability.
This offers a performance improvement over the software sequence of a saturating add of the results of <<GCBASE>> and <<GCLEN>>, and is useful for memory allocators.
