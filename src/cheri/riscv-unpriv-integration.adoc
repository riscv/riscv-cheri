...
. Defines the <<sec_cap_type>> of the sentry type to be type `1`.
...
. For HOOK 1: No action.
. For HOOK 2: If the capability in {cs1} is sealed with a sentry of type `1`, then unseal before installing in <<pcc>>.
. For HOOK 3: Seal the return capability as a sentry of type `1`.
