...
NOTE: {cheri_base_ext_name} implementations that use a different encoding scheme to <<rvy32_cap_description>>/<<rvy64_cap_description>> (e.g., for accelerators or specific micro-controllers, see <<uycfg>>) may specify an alternative to the representable region check, and may never allow the address to be out of bounds.
Therefore, the rest of the specification uses the phrase *represented exactly* for this check.

The bounds and representable region (or space) for <<rvy32_cap_description>>/<<rvy64_cap_description>> are illustrated in <<cap_bounds_map>>.
...
(<<rvy32_cap_description>>/<<rvy64_cap_description>> does not avail itself of this option,
...
(With the capabilities of <<rvy32_cap_description>>/<<rvy64_cap_description>>, that would be type `1`.)
...
When using <<rvy64_cap_description,{rvy64_uni_base_name}>> and <<rvy32_cap_description,{rvy32_uni_base_name}>> the complete set of rules with and without <<section_cheri_hybrid_ext,{cheri_default_ext_name}>> and <<section_zylevels1>> are shown in <<clrperm-rules-no-zylevels1>> and <<clrperm-rules-zylevels1>>.
...
NOTE: The {rvy64_uni_base_name} and {rvy32_uni_base_name} base extensions are closely modeled upon features from CHERI v9 cite:[cheri-v9-spec], and the bounds encoding scheme is based upon CHERI Concentrate cite:[woodruff2019cheri].
...
.2+|0x1       | 0x0    | <<rvy32_cap_description>>/<<rvy64_cap_description>> capabilities, without <<section_zylevels1>> features
              | 0x1   <| <<rvy32_cap_description>>/<<rvy64_cap_description>> capabilities, with <<section_zylevels1>> features
...
Extensions that define encodings derived from <<rvy32_cap_description>>/<<rvy64_cap_description>>
...
NOTE: In principle, despite the names, the type used by <<SENTRY>> might not be a <<sentry_cap>> type, though that is not true of the capabilities of <<rvy32_cap_description>>/<<rvy64_cap_description>>.
