...
[NOTE]
====
Any instruction that can set the {ctag} to one on a previously untagged input (such as <<CBLD>>) must reject capabilities with decoded top > 2^XLEN^; for <<CBLD>>, this is automatically satisfied by its subset check (top~2~ {le} top~1~ {le} 2^XLEN^).
For memory accesses using an authorizing capability (such as loads, stores, and instruction fetch), hardware is not required to detect or raise an exception for decoded top bounds exceeding 2^XLEN^.
Standard XLEN + 1 bit bounds checking already fails closed: because memory addresses are strictly less than 2^XLEN^ and the decoded top bound does not wrap, an overflowed top bound never authorizes access to unintended memory locations (such as address 0).
Detecting fault-induced capabilities with top > 2^XLEN^ is deferred to future reliability extensions.
====
