...
On system initialization the <<infinite-cap>> is available which has bounds which cover all of memory, and maximum permissions set.
...
NOTE: CHERI versions which use a different encoding scheme to the default may specify an alternative to the representable region check.
 Therefore the rest of the specification uses the phrase *represented exactly*
...
NOTE: This permission affects access to the <<utidc>> register.
...
{cheri_base32_ext_name} and {cheri_base64_ext_name} add the capability CSR shown in
xref:utidc-csrnames-added[xrefstyle=short].

[[utidc-csrnames-added]]
.Unprivileged capability CSRs added in {cheri_base32_ext_name} and {cheri_base64_ext_name}
[%autowidth,float="center",align="center",cols="<,<,<,<",options="header"]
|===
|CLEN CSR|Address|Permissions|Description
|<<utidc>>|0x480|RW, <<asr_perm>> required for writes, not reads|User Default Data Capability
|===
...
On reset the valid tag of <<utidc>> will be set to 0 and the remainder of the data is UNSPECIFIED.

.User thread identifier capability register
include::img/utidcreg.edn[]

NOTE: This section describes how this specification enables support for
...
NOTE: The RISC-V ABI includes a _thread pointer (tp)_ register, which is not
...
by untrusted code. Therefore, this specification offers an additional CSR
that facilitates a trusted source for the thread ID. The register is readable
from U mode and writeable with <<asr_perm>>.
...
Instruction fetches, branches, jumps, and data memory accesses may result in a fatal exception if the access is out of <<sec_cap_bounds,bounds>>, or if the authorizing capability is missing the required <<sec_cap_ap,permissions>>. I.e.:
...
Stores instructions except for <<SC>> always write zero to the valid tag or tags associated with the memory locations that are written to.
