...
[#sec_cap_type, reftext="Capability Type"]
==== Capability Type (CT)

This metadata value indicates the type of the capability.
The sole type defined in {cheri_base_ext_name} is `0`.
...
They cannot be dereferenced to access memory and it is not permitted
to change the address, bounds, permissions, type, or any other field of the capability.
Extensions that augment capability metadata must describe their interaction(s) with sealed capabilities.

Given a capability with `CT=0`, deriving a capability with `CT≠0` is termed "sealing".
Dually, deriving a `CT=0` capability from a `CT≠0` capability is termed "unsealing".
Each of these actions require *authority* to operate at the non-zero type;
extensions will specify how software expresses this authority
(for example, with novel instructions and/or forms of capabilities).
...
For the base set of permissions just defined, the follow rules apply.
...
even non-dependency constraints,
...
| base-1          | <<c_perm>>   | <<r_perm>> or <<w_perm>>
| base-2          | <<lm_perm>>  | <<c_perm>> and <<r_perm>>
| base-3          | <<asr_perm>> | <<x_perm>>
|===
...
|<<ycfg>>|WARL; see section for details|Capability encoding version
|===
...
trusted code will be given unsealing rights (see <<section_zyseal>>) or a superset capability (see <<CBLD>>)
sufficient to unseal the <<utidc>> capability.
For the untrusted code, the sealed capability in <<utidc>> grants no authority
The sealed capability itself is no secret,
but the memory to which it points is trusted and must not be accessed by any untrusted code.

Trusted code can use the capability in <<utidc>> to implement secure compartment switches.
Often, it is used to implement a trusted stack.
Whenever a compartment switch happens, the trusted code can pass arguments between
...
[#ycfg,reftext="ycfg"]
===== CHERI Capability Encoding (ycfg)

The <<ycfg>> register is used to identify which CHERI capability encoding is used by the platform.
The capability encoding determines both the in-memory representation of a CHERI capability
and which CHERI features are available for software's use.
...
This CSR is always present in the machine mode read/write CSR space;
if the hart supports user mode, a `uycfg` register exists,
if the hart supports supervisor mode, an `sycfg` register exists, and
if the hart supports hypervisor mode, a `vsycfg` register exists.
`mycfg` is always read-only, as is `vsycfg`, if it exists.
`uycfg` is a read-only alias of `sycfg`, if it exists, or of `mycfg` otherwise.
If the hypervisor extension is enabled, then `sycfg` is WARL,
updatable from M or H mode with <<pcc>> granting <<asr_perm>>.

The base specification defines the following values for `mycfg`, `sycfg`, and `uycfg`:
|===
|Version value|Encoding
|0|Reserved (unspecified capability encoding)
|0x001|<<app_cap_description>> capabilities, without <<section_zylevels1>> features
|0x101|<<app_cap_description>> capabilities, with <<section_zylevels1>> features
|0x002|Reserved for CHERIoT's (RV32-only) encoding
|===

Future extensions that change capability encoding are expected to define new values.
Extensions that define encodings derived from <<app_cap_description>>
are encouraged to preserve the bottom 8 bits as `0x01`.
