...
The capability encoding must be able to encode type `0` (_unsealed_) and at least one non-zero type to implement control-flow integrity.
...
<<sec_cap_type>> of 1 is enabled by the <<section_rvy_sentry_insn_ext>> extension.
...
If <<section_rvy_sentry_insn_ext>> is implemented, control-flow integrity is achieved using <<JALR_CHERI>>:
