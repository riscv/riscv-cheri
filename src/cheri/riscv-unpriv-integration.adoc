...
{cheri_base_ext_name} specifies the minimum required fields which the capability format must support, and their semantics.
...
For example, a small embedded controller with limited address space of a few megabytes may need to make different tradeoffs than an auxiliary core within a larger SoC with many gigabytes of address space, and a accelerator using the {cheri_base_ext_name} ISA may need yet another set of features.
...
** #If {rvy_topr_insn_ext_name} is implemented, otherwise the _top_ can be calculated using a saturating addition of the <<GCBASE>> and <<GCLEN>> results.#
...
NOTE: In principle, despite the names, the type used by <<SENTRY>> might not be a <<sentry_cap>> type, though that is not true of the capabilities of <<rv32y_cap_description>>/<<rv64y_cap_description>>.

NOTE: Future extensions to {cheri_base_ext_name} (such as CHERIoT) support additional <<sec_cap_type>> values, so this instruction is part of an optional extension instead of the {cheri_base_ext_name} base ISA.
...
This offers a performance improvement over the software sequence of a saturating add of the results of <<GCBASE>> and <<GCLEN>>, and is useful fo rmemory allacators.
