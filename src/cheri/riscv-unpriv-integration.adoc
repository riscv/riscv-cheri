...
* Storing a capability to memory preserves the {ctag} in memory only if the store is performed using a valid capability that authorizes storing capabilities to that memory location.
* Loading a capability from memory preserves the {ctag} in the destination register only if the load is performed using a valid capability that authorizes loading capabilities from that memory location.
...
This {ctag} is set to zero whenever an invalid capability value is written to the register.
...
The standard bounds encoding format for {rv64y_uni_base_name} is based upon the CHERI Concentrate scheme cite:[woodruff2019cheri] and defined in <<section_cap_bounds>>.
...
On initialization of the execution environment, the execution environment defines how the <<root-cap>> capabilities are made available to the execution context.
...
In addition, the bounds encoding allows the address to be moved out of bounds within the <<section_cap_representable_check,representable range>> without changing the decoded bounds.
Because the upper bits of the address are used to calculate the top and base bound addresses, deriving an address that falls outside this representable range would change the resulting bounds.
All instructions that derive a capability with a new address check that the new address does not change the bounds.
If an address calculation moves the address outside the representable range such that the bounds would change, the {ctag} of the derived capability is set to zero, so that the capability becomes invalid.

Software can derive a capability with a new address using instructions such as <<SCADDR>>, <<CADD>> and <<CADDI>>.

NOTE: <<SCADDR>> writes back a derived capability with a new address field and, if the {ctag} was previously set to one, sets the {ctag} of the derived capability to one if the resulting capability still has the same bounds.
...
NOTE: Requiring hardware to explicitly detect top > 2^XLEN^ values and raise a dedicated exception would add hardware complexity in critical paths for no practical benefit, as such values cannot be produced by valid capability operations.
...
* <<sentry_cap>> types which can be used only for function calls or only for function returns for improved CFI.
...
Consequently, an integrity check should only change the result of an operation when it is performed on an invalid input capability (i.e. whose {ctag} is zero).
...
NOTE: A capability whose {ctag} is set to one is assumed to satisfy the <<section_cap_integrity,integrity>> checks, so this rule can only affect the result for an operand whose {ctag} is zero.
