...
==== Zca HINT instructions

For {cheri_base_ext_name} _all_ arithmetic HINT instructions require _rd_=`x0`.

This is because metadata is set to zero by arithmetic instuctions which do not affect the integer data value.

[NOTE]
====
For non-{cheri_base_ext_name} bases:

* C.SRLI, C.SRAI and C.SLLI all have code points which are HINTs if _shamt_=0.
* C.ADDI has a code point which is a hint if _imm_=0.

As a result, for {cheri_base_ext_name} bases:

 * No code points of C.ADDI, C.SRLI and C.SRAI are HINTs.
 * The code points of C.SLLI are only HINTs if _rd_=`x0`.
====
