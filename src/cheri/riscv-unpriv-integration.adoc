...
===== NULL Capability
A capability with all-zero metadata, its {ctag} set to zero, and an address of zero is referred to as the <<section_null_cap_encoding,NULL>> capability.
...
NOTE: In the {cheri_base_ext_name} ISA, this mandatory integrity check only applies to <<CBLD>>.
...
.All {cheri_base_ext_name} YLEN-bit User-Mode CSRs.
...
.Action taken on writing to YLEN-bit User-Mode CSRs
...
.YLEN-bit User-Mode CSRs storing code pointers or data pointers
[#CSR_exevectors_u]
