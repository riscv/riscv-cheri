...
|01           |STORE  |STORE-FP |*RVY-1*    |AMO      |OP     |LUI        |OP-32           |_reserved_
|10           |MADD   |MSUB     |NMSUB      |NMADD    |OP-FP  |OP-V       |*RVY-2*         |_reserved_
|11           |BRANCH |JALR     |_reserved_ |JAL      |SYSTEM |OP-VE      |*RVY-3*         |_reserved_
|===

Therefore:

. _custom-0_ is still available for custom encodings
. _custom-1/2/3_ are repurposed as *{cheri_base_ext_name}-1/2/3*

NOTE: Standard {cheri_base_ext_name} encodings will _never_ be placed in _custom-0_.

NOTE: Parts of *{cheri_base_ext_name}-1/2/3* may be returned to custom space in the future.
