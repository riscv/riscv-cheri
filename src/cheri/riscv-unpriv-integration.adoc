...
(draw-box "" [:box-first {:span 1}]) (draw-box "y0" [:box-related {:span 32}]) (draw-box "x0" [:box-last {:span 32 }])
(draw-box "" [:box-first {:span 1}]) (draw-box "y1" [:box-related {:span 32}]) (draw-box "x1" [:box-last {:span 32 }])
(draw-gap "Same layout for y2-y30 (extending x2-x30)" {:height 100})
(draw-box "" [:box-first {:span 1}]) (draw-box "y31" [:box-related {:span 32}]) (draw-box "x31" [:box-last {:span 32 }])
...
When referring to the extended registers, the `x` prefix is replaced with a `y`: i.e., the extended version of `x1` becomes `y1`.
The register can be referred to either way - as `y1` accessing all YLEN bits or as `y1` accessing only the address field (i.e., the lower XLEN bits).
For the ABI register names the `y` prefix is added, e.g., `ysp` to refer to all YLEN bits, and so the assembler can refer to either `sp` or `ysp` depending on the instruction operand.
...
The assembler pseudoinstruction to read a capability CSR `csrr cd, csr`, is encoded as `csrrs cd, csr, y0`.
...
// E.g., `lw t0, 16(ysp)` loads a word from memory, getting the address, bounds, and permissions from the `ysp` (capability stack pointer) register.
...
* <<LC>> reads YLEN bits of data from memory, and returns the associated {ctag}.
* <<SC>> writes YLEN bits of data to memory, and writes the associated {ctag}.

If <<c_perm>> is not granted then:

* <<LC>> reads YLEN bits from memory, but does not return the associated {ctag}, instead zero is returned.
* <<SC>> writes YLEN bits to memory, and writes zero to the associated {ctag}.
...
^1^ All load/store encodings are _reserved_ if `cs1=y0` (since dereferencing <<null-cap>> always faults).
