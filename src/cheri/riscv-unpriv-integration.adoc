...
The register can be referred to either way - as `c1` accessing all YLEN bits or as `x1` accessing only the address field (i.e., the lower XLEN bits).
For the ABI register names the `c` prefix is added, e.g., `csp` to refer to all YLEN bits, and so the assembler can refer to either `sp` or `csp`.
...
. Accesses to YLEN-bit CSRs and extended CSRs, using encodings other than CSRRW will:
.. Read YLEN bits
...
NOTE: An access fault is raised instead of a misaligned exceptions since these instructions cannot be emulated since there is one hidden {ctag} per YLEN-aligned memory region.
