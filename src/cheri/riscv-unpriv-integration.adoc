...
(or "sealing at ``CT``" when a particular output ``CT`` is meant).
Dually,
deriving a `CT=0` capability from a `CT≠0` capability is termed "unsealing"
(or "unsealing at ``CT``" when a particular input ``CT`` is meant).
...
if they require no particular authority to use to seal a capability.
Type 1 is considered ambiently available on a given platform
if <<section_rvy_sentry_insn_ext>> is available on that platform.
...
.2+|0x1       | 0x0    | <<app_cap_description, Default Encoding>> capabilities, without <<section_zylevels1>> features
              | 0x1   <| <<app_cap_description, Default Encoding>> capabilities, with <<section_zylevels1>> features
...
In the {cheri_base_ext_name} base ISA the only ways to create
a sealed capability are either through

* <<JALR_CHERI>>, which writes back a sentry capability to the link register

* careful bit manipulation and <<CBLD>>.
...
NOTE: In principle, the type used by <<SENTRY>>
may not be any of the type(s) used by <<section_Zysentry>>,
though that is not true of the capabilities of <<app_cap_description>>.
...
[#section_rvy_sunseal_insn_ext,reftext="{rvy_sunseal_insn_ext_name}"]
== "{rvy_sunseal_insn_ext_name}" Extension for Superset Unsealing

In the {cheri_base_ext_name} base ISA the only way to unseal a capability is
through careful, encoding-specific bit manipulation and use of <<CBLD>>.
The {rvy_sunseal_insn_ext_name} extension adds the <<YSUNSEAL>> instruction
to allow unsealing capabilities for which software already holds an unsealed
superset.
