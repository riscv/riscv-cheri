...
The <<ycfg>> register is used to identify which CHERI capability encoding is in use.
...
This CSR is always present in the machine mode CSR space;
if the core supports user mode, a `uycfg` alias exists in the appropriate space,
and, if the core supports supervisor mode, a `sycfg` alias exists, again in the appropriate space.

The base specification defines the following values for <<ycfg>>:
|===
|Version value|Encoding
|0|Reserved (unspecified capability encoding)
|1|<<app_cap_description>> capabilities, with <<section_zylevels1>> features
|2|<<app_cap_description>> capabilities, without <<section_zylevels1>> features
|3|Reserved for CHERIoT's (RV32-only) encoding
|===
