...
CAUTION: TODO: This section is missing any kind of rationale or usage for {cheri_levels_ext_name}, covering SL/EL/CL, or information about what happens if the extension is not implemented..

[#sl_perm,reftext="SL-permission"]
Store Level Permission (SL):: This is a `LVLBITS` wide field that allows limiting the propagation of capabilities using the following logic: capabilities with a <<section_cap_level>> less than the inverse of the authorizing capability's <<sl_perm>> will be stored with the validity tag cleared.
...
Unlike architectural permissions, CL can be reduced even if the capability is sealed.
...
//FIXME: need to allow CL/EL/SL to have fixed values to remove levels extension
...
In the base {cheri_base_ext_name}, the only such register is the program counter `pc` that is extended to the program counter capability (<<pcc>>). Extending `pc` ensures that the range of branches, jumps and linear execution for currently executing code can be restricted.
...
This validity tag is cleared whenever an invalid capability operation is performed. Such invalid operations include e.g. writing only the integer portion of the register or attempting to increase bounds or permissions.
...
//FIXME - need a forward reference to int mode here - I don't thin this section should be here at all though
...
* Pointer arithmetic (e.g. <<CADD>>, <<CADDI>>): if a pointer is updated then instead of using an integer ADD/ADDI then a capability version must be used which includes a check that the resulting pointer is valid for the current capability metadata. Invalid updated clear the validity tag.
...
|<<SH4ADD>>   |shift word and add to capability
|<<SH4ADD.UW>>|shift unsigned word and add to capability
...
* All jumps and branches check that a minimum sized instruction (2 or 4 bytes) is checked against the bounds the target PC.
...
//FIXME - this all needs to move to priv
