...
AP_ENC::
 Whether permissions in the <<AP-field>> are encoded using a _complete_ (`cmp`) representation allowing all valid combinations or an _incomplete_ (`incmp`) representation which doesn't represent all possible states and so uses fewer encoding bits.
REP_RANGE::
