...
If the hart supports RV32E/RV64E instead of RV32I/RV64I, {cheri_base_ext_name} the restrictions from <<rv32e>> apply and 16 general purpose registers instead of 32 -- but all other {cheri_base_ext_name} changes are identical so the remainder of this chaper only refers to extending RV32I/RV64I behavior.
...
{cheri_base_ext_name} extends all registers that hold addresses to `2*XLEN` bits (hereafter referred to as CLEN), adding metadata to protect its integrity, limit how it is manipulated, and control its use.
...
If the rules are followed then the capability tag will propagate through the instructions that modify, load or store the capability.
...
// NOTE: Instructions like <<SCBNDS>> can be reused to reduce the bounds of an existing capability while retaining the {ctag}, or they can be used to construct a new capability which is then validated using <<CBLD>>.
...
Due to the encoding of capability bounds, not all out-of-bounds pointers can be represented.
...
NOTE: {cheri_base_ext_name} implementations that use a different encoding scheme to the default (e.g., for accellerators or specific micro-controllers) may specify an alternative to the representable region check, and may never allow the address to be out of bounds.
...
NOTE: Future extensions are likely to add more types of sealed capabilities by extending the width of the CT-field.
...
+
Capabilities with a <<section_cap_level>> (see below) less than the inverse of the authorizing capability's <<sl_perm>> will be stored with a zero {ctag}.
+
...
NOTE: This specification only defines the architectural mechanics of this feature, for further information on how this can be used by software please refer to (#citation removed as it breaks the build#).
//cite:[cheri-v9-spec].
...
failing check raises a CHERI exception.
...
The {cheri_base_ext_name} ISA requires capabilities written to the <<pcc>> to follow these rules:

1. The {ctag} is set.
2. <<x_perm>> is granted.
3. The capability is not sealed.
4. No <<section_cap_integrity, integrity>> checks fail.

If any of these rules are not followed then execution will raise an exception.

NOTE: Future ISA extensions should respect these rules so that these bits do not need storing in all copies of the <<pcc>> in the implementation.
...
Any operations that modifies <<utidc>> raises an exception unless the <<asr_perm>> is set in the current <<pcc>>.
...
Every piece of code in the user space (and more privileged levels) can read the content of the <<utidc>>
...
No other exception paths are added by {cheri_base_ext_name}: in particular, capability manipulations do not except, but may set {ctag} of the resulting capability to zero if the operation is not permitted.
...
=== Instructions to Manipulate Capability Data
...
NOTE: An access fault is raised instead of a misaligned exceptions since these instructions cannot be emulated since there is one hidden {ctag} per CLEN-aligned memory region.
...
Load instructions, except for <<LC>>, always zero the {ctag} and metadata of the result register.

Store instructions, except for <<SC>>, always write zero to the {ctag} or tags associated with the memory locations that are written to.
