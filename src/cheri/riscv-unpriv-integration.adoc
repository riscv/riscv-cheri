...
It is stored separately and may be referred to as _out of band_ or _hidden_ and is hardware managed.
...
* The _length_ is XLEN-bits and is defined to be _top - base_, saturated to 2^XLEN^ -1
...
The ISA only allows the bounds (and permissions) of a capability to be reduced.
...
Bounds can be reduced using the <<SCBNDS>>/<<SCBNDSI>> instructions, which require the final bounds to be accurately encoded, setting the valid tag to zero if that is not possible due to the granularity constraints.
The requested bounds can be rounded up to the nearest encodable value above the requested bounds using <<SCBNDSR>> and <<CRAM>> can be used to determine the granularity which is possible to encode.
...
The CHERI concentrate encoding scheme shares the upper bits of the address with the bounds.
Therefore the address of a capability cannot be arbitrary modified without risking changing the meaning of the bounds.
...
Instructions which decode bounds will typically set the output valid tag to zero if the input is found to be malformed.
...
Sentry capabilities::
Capabilities with `CT=1` are sealed against modification.
...
+
#Tariq: I'm concerned by this note - as it doesn't seem to fit the definition of CBLD#
+
