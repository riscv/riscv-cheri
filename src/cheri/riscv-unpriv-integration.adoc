...
The memory representation of capabilities holds the address in the first `XLEN` bits of memory followed by the metadata.
...
If `{cd}` is not `0`, the link capability written to `{cd}` is also written as a <<sentry_cap>>.
...
(draw-column-headers {:height 30 :font-size 25 :labels (concat ["1" "0" "" "" "" "2*XLEN-1 (YLEN-1)"] (repeat 28 "") ["XLEN XLEN-1"] (repeat 30 "") ["0"])})
...
(draw-box "Capability tag" {:span 2 :borders {}}) (draw-box "Metadata" {:span 32 :borders {}}) (draw-box "Address" {:span 32 :borders {}})
----
