...
==== The Special Case of U-type Instructions and the <<AUIPC_CHERI>> Shift
...
(specifically, in the base specification, <<AUIPC_CHERI>>,
but future extensions may add instructions with similar semantics).
We refer to this value as **the AUIPC shift**.
...
might not be any of the type(s) used by <<section_rvy_sentry_jalr_ext>>,
