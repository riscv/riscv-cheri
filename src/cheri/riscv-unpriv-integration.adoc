...
=======
>>>>>>> 520fd0eb (add bounds and representability section to the intro, and move the bounds diagram there as requested)
=======
NOTE: Checking every byte of every executed instruction and byte of every data memory access is fundamental to the memory safety which CHERI provides.
 In a typical load/store unit, the expansion of the bounds from `cs1` and bounds checking is in parallel with the address calculation, the memory translation and/or the PMA/PMP checking.

>>>>>>> c5f06a00 (fix more feedback)
...
=======
CAUTION: #ARC suggest that this become a software error#
>>>>>>> c5f06a00 (fix more feedback)
...
=======
=======
>>>>>>> 3791fdc7 (clean up cap instruction page section, and some other improvements)
...
=======
No other exception paths are added by {cheri_base_ext_name}: in particular, capability manipulations do not except, but may clear the valid tag on the result capability if the operation is not permitted.
>>>>>>> 90adac13 (clean up cap instruction page section, and some other improvements)
>>>>>>> 3791fdc7 (clean up cap instruction page section, and some other improvements)
...
=======
They access CLEN-bits of data and also access the valid tag.
>>>>>>> 3791fdc7 (clean up cap instruction page section, and some other improvements)
...
=======
* Any memory access instruction that has a base address register (generally `rs1`) reads the full capability register (`cs1`) instead which is used to <<sec_cap_checks,check whether the access is permitted>>, taking an exception if it is not.
>>>>>>> 946b180c (add bounds and representability section to the intro, and move the bounds diagram there as requested)
* Any instruction that writes back an address (e.g. <<AUIPC_CHERI>>), writes a full capability register instead.
>>>>>>> 90adac13 (clean up cap instruction page section, and some other improvements)
* Whenever the address field of any capability (including the <<pcc>>) is modified, it is _always_ updated using <<SCADDR>> semantics.
** This includes adding an offset to the <<pcc>> from jumps and branches for both the target address and the link register.
...
** An exception is taken by the branch or jump on failure.
<<<<<<< HEAD
* <<JALR_CHERI>> copies _all_ of `cs1` into the <<pcc>>. If `cs1` is not suitable for execution then an exception is taken on the <<JALR_CHERI>> (see <<pcc>> for the definition).
>>>>>>> 3791fdc7 (clean up cap instruction page section, and some other improvements)
=======
* <<JALR_CHERI>> copies _all_ of `cs1` into the <<pcc>>. If `cs1` is not suitable for execution then an exception is taken on the <<JALR_CHERI>> (see <<pcc>> for the definition of suitability).
>>>>>>> c5f06a00 (fix more feedback)
...
<<<<<<< HEAD
<<<<<<< HEAD
 Therefore it is _not_ possible to speculatively jump somewhere without having _already_ completed the checks.
>>>>>>> 3791fdc7 (clean up cap instruction page section, and some other improvements)
=======
 Therefore it is _not_ possible to execute from a memory location without having _already_ checked that execution is permitted.
>>>>>>> 520fd0eb (add bounds and representability section to the intro, and move the bounds diagram there as requested)
=======
 Therefore it is _not_ possible to execute from a memory location without having _already_ checked that execution of that location is permitted.
>>>>>>> c5f06a00 (fix more feedback)
...
=======
Non-capability stores write zero to the valid tag associated with the memory locations that are naturally aligned to CLEN.
>>>>>>> c5f06a00 (fix more feedback)
