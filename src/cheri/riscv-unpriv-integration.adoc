...
+
When a WARL CSR is extended it is possible that writing a capability to it will be corrupted, as some address bits may be modified on writing, which in turn may effect the bounds decoding.
For this reason the WARL behavior is extended to include the full capability.
Any capability which cannot be written unmodified is considered to be _invalid_ and must be invalidated before writing.
+
Extended CSRs with WARL address fields need not be capable of holding all possible invalid capability values.
Prior to writing these CSRs, implementations may convert invalid capability values into some other invalid capability value that these CSRs _are_ capable of holding.
+
NOTE: The simplest representable invalid value is for the {ctag} to be set to zero if the capability cannot be written unmodified.
+
