...
As such, where a root capability value is specified, harts may specify implementation defined values instead which reduce the bounds or permissions as required.

NOTE: The hart specific root capabilities may be the maximum that the hart can ever have access to, or may allow access to capabilities in memory with greater bounds or permissions.
