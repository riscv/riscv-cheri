...
| AP_ENC      | cmp<suffix>/ incmp<suffix>| ps1             | Encoding scheme for the <<AP-field>>
4+| *Other parameters*
...
AP_ENC::
 Whether permissions in the <<AP-field>> are encoded using a _complete_ (`cmp`) representation allowing all valid combinations or an _incomplete_ (`incmp`) representation which does not represent all possible states and so uses fewer encoding bits.
REP_RANGE::
...
Performing integrity checks is generally optional.
The exception is for instructions that are defined to accept arbitrary input bit patterns and can write an output with the {ctag} set.
