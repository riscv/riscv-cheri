...
NOTE: {cheri_base_ext_name} implementations that use a different encoding scheme to <<app_cap_description>> (e.g., for accelerators or specific micro-controllers) may specify an alternative to the representable region check, and may never allow the address to be out of bounds.
