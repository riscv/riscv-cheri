...
* **Maximum exponent (ea or e<N>)**: If _all_ required values of the exponent can be encoded directly in the exponent field this uses `ea`, otherwise `e<N>` describes the exponent for a valid capability spanning the whole address space (e.g. `e15` for the CHERIoTv1 format).
This value `ea` for this parameter may be omitted as it is default value.
