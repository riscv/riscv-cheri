...
Other standard options to bases are also supported such as E (16-registers), Zfinx and endianness.
...
NOTE: Capability encoding formats are free to define their own bounds encoding scheme.
...
The bounds and representable region (or space) for <<rv32y_cap_description>>/<<rv64y_cap_description>> are illustrated in <<cap_bounds_map>>.
...
NOTE: The {cheri_base_ext_name} ISA does not make any use of non-zero `CT` values.
...
NOTE: This is the minimum set of permissions which a capability format must support. The in-memory format will depend on the capability encoding.
...
This is because this is the only currently defined instruction that sets the `{ctag}` of an invalid capability.
...
== "{rvy_topr_insn_ext_name}" Extension for Extracting the Top Bound
