...
In a typical load/store unit, the expansion of the bounds from `{creg_pref}s1` and bounds checking is in parallel with the address calculation, the memory translation and/or the PMA/PMP checking.
...
* <<SCBNDS>> sets the _base_ to `{creg_pref}s1.address`, and the _length_ to `rs2`. Set the {ctag} to zero if the bounds cannot be encoded exactly.
* <<SCBNDSI>> sets the _base_ to `{creg_pref}s1.address`, and the _length_ to the immediate value. Set the {ctag} to zero if the bounds cannot be encoded exactly.
* <<SCBNDSR>> sets the _base_ to `{creg_pref}s1.address`, and the _length_ to `rs2`. The bounds may be rounded up if they cannot be encoded exactly.
...
Permissions can only be removed using <<CLRPERM>> on valid capabilities, they cannot be added.
...
The assembler pseudoinstruction to read a capability CSR `csrr {creg_pref}d, csr`, is encoded as `csrrs {creg_pref}d, csr, {creg}0`.
...
To clarify whether an instruction accesses XLEN or YLEN bits of the register, the instruction listing uses `{creg_pref}d/{creg_pref}s1/{creg_pref}s2` for capability operands and `rd/rs1/rs2` for integer operands.
...
|<<SH1ADD_CHERI>> |Increment capability address by shifted register value, <<section_cap_representable_check,represented exactly>> check
|<<SH2ADD_CHERI>> |Increment capability address by shifted register value, <<section_cap_representable_check,represented exactly>> check
|<<SH3ADD_CHERI>> |Increment capability address by shifted register value, <<section_cap_representable_check,represented exactly>> check
|<<SH4ADD_CHERI>> |Increment capability address by shifted register value, <<section_cap_representable_check,represented exactly>> check
|<<SH1ADD_UW_CHERI>> |Increment capability address by shifted register value, <<section_cap_representable_check,represented exactly>> check
|<<SH2ADD_UW_CHERI>> |Increment capability address by shifted register value, <<section_cap_representable_check,represented exactly>> check
|<<SH3ADD_UW_CHERI>> |Increment capability address by shifted register value, <<section_cap_representable_check,represented exactly>> check
|<<SH4ADD_UW_CHERI>> |Increment capability address by shifted register value, <<section_cap_representable_check,represented exactly>> check
//|<<ADD_UW_CHERI>> |Increment capability address by unsigned word, <<section_cap_representable_check,represented exactly>> check
|=======================
...
|<<CBLD>>     |Set {creg_pref}d to {creg_pref}s2 with its tag set after checking that {creg_pref}s2 is a subset of {creg_pref}s1
|=======================
...
|<<SCSS>>   |Set result true if {creg_pref}s1 and {creg_pref}s1 tags match and {creg_pref}s2 bounds and permissions are a subset of {creg_pref}s1
...
All capability memory accesses check for <<c_perm>> in the authorizing capability in `{creg_pref}s1`.
...
All memory accesses, of any type, require permission from the authorizing capability in `{creg_pref}s1`.
...
* Any memory instruction that has `rs1` as a base address register reads the full capability register (`{creg_pref}s1`) instead.
...
All load and store instructions authorized by `{creg_pref}s1` raise exceptions if any of these checks fail:

* `{creg_pref}s1` must not be `{creg}0`^1^
* The {ctag} (`{creg_pref}s1.tag`) must be set
* `{creg_pref}s1` must be unsealed
ifdef::invalid_address_viol[]
* The virtual address in `{creg_pref}s1` must not be invalid if virtual memory is enabled.
endif::[]
* For loads, <<r_perm,read permission>> must be set in `{creg_pref}s1`
* For stores, <<w_perm,write permission>> must be set in `{creg_pref}s1`
* All <<section_cap_integrity,integrity>> checks on `{creg_pref}s1` must pass

^1^ All load/store encodings are _reserved_ if `{creg_pref}s1={creg}0` (since dereferencing <<null-cap>> always faults).
...
*Under {cheri_base_ext_name} _all_ loads and stores are authorized by `{creg_pref}s1`.*
...
|=======================
|Mnemonic                   |Description
|<<section_cheri_int_load_store_insns,LD, LW[U], LH[U], LB[U]>>|Integer loads (authorized by the capability in `{creg_pref}s1`)
|<<section_cheri_int_load_store_insns,SD, SW, SH, SB>>|Integer stores (authorized by the capability in `{creg_pref}s1`)
|=======================
...
* <<JALR_CHERI>> copies `{creg_pref}s1` into the <<pcc>>, and increments the address field with the offset.
  In this case, e.g., `new_{pcc} = {SCADDR}({creg_pref}s1, offset)`
...
|<<JAL_CHERI>>              |Jump to <<pcc>>+offset, link and seal capability to {creg_pref}d.
|<<JALR_CHERI>>             |Indirect jump, link and seal capability to {creg_pref}d.
