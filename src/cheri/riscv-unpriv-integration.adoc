...
Software can query the capability format by reading the <<ycfg>> CSR.
...
[#ycfg,reftext="__x__ycfg"]
===== CHERI Capability Encoding (__x__ycfg)
...
`sycfg` is always read-only, as is `vsycfg`, if it exists.
`uycfg` is a read-only alias of `sycfg`, if it exists, or of `mycfg` otherwise.
If the hypervisor extension is enabled, then `vsycfg` is WARL,
updatable from M-mode or HS-mode with <<pcc>> granting <<asr_perm>>.
