...
** The term _validating a capability_ means deriving a new capability with the {ctag} set to one.
* Invalid capabilities have the {ctag} set to zero.
** The term _invalidating a capability_ means deriving a new capability with the {ctag} set to zero.
...
NULL Capability::
An invalid capability with all-zero metadata and an address of zero is referred to as the <<section_null_cap_encoding,NULL>> capability.
...
.Capability states
[#table_cap_states,%autowidth,options=header,align="center",cols="2,1,4"]
|=======================
