...
NOTE: <<SCADDR>> updates the address field of a capability and, if the {ctag} was previously set, sets the {ctag} of the output if the updated capability is still valid.
...
* The affected operands change type from *x* to *c* to represent this. #ARC-COMMENT: What does this actually mean?  What type is x versus c?#
* Whenever the address field of any capability is modified, it is _always_ updated using <<SCADDR>> semantics. #ARC-COMMENT: Capabilities are not modified.  Capabilites are either input source operands or results from an instruction, they are not modified in place by an instruction.  This should be stated as a rule on results of operations.#
* Any instruction that writes back an address (e.g., <<AUIPC_CHERI>> or <<CSRRW_CHERI>>) to the destination register, writes a full capability register instead.
 #ARC-COMMENT: Doesn't this rule apply to all instructions that operate on a capability?  Or are there instructions that will always convert result to a non-capability?  The rule should be stated more cleanly - the instructions don't know if values are addresses, and non-capability values can be addresses.#
...
The rules above the following <<rv32,base ISA>> instructions listed in the following subsections, and also apply to instructions added by other extensions.

#ARC-COMMENT: Confusing to list these here as these are not base ISA instructions.#

#TODO: Add another chapter listing the changes to other extensions?#
...
Also note that the RVC encodings C.FSD, C.FLD, C.FSDSP, C.FLDSP are remapped to load/store capability data instead.#ARC-COMMENT: These are not part of the base.#
