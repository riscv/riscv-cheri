...
<<section_cheri_disable,{cheri_priv_hybrid_exts}>>,
...
All of <<pcc>>, <<mtvec_y>>, and <<mepc_y>> will clear their {ctag} if the
capability they would come to hold after an update
would be sealed or would not grant <<x_perm>>.

[NOTE]
=====
The <<section_priv_cheri,{cheri_priv_m_ext}>> extension requires that
<<mtvec_y>> and <<mepc_y>> nominally reset to <<root-rx-cap>> capabilities, and
...
and discusses some aspects of their interaction with `JALR` and `JAL` instructions.
...
outlined functions using a non-standard link register are still possible but
cannot change the interrupt status.
...
* A successfully retiring `JALR` or `JAL` whose `{cd}` is `ra` will
  seal its written-back return capability based on the value of the hart's `xstatus.xIE` bit
  immediately before this instruction's execution.
  If that bit is `0`,
...
(that is, addresses aligned to `YLEN/8` bytes).
...
avail themselves of a particular architectural affordance therein.
=====
...
Such memory block(s) are said to be _revocable_.
