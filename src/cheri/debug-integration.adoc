...
. Program buffer configured as specified in <<progbuf_access_ctags>>.
...
Implementations can choose to also support accessing full capabilities, including the {ctag}s, as part of the Access Register command by supporting `aarsize=7`.
...
The minimum Program Buffer configuration to support {ctag} access in integer registers is as follows:
...
[NOTE]
====
These requirements allow a debugger to read and write {ctag}s in integer registers without disturbing other registers.
...
[NOTE]
====
Accessing capabilities held in integer registers is a foundational operation that
...
Any integer registers that serve as temporaries during these operations
