...
If the platform incorporates <<section_rvy_sentry_jalr_ext>>,
the `DRET` instruction reads the full YLEN bits of the <<mepc_y>>/<<sepc_y>> register and unseals it prior to exception return,
...
`DRET` unseals the capability in question.)
