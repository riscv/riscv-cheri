...
** The debugger can also execute <<MODESW_INT>> to change the mode back to {cheri_int_mode_name}, which also affects the execution of the next instruction in the program buffer, updates the <<p_bit>> of this capability and controls which {cheri_mode_name} to enter on the next entry into debug mode.
