...
If {cheri_default_ext_name} is implemented, this <<root-rx-cap>> is further
specified thus:
...
Writes are WARL, so the debugger may confirm that its selection has applied.
...
{cheri_cap_mode_name} is controlled by the <<m_bit>> in the capability selected by <<drootcsel>> value 0.
...
<<root-rw-cap>> capability with its address set to that of the just-updated <<dddc>>.
