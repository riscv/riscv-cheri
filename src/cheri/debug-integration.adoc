...
When leaving debug mode, the value in <<dpc_y>> is unsealed if it is a {sentry_cap_lc} and is written to <<pcc>>.
...
When debug mode is exited, the hart's <<ddc>> is updated to
the capability stored in <<dddc>>. A debugger may write <<dddc>> to change the
hart's context.
...
== Debug-mode CSR Summary

The tables below show all Supervisor-mode CSRs added or modified by {cheri_base_ext_name}.
