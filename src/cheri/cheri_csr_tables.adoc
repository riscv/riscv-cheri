...
This section summarizes all YLEN CSR behavior.
...
Code pointers and data pointers are WARL CSRs that do not need to store full 64-bit addresses on RV64, and so may be subject to <<section_invalid_addr_conv>> on writing.

The tables below show all CSRs added or modified by {cheri_base_ext_name}.

.All {cheri_base_ext_name} CSRs.
[#all_capability_CSRs]
