...
{cheri_default_ext_name} adds the ability to dynamically change the base ISA between using capability pointers for all memory references ({cheri_base_ext_name}) and using standard RISC-V integer pointers (RV32I/RV64I).
...
The additional instructions added by implementing {cheri_default_ext_name} are shown in <<{rvyi_default_ext_name}_summary_table, {rvyi_default_ext_name}>>.
...
In {cheri_cap_mode_name} all memory accesses behave as defined by <<rv32y,{cheri_base_ext_name}>> and are authorized by the base capability register (`{cs1}`), from which the bounds and permissions are taken.
In {cheri_int_mode_name} all memory accesses must still be checked, but now they are authorized by the capability in <<ddc>> instead.

=== {cheri_default_ext_name} Branches and Jumps

In {cheri_int_mode_name} <<JALR_CHERI>> does _not_ copy the target capability register `{cs1}` into <<pcc>>, instead it updates the address of the <<pcc>> with the target value, using the semantics of the <<SCADDR>> instruction.
The only difference to RV32I/RV64I without {cheri_default_ext_name}, is that the target address is checked against the bounds of the <<pcc>>.

[#section_hybrid_ext_instructions]
=== {cheri_default_ext_name} Instructions

{cheri_default_ext_name} introduces new instructions to switch CHERI execution modes, and to query the current CHERI execution mode from a capability.
...
* Writing an extended CSR writes the address field (XLEN bits) only, and the full CSR is updated using semantics of the <<SCADDR>> instruction.
* Reading an extended CSR reads the address field (XLEN bits) only

Accesses to extended CSRs in {cheri_int_mode_name} must only access XLEN bits for compatibility, and so use semantics of the <<SCADDR>> instruction to determine the final written value.
