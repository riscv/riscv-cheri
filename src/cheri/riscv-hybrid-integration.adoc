...
That is, all memory accesses, including <<PREFETCH_W_CHERI>>, <<PREFETCH_R_CHERI>> and <<PREFETCH_I_CHERI>>, are implicitly authorized by <<ddc>> and only the memory address is sourced from `rs1`.
+
