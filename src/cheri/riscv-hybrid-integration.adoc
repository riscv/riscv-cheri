...
the capability format. Although always present, it only takes effect in code capabilities, i.e. when the <<x_perm>> is set.
...
For example, the `mtvec` CSR is extended to `mtvecc`, the `c` suffix indicating that it's a capability.
It is referred to as `mtvec` if any accessing the address or `mtvecc` if accessing the whole capability.

* In {cheri_cap_mode_name} <<CSRRW_CHERI>> _always_ read/writes the entire CLEN bits of all CLEN-width CSRs.
* In {cheri_int_mode_name} <<CSRRW_CHERI>> _only_ accesses XLEN bits of extended CSRs (for compatibility) and all CLEN-bits of CSRs added by CHERI extensions.
** <<CSRRW_CHERI>> in {cheri_int_mode_name} can only update the address field of `mtvec`, but it can write a full value into <<ddc>>.
** {cheri_cap_mode_name} always accesses `mtvecc`, never `mtvec`.
* when only writing XLEN bits to an extended CSR (e.g. `mtvec`), the CSR value is updated using <<SCADDR>> semantics.

NOTE: this section is tricky to write as it needs an address CSR to refer to - maybe it needs to move to priv? Or to depend on Zcmt? TBD
