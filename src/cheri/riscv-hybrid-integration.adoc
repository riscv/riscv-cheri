...
In {cheri_int_mode_name}, <<ddc>> is implicitly used. In {cheri_cap_mode_name}, the authorizing  capability is supplied as an explicit *y* operand register to the instruction.
...
//   For example, the encoding of an instruction like <<LW>> remains unchanged, but the mode indicates whether the capability authorizing the load is the register operand `ys1` ({cheri_cap_mode_name}).
...
| <<JALR_CHERI>> |{cheri_cap_mode_name}| Install `ys1` in the <<pcc>> and so update `pcc.M`
...
In {cheri_cap_mode_name} all memory accesses behave as defined by <<rv32y,{cheri_base_ext_name}>> and are authorised by the base register (`ys1`), from which the bounds and permissions are taken. For example:
...
NOTE: In {cheri_int_mode_name} JALR does _not_ copy the target capability register `ys1` into <<pcc>>, instead it updates the address of <<pcc>> with the target value.
...
// The output is written to an unprivileged *y* register, not <<pcc>>.
...
Therefore, such registers are extended to YLEN bits in {cheri_base_ext_name} and a `c` suffix is added when referring to the entire CSR instead of the only the address field.
...
* In {cheri_cap_mode_name} <<CSRRW_CHERI>> _always_ read/writes the entire YLEN bits of all YLEN-width CSRs, i.e. it accesses `jvtc`.
* In {cheri_int_mode_name} <<CSRRW_CHERI>> _only_ accesses XLEN bits of extended CSRs (e.g. `jvt`, for compatibility) and all YLEN-bits of CSRs added by CHERI extensions (e.g. <<ddc>>).
...
//
//* The register operand is a *y* register.
//* The full capability in the *y* register source is written to the CSR.
//** The capability may require modification before the final written value is
//determined (see <<extended_CSR_writing>>).
//* The full capability is written to destination *y* register.

When any YLEN-width CSR is accessed with another CSR instruction:
...
* When accessing an extended CSR:
    ...
    ** In {cheri_cap_mode_name}, YLEN bits are read from the CSR and written to an output *y* register.
* When accessing a CSR added by a CHERI extension:
** YLEN bits are read from the CSR and written to an output *y* register.
