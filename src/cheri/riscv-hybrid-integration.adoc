...
The mode is controlled by a new bit allocated in the capability metadata: the <<p_bit>>.
The <<p_bit>> in the <<pcc>> controls the current {cheri_mode_name}.

NOTE: The <<p_bit>> determines how pointers are represented, either as standard RISC-V _Integer Addresses_ or as CHERI _Capabilities_.
...
+
Instructions which are _modified_ on an {cheri_base_ext_name} architecture (see <<{rvy_i_mod_file_name}>>) _revert to their standard behavior_.
+
...
When executing <<CLRPERM>>, if <<x_perm>> is removed when the <<p_bit>> is set to one, and the capability encoding still permits representing the <<p_bit>>, then the <<p_bit>> must be set to zero.
...
[#m_bit_observe,reftext="Observing the {cheri_mode_name}"]
