...
NOTE: If an implementation needs to be able access custom encodings in spaces which have been reallocated, then it should disable CHERI, see <<section_cheri_disable>>.

The ability to choose between these two behaviors is referred to as switching between _{cheri_mode_name}s_.
The mode is controlled by a new bit allocated in the capability metadata (the <<p_bit>>) and the <<p_bit>> in the <<pcc>> controls the current {cheri_mode_name}.

NOTE: The <<p_bit>> determines how pointers are represented, either as standard RISC-V _Addresses_ or as CHERI _Capabilities_.
...
{cheri_int_mode_name}::
Pointers are interpreted as for the RV32[IE]/RV64[IE] base ISA.
+
