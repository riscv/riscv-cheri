...
NOTE: Implementations that support {cheri_default_ext_name} will typically boot into {cheri_int_mode_name} so that non-CHERI aware software can run unmodified even though the base architecture is {cheri_base_ext_name}.
