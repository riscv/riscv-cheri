...
Implementations that support {cheri_base_ext_name} and {cheri_default_ext_name} have full binary compatibility with existing RISC-V code.

Existing RISC-V code runs in a compartment, so all data memory and instruction memory accesses are bounded.

Key features in {cheri_default_ext_name} include a definition of a CHERI execution mode, a new unprivileged register, additional instructions and extensions to some existing CSRs enabling CHERI features.
...
All {cheri_base_ext_name} instructions and CSRs are available in in {cheri_default_ext_name}.
...
* {cheri_cap_mode_name}: this is executing the {cheri_base_ext_name} base ISA.
* {cheri_int_mode_name}: this is executing in RISC-V binary compatibility mode.

The mode is determined by a bit allocated into the metadata of the <<pcc>> called the <<m_bit>>.
The <<m_bit>> can be updated as shown in <<tab_cheri_hybrid_insn_summary>>:

.Capability hybrid instructions
...
All existing RISC-V extensions can be run in {cheri_int_mode_name}.

=== CHERI Memory Accesses

In {cheri_cap_mode_name} all memory accesses are authorised by `cs1`, from which the bounds and permissions are taken. For example:
...
At reset <<ddc>> is set to the <<infinite-cap>> capability so that _all_ memory accesses are permitted.
...
NOTE: JALR does _not_ install `cs1` in <<pcc>> in {cheri_int_mode_name}, the only difference is that it has a target check.

At reset <<pcc>> bounds and permissions are set to those of the <<infinite-cap>> capability so that _all_ instructions are in bounds.
