...
When in {cheri_int_mode_name}, there is a special rule for updating extended CSRs (e.g., <<utid>>/<<utidc>> or <<jvt>>/<<jvtc>>)
...
For this purpose the assembly syntax will refer to <<jvt>> or <<utid>>.
