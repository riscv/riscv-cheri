...
//   For example, the encoding of an instruction like <<LW>> remains unchanged, but the mode indicates whether the capability authorizing the load is the register operand `{creg_pref}s1` ({cheri_cap_mode_name}).
...
| <<JALR_CHERI>> |{cheri_cap_mode_name}| Install `{creg_pref}s1` in the <<pcc>> and so update `{pcc}.M`
...
In {cheri_cap_mode_name} all memory accesses behave as defined by <<rv32y,{cheri_base_ext_name}>> and are authorised by the base register (`{creg_pref}s1`), from which the bounds and permissions are taken. For example:
...
NOTE: In {cheri_int_mode_name} JALR does _not_ copy the target capability register `{creg_pref}s1` into <<pcc>>, instead it updates the address of <<pcc>> with the target value.
