...
XLEN bits of extended CLEN-wide CSRs are written when executing
<<CSRRWI_CHERI>>, <<CSRRC_CHERI>>, <<CSRRS_CHERI>>, <<CSRRCI_CHERI>> or <<CSRRSI_CHERI>> regardless of the
CHERI execution mode. When using <<CSRRW_CHERI>>, CLEN bits are written when the
...
.Action taken on writing to CLEN-wide CSRs
[#new_cap_CSR_writing]
[width="100%",options=header,cols="1,2,2"]
|==============================================================================
...
XLEN bits of CLEN-wide CSRs added in {cheri_default_ext_name} are
written when executing <<CSRRWI_CHERI>>, <<CSRRC_CHERI>>, <<CSRRS_CHERI>>, <<CSRRCI_CHERI>> or
<<CSRRSI_CHERI>> regardless of the CHERI execution mode. CLEN bits are always written
...
xref:all_capability_CSRs[xrefstyle=short] shows all CLEN-wide CSRs.
