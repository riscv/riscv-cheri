[#rv32y_cheriot_cap_description, reftext="{rvy32_cheriot_base_name} (CHERIoT V1)"]
== The {rvy32_cheriot_base_name} Capability Base Extension for {cheri_base32_ext_name}, (CHERIoT V1)
...
NOTE: This encoding format assumes support for M-mode only. Future versions of this encoding format may support additional privilege modes.
...
.Capability encoding for {rvy32_cheriot_base_name}
...
.{rvy32_cheriot_base_name} parameter summary
[#rvy32_cheriot_base_name_param_summary,width="100%",options=header,]
|==============================================================================
...
.{rvy32_cheriot_base_name} extension summary
[#rvy32_cheriot_base_name_ext_summary,width="100%",options=header,]
|==============================================================================
| Extension                    | Comment
...
.{rvy32_cheriot_base_name} Feature summary
[#rvy32_cheriot_base_name_feature_summary,width="100%",options=header,]
|==============================================================================
| Feature                      | Comment
...
The permission encoding space is split into quadrants using AP[3:2].
Each quadrant may include some fixed permissions (indicated with ✔) and some dependent permissions encoded using AP[2:0].
Quadrant 0 is used to encode permissions that authorise sealing and also a single software-defined permission, SDP0. It can also encode 'no permissions'.
...
|===
4+| AP[4:0] 12+| Decoded Permissions
| AP[4:3] | AP[2] | AP[1] | AP[0] |  R   |  W   |  C   |  SL  |  LM  |  LG  |  X  | ASR  |  SDP0  |  SE  |  US  | Notes

| 00     | SDP0   | SE   | US   |      |      |      |      |      |      |     |      | AP[2] | AP[1] | AP[0] | Sealing
| 01     | ASR    | LM   | LG   |  ✔   |      |  ✔   |      | AP[1] | AP[0] |  ✔  | AP[2] |      |      |      | Executable
| 10     | 0      | 0    | 0    |      |  ✔   |  ✔   |      |      |      |     |      |      |      |      | Cap WO
| 10     | 0      | R    | W    | AP[1] | AP[0] |      |      |      |      |     |      |      |      |      | Data RW (must have R or W)
| 10     | 1      | LM   | LG   |  ✔   |      |  ✔   |      | AP[1] | AP[0] |     |      |      |      |      | Cap RO
| 11     | SL     | LM   | LG   |  ✔   |  ✔   |  ✔   | AP[2] | AP[1] | AP[0] |     |      |      |      |      | Cap RW
|===

For example, if AP[4:3] = 01 and AP[2:0] = 101 then the decoded permissions are R, C, X, LG, and ASR.
...
This means there are three <<root-cap>> capabilities from which all other capabilities can be derived using <<CLRPERM>>. These are shown in <<cheriot_root_caps>>.
The privileged specification must define where these root capabilities are found on system reset.

[#cheriot_root_caps,width="100%",options=header,cols="1,1,2",align="center",reftext="CHERIoT root capabilities"]
|===
| Root name  | AP[4:0] | Permissions

| Sealing    | 00111  | SDP0, SE, US
| Executable | 01111  | X, R, C, LM, LG, ASR
| RW Root    | 11111  | W, R, C, LM, LG, SL
|===
...
executable format (AP[4:3] = 01).
2. Otherwise, if the permissions include R, W and C then encode SL, LM and LG
using the Cap RW format (AP[4:3] = 11).
3. Otherwise, if the permissions include R and C then encode LM and LG using
the Cap RO format (AP[4:2] = 101).
4. Otherwise, if the permissions include W and C then encode using the Cap WO format (AP[4:0] = 10000).
5. Otherwise, if the permissions include R or W then encode using the Data RW format (AP[4:0] = 100RW).
6. Otherwise, encode SDP0, SE and US using the sealing format (AP[4:3] = 00).
...
Note that sealing type 8 is not used in order to allow CT value of zero to represent unsealed for both executable and non-executable capabilities.

In addition to the ordinary <<sentry_cap>> type this encoding defines four types of sentry for enforcing structured control flow and atomically changing the value of `mstatus.MIE` for the current mode, X, during a <<JALR_CHERI>>.
...
Due to the limited representable range of this encoding there can be a problem when using <<AUIPC_CHERI>> to form addresses relative to <<pcc>>. For example, the following sequence might used to perform a pc relative relocation by an offset [imm20|imm12]:
...
The encoding for the root capabilities have maximum bounds. The AP field depends on which root capability is being represented as per <<cheriot_root_caps>>.
