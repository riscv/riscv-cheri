...
Teh following tables summarize the mappings.
...
The jump vector table CSR <<jvt_cheri>> has a capability alias <<jvtc>> so that it can only be configured to point to accessible memory. All accesses to the jump table are checked against <<jvtc>> in {cheri_cap_mode_name}, and against <<pcc>> bounds in {cheri_int_mode_name}. This allows the jump table to be accessed
...
NOTE: Zcmt defines that the fetch from the jump table is from instruction memory.
 The overall instruction executed is effectively 48-bit, with 16-bits from <<CM_JALT_CHERI>>/<<CM_JT_CHERI>>, the other 32-bits (for RV32) from the table.
...
<<jvtc>> extends The Zcmt <<jvt_cheri>> to be CLEN-bits, as shown in xref:ucsrnames-renamed[xrefstyle=short].
...
** <<CM_JT_CHERI>> executes as <<JAL_CHERI,J>> or <<AUIPC_CHERI>>+<<JALR_CHERI,JR>>
** <<CM_JALT_CHERI>> executes as <<JAL_CHERI>> or <<AUIPC_CHERI>>+<<JALR_CHERI>>
