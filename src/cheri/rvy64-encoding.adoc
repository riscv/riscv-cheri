...
<<section_zylevels1>>::
Bits 6 and 7 of the `AP` field are allocated, otherwise they are reserved and set to one.
+
The <<zylevels1_gl_perm,GL>> flag is allocated, otherwise it is reserved and must be zero.
...
| AP_ENC      | ps1                  | Encoding scheme for the <<AP-field>>
| AUIPC_SHIFT | 12                   | Shift distance of the immediate in AUIPC
| REP_REGION  | rc1                  | Whether out of bounds addresses are representable
|==============================================================================

NOTE: All parameters match the default values from <<cap_encoding_param_default_summary>>.
...
==== Architectural Permissions (AP) Encoding - `ps1`

The permissions field is up to {cap_rv64_perms_width} bits wide and is encoded using one bit per architectural permission as shown in xref:cap_perms_encoding64[xrefstyle=short].
