...
As shown in xref:section_cap_bounds_format[xrefstyle=short] below, the upper bits of the capability address are used to form the upper bits of the bounds.
Sometimes the bounds need to be adjusted up or down relative to the capability address, because the memory bound and the capability address cross alignment boundaries (address 0x20000 in <<capformat_correction_illustration>>).

[#capformat_correction_illustration]
.Example capability bounds that use the correction. The base address bound is an s-aligned region below the address, meaning the upper bits (above the mantissa) of the base are smaller than the upper bits of the address. Therefore c~b~=-1. The address and top address bound are in the same region, so c~t~=0.
