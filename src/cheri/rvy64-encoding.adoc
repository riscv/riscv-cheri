...
As described in xref:section_cap_bounds_format[xrefstyle=short] below, the `_base_` and `_top_` addresses are computed by substituting `B` and `T` for the upper `MW` bits of the capability address, leaving the address's own 2^E+MW^-aligned block unchanged.
This substitution is only correct if `B`/`T` and the capability address lie in the same 2^E+MW^-aligned block.
When they do not (as with the base in <<capformat_correction_illustration>>), the correction factors c~t~ and c~b~ shift the substituted block up or down by one so that the computed bound falls in the block that actually contains `_base_` or `_top_`.

`R` marks the lower boundary of the <<section_cap_representable_check,representable range>> (see <<cap_bounds_map>>).
Comparing `A` (the bit range of the capability address which gives the offset within the 2^E+MW^-aligned block), `B` and `T` against `R` tells us whether each lies in the 2^E+MW^-aligned block containing `R` (`space~L~` in <<cap_bounds_map>>) or the block above it (`space~U~`).
...
<<capformat_correction_cases>> shows the six reachable combinations of c~t~ and c~b~ that arise from this table.

[#capformat_correction_cases]
.The reachable combinations of c~t~ and c~b~.
...
The _base_ and _top_ addresses are derived from the address by
substituting `a[E + MW - 1:E]` with `B` and `T` respectively and clearing the
lower `E` bits, then applying the corrections _c~b~_ and _c~t~_ from
xref:section_cap_bounds_correction_factors[xrefstyle=short] above.
...
The compressed bounds encoding allows the address to roam over the _representable_ range, `space~R~`, while maintaining the original bounds  (see xref:section_cap_bounds_correction_factors[xrefstyle=short] above for how
`R` and the corrections are calculated).
The overall effect is that the  address can roam at least `2^E+MW^/4` bytes below the base address and at least  `2^E+MW^/4` bytes above the top address while still allowing the bounds to be
correctly decoded.
...
That is, invert the most significant bit of _top_ if the decoded top and base are one address space (2^XLEN^ bytes) or more apart, and E is too small to allow this.
...
base bound is entirely derived from `B` and `E` and therefore `base == base'`.
...
|==============================================================================
| input_t[XLEN:XLEN-1]  | base[XLEN-1]  | output_t[XLEN:XLEN-1]
|      00                 |      0      | 00
|      01                 |      0      | 01
|      10                 |      0      | 00
|      11                 |      0      | 01
...
* `output_t[XLEN] = {input_t[XLEN-1], base[XLEN-1]} == 0b01`.
