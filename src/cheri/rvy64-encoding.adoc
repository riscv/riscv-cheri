...
| All RVY versions of other standard extensions | Compatible if the extension is compatible with RV64I
|==============================================================================
...
The capabilities of this chapter have sufficient <<section_rep_check_concept,representability>> such that sequences used to form XLEN-bit addresses (such as <<AUIPC_CHERI>>+<<JALR_CHERI>>) will not clear the {ctag}.
...
| <<m_bit,M>>^1^   | {INT_MODE_VALUE} | CHERI execution mode
| CT            | zero  | Unsealed
| EF            | zero  | Internal exponent format
| L~8~^2^       | zero  | Top address reconstruction bit
| T             | zeros | Top address bits
...
| B             | zeros | Base address bits
| B~E~          | zeros | Exponent bits
| Address       | any^3^ | Capability address
| Reserved      | zeros | All reserved fields
|==============================================================================

^1^ Only present if `AP_MBit=1` and {cheri_default_ext_name} is implemented.

^2^ Only present if `enableL8` is set.

^3^If an infinite capability is used as a constant in either hardware or software, then the address field will typically be set to zero.
