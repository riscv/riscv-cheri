...
As described in xref:section_cap_bounds_format[xrefstyle=short] below, the base, _b_, and top, _t_, addresses are computed by substituting `B` and `T` for the upper `MW` bits of the capability address, leaving the address's own 2^E+MW^-aligned block unchanged.
This substitution alone is only correct if `B`/`T` and the capability address lie in the same 2^E+MW^-aligned block.
When they do not (address 0x20000 in <<capformat_correction_illustration>>), the correction factors c~t~ and c~b~ shift the substituted block up or down by one so that the computed bound falls in the block that actually contains _b_ or _t_.
...
^1^ `B < R` requires `B[MW-1:MW-2] = 0`, which makes `R[MW-1:MW-2] = 3`.
Since `T[MW-1:MW-2] = B[MW-1:MW-2] + LCout + LMSB` (xref:section_cap_bounds_decoding[xrefstyle=short]) is at most 2 in that case, `T < R` always holds when `B < R`, ruling out c~t~=-1,c~b~=0 and c~t~=0,c~b~=+1.
