...
In this case the relevant _correction factor_ is zero (_c~b~_ for the _base_ and _c~t~_ for the _top_).
...
Comparing `A` (the bit range of the capability address which gives the offset within the 2^E+MW^-aligned block), `B` and `T` against `R` tells us whether each lies in the 2^E+MW^-aligned block containing `R` (`space~L~` in <<cap_bounds_map>>), the block above it (`space~U~`) or the block below (not shown in <<capformat_correction_illustration>>, all possible combinations are shown in <<capformat_correction_cases>>).
