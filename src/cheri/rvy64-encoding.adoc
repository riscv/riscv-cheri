...
== The {rv64y_uni_base_name} Capability Base for RV64

IMPORTANT: #ARC-NOTE: We originally had these two sections as separate chapters since they have a forward reference to the levels extension to define how those bits are allocated.#
#Is this forward reference acceptable or do we need to push the definition into the Zylevels chapter?#
...
| <<p_bit,P>>^1^   | {INT_MODE_VALUE_WORD} | {cheri_mode_name}
| CT            | zero  | Unsealed
| EF            | zero  | Internal exponent format
| L~8~^2^       | zero  | Top address reconstruction bit
| T             | zeros | Top address bits
