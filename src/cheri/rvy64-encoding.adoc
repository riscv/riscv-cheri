...
The encoding diagram above shows the format of capabilities in the case where all {cheri_base_ext_name} (Zy*) extension listed in this manual are supported.
...
[NOTE]
=====
The permission encodings of <<rvy32_cap_description>>/<<rvy64_cap_description>> do not provide mappings for
...
.Mantissa width (MW) values depending on capability encoding
[#mantissa_bit_width,options=header,align="center",width="55%"]
|==============================================================================
^| Encoding   ^| MW
^| {rvy32_uni_base_name} ^| {cap_rv32_mw_width}
^| {rvy64_uni_base_name} ^| {cap_rv64_mw_width}
|==============================================================================
...
.Exponent widths and CAP_MAX_E depending on capability encoding
[#exp_bit_width,options=header,align="center",width="55%"]
|==============================================================================
^| XLEN                 ^| EW ^| CAP_MAX_E
^| {rvy32_uni_base_name} ^| {cap_rv32_exp_width}  ^| {cap_rv32_exp_max}
^| {rvy64_uni_base_name} ^| {cap_rv64_exp_width}  ^| {cap_rv64_exp_max}
|==============================================================================
...
(as well as L~8~ for {rvy32_uni_base_name}).
...
{cheri_int_mode_name} for compatibility with standard RISC-V code.
Therefore, the <<m_bit>> is set to {INT_MODE_VALUE} in a separate M field which is _not shown_ in the table above.
