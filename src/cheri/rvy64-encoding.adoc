...
<<section_zylevels1>>::
If <<section_zylevels1>> is available, bits 6 and 7 of the `AP` field are allocated, otherwise they must always be set to one for any valid capability.
...
ifndef::expanded_ap_field[]
* **Permission encoding (ps)**: The permissions are encoded compressed representation allowing useful combinations to be represented.
endif::[]

==== Capability Encoding Summary

.{rv64y_uni_base_name} parameter summary
