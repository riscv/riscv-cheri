...
| AP_ENC      | full1                | Encoding scheme for the <<AP-field>> - all combinations are representable
| REP_RANGE   | rc1                  | Whether out of bounds addresses are representable
|==============================================================================

NOTE: All values in <<rvy64_uni_base_name_param_summary>> match the default values specified in <<cap_encoding_param_default_summary>>.
...
possible to encode any arbitrary combination of _base_ and _top_ addresses. An
...
The variables related to the bounds decoding are shown in <<rvy64_uni_base_name_bndsenc_param_summary>>.
...
NOTE: The address and bounds must be representable in valid capabilities (see xref:section_cap_malformed[xrefstyle=short]).
...
The _base_ and _top_ bound addresses are derived from the capability address `a` by
substituting `a[E + MW - 1:E]` with `B` and `T` respectively and clearing the
lower `E` bits, then applying the <<section_cap_bounds_correction_factors>> _c~b~_ and _c~t~_ which are described below.
...
.Decoding of the XLEN-bit _base_ bound address
[#base_bound_dec]
...
The compressed bounds encoding allows the address to roam over the _representable range_, (`range~R~` in <<cap_bounds_map>> and <<capformat_correction_illustration>>), while maintaining the original bounds.
...
`R` marks the lower boundary of the <<section_cap_representable_check,representable range>> (shown as r~base~ in <<capformat_correction_illustration>>).

If `B`, `T` or `A` is less than `R`, it is inferred that they lie in the space above the s-alignment boundary straddled by the representable region, labeled space~U~ in xref:capformat_correction_illustration[xrefstyle=short].

`A` and `R` are calculated as follows.

[source,subs="quotes"]
----
A[MW-1:0] = a[E + MW - 1:E]; // offset into s-aligned region from capability address field
R[MW-1:0] = B - 2^MW-2^;        // r-base which is 1/4 of the 2^MW aligned region below B
----

NOTE: The comparisons in xref:cap_encoding_corrections[xrefstyle=short] are _unsigned_.

The correction factors _c~b~_ and _c~t~_ are calculated as follows.
...
^1^ Since `B[MW-1:MW-2] = 0` in these cases, `R[MW-1:MW-2] = 3` as the value has wrapped. +
As a result `T[MW-1:MW-2]` is at most 2 if LMSB and LCout are both 1 (according to this <<formula_top_bits_of_t,formula>>). +
Therefore `T < R` always holds when `B < R`, ruling out _c~t~_=-1,_c~b~_=0 and _c~t~_=0,_c~b~_=+1.

<<capformat_correction_cases>> shows the six reachable combinations of _c~t~_ and _c~b~_ that arise from this table.

[#capformat_correction_cases]
.The reachable combinations of _c~t~_ and _c~b~_.
image::../cheri/img/capformat-correction-cases.png[width=80%,align=center]

====== Correction Factor Summary

In summary, for either bound:

* If the bound is below the _s_-aligned boundary and the capability address is above the _s_-aligned boundary, then set the correction factor to -1 to correct the bound downwards.
* If the bound and the capability address are within the same _s_-aligned boundary, then set the correction factor to 0 so the bound is unchanged.
...
A capability has <<infinite_bounds,infinite bounds>> if its bounds cover the entire address space
such that _base_=0 and _top_{ge}2^XLEN^,
i.e., _top_ is an XLEN + 1 bit value. However, _base_ is an
XLEN-bit value and this width mismatch introduces additional complications
when decoding, so the following condition is required to correct _top_ for
capabilities whose <<section_cap_representable_check>> wraps the edge of the address
space:

[source]
----
if ( (E < (CAP_MAX_E - 1)) && (top[XLEN: XLEN - 1] - {0b0, base[XLEN - 1]}) >= 0b10)
    top[XLEN] = !top[XLEN]
----
NOTE: The comparison is _unsigned_.

That is, invert the most significant bit of _top_ if the decoded _top_ and _base_ are one address space (2^XLEN^ bytes) or more apart, and E is too small to allow this.

NOTE: A capability has <<infinite_bounds,infinite bounds>> if `E=CAP_MAX_E` and it is not
...
* A source capability with address _a_, metadata _m_ that decodes to give the bounds _base_ and _top_.
* A derived capability with arbitrary address _a'_ with the same metadata _m_ that decodes to give the bounds _base'_ and _top'_.

The address _a'_ is within the source capability's _representable range_ if `base == base' && top == top'`.

If the address _a'_ is outside the _representable range_, then the derived capability is invalidated.

==== Practical Information

An artifact of the bounds encoding is that if the new address
causes `top != top'`, then it is also the case that `base != base'`.

The converse is also true, if `base != base'` then `top != top'`.

Therefore, for representable range checking, it is acceptable
to either check `top == top'` or `base == base'`.

The _top_ and _base_ capability bounds are formed of two or three sections:
...
|==============================================================================
| input_top[XLEN:XLEN-1]  | base[XLEN-1]  | output_top[XLEN:XLEN-1]
|      00                 |      0      | 00
|      01                 |      0      | 01
|      10                 |      0      | 00
|      11                 |      0      | 01
...
that `output_top[XLEN]` does not depend on `input_top[XLEN]` as:

* `output_top[XLEN] = {input_top[XLEN-1], base[XLEN-1]} == 0b01`.

This leads to the conclusions:

* If `top[XLEN-1] == top'[XLEN-1]` and `base[XLEN-1] == base'[XLEN-1]`, then it is guaranteed `top[XLEN] == top'[XLEN]`.
* If `top[XLEN-1] != top'[XLEN-1]` or `base[XLEN-1] != base'[XLEN-1]`, then the representable check will fail regardless of checking `top[XLEN] == top'[XLEN]`.

Therefore, for the purpose of representable range checking, it is not
required to check that `top[XLEN]==top'[XLEN]`.

Given that `top[XLEN]` is not part of the representable range check:

* when `E == CAP_MAX_E - 2`, `top[XLEN-1:E] == T[MW-1:0]` and `base[XLEN-1:E] == B[MW-1:0]`.
...
though the bounds are only <<infinite_bounds,infinite>> if E = CAP_MAX_E_
...
This encoding is for an _Infinite Capability_ value, which grants all permissions, has <<infinite_bounds,infinite bounds>> and is both a <<root-rx-cap>> and a <<root-rw-cap>> capability.
...
^2^If an infinite capability is used as a constant in either hardware or software, then the address field will typically be set to zero.
 If the address field is non-zero then it is still referred to as the _Infinite Capability_, and it still has the authority to authorize all memory accesses.

NOTE: Permissions added by extensions (such as those of <<section_zylevels1>>) are presumed present in the _Infinite Capability_, which may involve redefining the AP_MAX value.
