...
Once all of the permission rules in <<base_clrperm_rule_summary>> and <<zylevels1_clrperm_table>> have been applied, and also the <<p_bit>> validity depends upon <<x_perm>> (see <<sec_p_bit_hybrid_rule>>) the total valid combinations of permissions totals 180.

Encoding 180 valid states in 9 bits, which allows for 512 states, is inefficient.

Future extensions or capability encoding formats may address this by implementing a more compact <<AP-field>> and <<p_bit>> encoding that is fully _compatible for all software_ to increase the number of reserved bits.

Such as encoding would have observable differences in the in-memory representation of capabilities.
...
----
NOTE: With `enableL8=1`, the `EF=0` format has L~8~ align with the implied `LMSB` with `EF=1` and `E=0`.
