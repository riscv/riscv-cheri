...
NOTE: AP[6:2] encodes <<r_perm,R>>/<<w_perm,W>>/<<c_perm,C>>/<<lm_perm,LM>> permissions; AP[1] is <<asr_perm,ASR>>; AP[0] is <<x_perm,X>>.
...
When <<section_cheri_hybrid_ext>> is supported, capabilities include the <<p_bit>>. Otherwise, that field is _reserved_.

<<section_zylevels1>>::
When <<section_zylevels1>> is supported, capabilities include the <<zylevels1_gl_flag,GL>> flag. Otherwise, that field is _reserved_.
+
The AP-field is also sized to include two additional permissions which are available as _reserved_ states if <<section_zylevels1>> is not supported.
...
==== Architectural Permissions (AP) Encoding - `full1_rv64ya_1`

The <<AP-field>> encodes all permissions in a compact format.
<<x_perm>> and <<asr_perm>> each occupy a dedicated single bit (AP[0] and AP[1] respectively).
The remaining permissions (R, W, C, LM) have a restricted set of valid combinations due to the dependency rules in <<base_clrperm_rule_summary>>; they are encoded together in AP[6:2] to exploit these constraints.

NOTE: AP[1] encodes <<asr_perm>> and AP[0] encodes <<x_perm>>.
The combination ASR=1, X=0 is _reserved_ following <<perm_req:base:asr:x,base-3>>.

NOTE: <<w_perm>> is valid in all combinations with R, C, LM in the base ISA, but <<section_zylevels1>> restricts its usage.
 AP[6:2] is allocated with 5 bits to leave space for <<section_zylevels1>> states without requiring a definition change.
 Future extensions may redefine the _reserved_ states if they are incompatible with <<section_zylevels1>>.

The encoding of AP[6:2] is shown in <<rvy64_uni_base_name_ap_fmt>>.

.{rv64y_uni_base_name} AP[6:2] encoding (R/W/C/LM permissions)
[#rvy64_uni_base_name_ap_fmt,width="100%",options=header,cols="^2,^1,^1,^1,^1,4"]
|==============================================================================
| AP[6:2] | R | W | C | LM | Comment
6+| *AP[6:5]=00: AP[4]=R, AP[3]=W, AP[2]=C*
| *00* 000 |   |   |   |    | *No permissions* (NULL capability)
| *00* 001 5+|  _reserved_ due to <<perm_req:base:c:r-or-w,base-1>>
| *00* 010 |   | W |   |    |
| *00* 011 |   | W | C |    |
| *00* 100 | R |   |   |    |
| *00* 101 | R |   | C |    |
| *00* 110 | R | W |   |    |
| *00* 111 | R | W | C |    |
6+| *AP[6:4]=010: LM=1, AP[3]=W*
6+| *LM implies C and R*
| *010* 00 5+|  _reserved_ due to <<perm_req:base:lm:c-and-r,base-2>>
| *010* 01 | R |   | C | LM |
| *010* 10 5+|  _reserved_ due to <<perm_req:base:lm:c-and-r,base-2>>
| *010* 11 | R | W | C | LM | *Maximum base permissions*
6+| *Reserved for <<section_zylevels1>> (AP[6:4]=011, 100)*
| *011* xx 5+|  _reserved_ for <<section_zylevels1>>
| *100* xx 5+|  _reserved_ for <<section_zylevels1>>
6+| *Reserved for future extensions*
| 10101 5+|  _reserved_, possibly for future sealing permission
| 10110 5+|  _reserved_, possibly for future unsealing permission
| 10111 5+|  _reserved_, possibly for future sealing and unsealing permission
| 11xxx 5+|  _reserved_ for other future use
|==============================================================================

AP_MAX = 0x2F (AP[6:2]=01011, AP[1]=1, AP[0]=1: R, W, C, LM, ASR, X all granted)
...
* The encoded value `1` indicates an _sealed_ type which can unsealed if in `{cs1}` passed to <<JALR_CHERI>>, and is used to _seal_ the capability written to `{cd}`.
...
If EF = 1:
    ...
    E               = CAP_MAX_E - ( (XLEN == 32) ? { L8, TE, BE } : { TE, BE } )
    T[EW / 2 - 1:0] = 0
    B[EW / 2 - 1:0] = 0
...
| EF=0            | address[XLEN-1:E + MW] + ct | T[MW - 1:0] | {E{0b0}}
...
The exponent value causes its bounds cover the entire address space, but no permissions are granted and so it cannot be successfully be used to grant authorization.
