...
| enableL8  | 0                    | Whether the encoding format includes the L8 bit
...
When EF=0, the exponent `E=CAP_MAX_E - ( (XLEN == 32) ? { L8, TE, BE } : { TE, BE } )`,
...
If `enableL8` is true, the `L8` bit is available in the capability encoding format for extra precision when `EF=1`.
