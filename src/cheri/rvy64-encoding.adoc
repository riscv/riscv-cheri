...
AP_CMP, ASR and X are concatenated to form the <<AP-field>>.
...
| AP_ENC      | cmp1_rv64ya_1        | Encoding scheme for the <<AP-field>> - all combinations are representable
...
==== Architectural Permissions (AP) Encoding - `cmp1_rv64ya_1`

The permissions field represents all possible combinations of permissions in an optimized format.

The following permissions each have a single bit allocated to them:

* <<x_perm>>
* <<asr_perm>>

The combination of <<x_perm>>=0 and <<asr_perm>>=1 is _reserved_ following <<perm_req:base:asr:x,base-3>>.

The remaining permissions have a restricted set of valid combinations and so are encoded into a single field called AP_CMP.

NOTE: Even though <<w_perm>> is valid in all combinations with the base set of permissions, <<section_zylevels1>> restricts its usage and so it is included in AP_CMP.
 Additionally, AP_CMP is allocated to have 5-bits to allow space for <<section_zylevels1>> without changing the definition.
 Future extensions may extend it further if necessary or may redefine the _reserved_ states if they are incompatible with <<section_zylevels1>>.

.{rv64y_uni_base_name} AP_CMP (compressed architectural permissions) field
[#rvy64_uni_base_name_ap_cmp_fmt,width="100%",options=header,cols="1,1,1,1,1,5"]
|==============================================================================
| Encoding | C | W | R | LM | Comment
6+| *C=Encoding[2], W=Encoding[1], R=Encoding[0], LM=0*
| 00000    |   |   |   |    | *No permissions*
| 00001    |   |   | R |    |
| 00010    |   | W |   |    |
| 00011    |   | W | R |    |
| 00100  5+|                  _reserved_ due to <<perm_req:base:c:r-or-w,base-1>>
| 00101    | C |   | R |    |
| 00110    | C | W |   |    |
| 00111    | C | W | R |    |
6+| *W=Encoding[1], R=Encoding[0], LM=1, LM implies C*
| 01000  5+|                  _reserved_  due to <<perm_req:base:lm:c-and-r,base-2>>
| 01001    | C |   | R | LM |
| 01010  5+|                  _reserved_  due to <<perm_req:base:lm:c-and-r,base-2>>
| 01011    | C | W | R | LM | *Maximum permissions (AP_CMP_MAX)*
6+| *Reserved states*
| 011xx  5+|                  _reserved_ for <<section_zylevels1>>
| 100xx  5+|                  _reserved_ for <<section_zylevels1>>
| 10101  5+|                  _reserved_ for Zyseal sealing permission
| 10110  5+|                  _reserved_ for Zyseal unsealing permission
| 10111  5+|                  _reserved_ for Zyseal sealing and unsealing permission
| 11xxx  5+|                  _reserved_ for other future use
|==============================================================================

AP_MAX = {AP_CMP_MAX, 1 (<<asr_perm>>), 1 (<<x_perm>>)} = 0x2F
...
^1^ Only present if `enableL8=1`
ifndef::cheri_ratification_v1_only[]
(as used in <<rv32y_cap_description>>).
endif::[]
