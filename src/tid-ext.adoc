== "Ztid Extension for Thread Identification
...
The <<stidc>> register is an MXLEN-bit read-write register. It is used to
identify the current thread. On reset the tag will be set
to 0 and the remainder of the data is UNSPECIFIED.
...
a read-only copy of the <<stidc>> register. On reset the tag will be set
to 0 and the remainder of the data is UNSPECIFIED.
...
code can only call code capability, but not modify it. Sentries can
