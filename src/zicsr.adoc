...
.. e.g., _fcsr_
. XLEN-bit CSRs extended to CLEN-bit, which are able to contain addresses (referred to as _extended CSRs_)
...
. Access to XLEN-bit CSRs are as specified by Zicsr
. Access to CLEN-bit CSRs, including extended CSRs, using CSRRW always accesses the full CLEN-bits
.. some CSRs may have special writing rules
. Access to CLEN-bit CSRs, including extended CSRs, using other encodings will:
...
* the permission could not have been produced by <<ACPERM>>
...
Therefore _all_ CSR writes write XLEN-bit data and use <<SCADDR>> semantics to update the full CLEN-bit CSR value
. Access to CLEN-bit CSRs using CSRRW always accesses the full CLEN-bits
...
or the permission could not have been produced by <<ACPERM>>
. Access to CLEN-bit CSRs using other encodings will:
