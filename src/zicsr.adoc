...
.. e.g., <<jvt>> from <<Zcmt_CHERI>> is extended to <<jvtc>>, the `c` suffix indicating capability
...
. Accesses to CLEN-bit CSRs, including extended CSRs, using CSRRW always accesses the full CLEN-bits
.. some CSRs may have special writing rules
. Accesses to CLEN-bit CSRs, including extended CSRs, using encodings other than CSRRW will:
.. Read CLEN-bits
.. Update the address field (XLEN-bits) using the standard semantics (e.g., set bits for CSRRSI) and then use <<SCADDR>> semantics to update the full CLEN-bit CSR value

NOTE: Any CLEN-bit CSR which only has the XLEN-bit address field updated may have special rules defined to determine the final written value of the metadata and/or valid tag.

When writing CLEN-bit data to a CSR, the valid tag is cleared if:
...
.CLEN-bit CSR access summary for {cheri_base_ext_name}
[#clen_access_summary,%autowidth,options=header,align="center"]
|=======================
...
Accesses to extended CSRs in {cheri_int_mode_name} must only access XLEN-bits for compatibility.

Therefore _all_ CSR writes to extended CSRs write XLEN-bit data and use <<SCADDR>> semantics to update the full CLEN-bit CSR value:

. Accesses to CLEN-bit CSRs using CSRRW always access the full CLEN-bits
.. this includes all CSRs extended to CLEN-bits
.. when writing CLEN-bit data, the valid tag is cleared if the bounds are <<section_cap_malformed,malformed>>, any reserved bits are set,
or the permissions could not have been produced by <<ACPERM>>
. Accesses to CLEN-bit CSRs using encoding other than CSRRW will:
.. Update the address field (XLEN-bits) using the standard semantics (e.g., set bits for CSRRSI) and then use <<SCADDR>> semantics to update the full CSR on writing
.. Read CLEN-bits
