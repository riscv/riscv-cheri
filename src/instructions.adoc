...
. In Hybrid Mode, every byte of each memory access is bounds
checked against <<ddc>>
. In Hybrid Mode, a minimum length instruction at the target of all indirect
...
The jump vector table CSR <<jvt>> has a capability alias <<jvtc>> so that it can only be configured to point to accessible memory. All accesses to the jump table are checked against <<jvtc>> in Capability Mode, and against <<pcc>> bounds in Hybrid Mode. This allows the jump table to be accessed
...
NOTE: Zcmt defines that the fetch from the jump table is from instruction memory.
 ...
 Therefore <<pcc>> is used to authorise the fetch in Hybrid Mode, as the fetch is designated to be from instruction memory in cite:[riscv-unpriv-spec].
...
Mode. In Hybrid Mode the address field gives the base address of the table, and the access is
