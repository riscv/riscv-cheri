...
If the base architecture is {cheri_base_ext_name}, then the addresses which are incremented
are capability types. Therefore most Zba instructions are updated as shown.

Also see <<SH4ADD_CHERI>>/<<SH4ADD_UW_CHERI>>

include::cheri/insns/adduw_32bit.adoc[]
include::cheri/insns/sh123add_32bit.adoc[]
include::cheri/insns/sh123adduw_32bit.adoc[]
