...
4+^|#Unprivileged CSRs extended to CLEN width by {cheri_base_ext_name}#
|`0x017` +
|URW +
|`jvtc` +
|#Jump Vector Table Capability (extends `jvt`).# +

4+^|#Unprivileged {tid_ext_name} CSRs#
|`0x480` +
|URW +
|`utidc` +
...
4+^| #{cheri_priv_s_ext} Supervisor CLEN-extended CSRs#
|`0x105` +
`0x140` +
`0x141`
|SRW +
...
`sscratchc` +
`sepcc`
|#Supervisor Trap Vector Base Address Capability Register (extends `stvec`).# +
#Supervisor Scratch Capability Register (extends `sscratch`).# +
#Supervisor Exception Program Counter Capability (extends `sepc`).# +

4+^|#Supervisor {tid_ext_name} CSRs#
|`0x580` +
|SRW +
|`stidc` +
|#Supervisor thread ID capability.# +
...
4+^| #{cheri_priv_h_ext} Virtual Supervisor CLEN-extended CSRs#
|`0x205` +
`0x240` +
`0x241`
|HRW +
...
`vsscratchc` +
`vsepcc`
|#Virtual Supervisor Trap Vector Base Address Capability Register (extends `vstvec`).# +
#Virtual Supervisor Scratch Capability Register (extends `vsscratch`).# +
#Virtual Supervisor Exception Program Counter Capability (extends `vsepc`).#

4+^|#Virtual Supervisor {tid_ext_name} CSRs#
|`0xA80` +
|HRW +
|`vstidc` +
|#Virtual supervisor thread ID capability.# +
...
4+^|#{cheri_priv_m_ext} Machine CLEN-extended CSRs#
|`0x305` +
`0x340` +
`0x341`
|MRW +
...
`mscratchc` +
`mepcc`
|#Machine Trap Vector Base Address Capability Register (extends `mtvec`).# +
#Machine Scratch Capability Register (extends `mscratch`).# +
#Machine Exception Program Counter Capability (extends `mepc`).# +

4+^|#Machine {tid_ext_name} CSRs#
|`0x780` +
|MRW +
|`mtidc` +
|#Machine thread ID capability.# +
...
4+^|#{cheri_priv_debug_ext} Debug Mode CLEN-extended CSRs#
|`0x7B1` +
`0x7B2` +
`0x7B3`
|DRW +
...
`dscratch0c` +
`dscratch1c`
|#Debug Program Counter Capability (extends `dpc`).# +
#Debug Scratch Capability 0 (extends `dscratch0`).# +
#Debug Scratch Capability 1 (extends `dscratch1`).# +

4+^|#{cheri_priv_debug_ext} Debug Mode added CLEN CSRs#
|`0x7BD`
|DRW
|`dinfc`
|#Debug Infinite Capability.# +

4+^|#{cheri_priv_debug_ext} Debug Mode CLEN CSRs added for {cheri_default_ext_name}#
|`0x7BC`
|DRW
|`dddc`
