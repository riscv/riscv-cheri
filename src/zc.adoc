...
=== Zc* ({cheri_base_ext_name})

When the base ISA is {cheri_base_ext_name}, some of the 16-bit encoding space is remapped for Zca YLEN-wide loads/store and therefore some Zc* ISA extensions are incompatible with {cheri_base_ext_name}.
The mappings are shown in <<insn_remapping_16bit_rv32_b>> and <<insn_remapping_16bit_rv64_b>>

As a result:

. Zcf and Zclsd      are incompatible with {cheri_base32_ext_name}.
. Zcd, Zcmp and Zcmt are incompatible with {cheri_base64_ext_name}.

Therefore Zcmp and Zcmt are available for {cheri_base32_ext_name} only, and so:

. Specifying {cheri_base64_ext_name} + C gives Zca and Zcb.
. Specifying {cheri_base32_ext_name} + C gives Zca and Zcb.
. Specifying {cheri_base32_ext_name} + Zce gives Zca, Zcb, Zcmp and Zcmt.

[#ZCMP_CHERI,reftext="Zcmp ({cheri_base32_ext_name})"]
==== Zcmp ({cheri_base32_ext_name})

The push/pop instructions (<<insns-cm_push>>, <<insns-cm_pop>>, <<insns-cm_popret>>, <<insns-cm_popretz>>) for {cheri_base32_ext_name} have the following modifications compared to RV32:

. The saved/restored registers are YLEN-wide (64-bit) and so are stored 8-bytes apart, following the RV64 definition.
. The registers are loaded and stored using <<LOAD_CAP>> and <<STORE_CAP>>, and so the `{abi_creg}sp`` is adjusted by 8-bytes each time.

The double move instructions (<<insns-cm_mvsa01,cm.mvsa01>>, <<insns-cm_mva01s,cm.mva01s>>) have the following modification compared to RV32:

. The copied registers are YLEN-wide.

.Saved register mapping for Zcmp
[#saved_register_mapping]
[width="100%",options=header, align="center"]
|====================================
| saved register specifier | xreg | {cheri_base32_ext_name} ABI
| 0                       | x8  | {abi_creg}s0
| 1                       | x9  | {abi_creg}s1
| 2                       | x18 | {abi_creg}s2
| 3                       | x19 | {abi_creg}s3
...
==== Zcmt ({cheri_base32_ext_name})

The table jump instructions (<<insns-cm_jt>>, <<insns-cm_jalt>>) are _not_ redefined to have capabilities in the jump table.
This is to prevent the code-size growth caused by doubling the size of the jump table.

NOTE: A future extension may add new jump table modes to have capabilities in the jump table.

The jump vector table CSR <<jvt_y>> is extended to be a full capability so that it can be configured to only allow access to the jump table.
All accesses to the jump table are authorised by <<jvt_y>> and require <<x_perm>>.
This allows the jump table to be accessed when the <<pcc>> bounds are set narrowly to the local function only.

Therefore the instruction fetch bounds check is authorized by two different capabilities - <<jvt_y>> for the table access and <<pcc>> for the target instruction.

As a result the capability metadata is retained in <<pcc>> during execution.

If {cheri_default_ext_name} is implemented, and the current mode is {cheri_int_mode_name}, the jump table access is checked against <<pcc>> bounds.

NOTE: In {cheri_cap_mode_name} the implementation doesn't need to expand and bounds check against <<jvt_y>> on every access.
 It is sufficient to decode the valid accessible range of entries after every write to <<jvt_y>>, and then check that the accessed entry is in that range.

include::cheri/insns/zcmp_cmpush.adoc[]

include::cheri/insns/zcmp_cmpop.adoc[]

include::cheri/insns/zcmp_cmpopret.adoc[]

include::cheri/insns/zcmp_cmpopretz.adoc[]

include::cheri/insns/zcmp_cmvsa01.adoc[]

include::cheri/insns/zcmp_cmva01s.adoc[]
...
include::cheri/img/jvtcreg.edn[]

All instruction fetches from the jump vector table are checked against <<jvt_y>>.

If {cheri_default_ext_name} is implemented, and the current mode is {cheri_int_mode_name}, the address field gives the base address of the table, and the access is checked against <<pcc>> bounds.
...
include::cheri/insns/zcmt_cmjalt.adoc[]

include::cheri/insns/zcmt_cmjt.adoc[]
