...
^1^ The choice here is whether to take a data dependent exceptions on loads or atomic operations.
 ...
 check the tag for certain instructions and not for others, for example, checking the tag on loads
 and not on atomics. The implementation is free to define if and when to check the tag.
