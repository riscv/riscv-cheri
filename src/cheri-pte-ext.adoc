...
| 1     |!=<<senvcfgreg_pte,senvcfg>>.CRG | Page fault, or page fault if tag is set
| 1     |==<<senvcfgreg_pte,senvcfg>>.CRG | Normal operation
|===
...
| 0     | 1                | Page fault if stored tag is set, or hardware CW and CRG update^1^
| 1     | X                | Normal operation
|===

^1^ The choice here follows the pattern of whether to implement the _Svade_ extension
 to take page-fault exceptions for A and D PTE bit state changes, or whether to a implement
...
WARNING: Unlike all other bits in *Xenvcfg*, CRG is in *senvcfg* and *not* in *menvcfg*. This suggests
 this it is in the wrong place and is likely to move at some point in the future to a location
 where it is associated with virtual memory, but the *satp* CSR is full, so this will have to do.

.Machine environment configuration register (*menvcfg*) with Zcheripte, RV64 only, no CRG field
[#menvcfgreg_pte]
include::img/menvcfgreg-pte.edn[]

.Supervisor environment configuration register (*senvcfg*) with Zcheripte, RV64 only, CRG field added
[#senvcfgreg_pte]
