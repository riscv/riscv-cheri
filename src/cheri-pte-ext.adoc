...
The *mstatus*, *sstatus* and *vsstatus* operate as described in
xref:mstatus[xrefstyle=short], xref:sstatus[xrefstyle=short] and
xref:vsstatus[xrefstyle=short] respectively. {cheri_pte_ext_name} adds a new Capability Read
Generation (CRG) bit as shown in xref:mstatusreg_pte[xrefstyle=short],
xref:sstatusreg_pte[xrefstyle=short] and xref:vsstatusreg_pte[xrefstyle=short] when
XLEN=64. The CRG bit is used as described in
xref:section_extending_pte[xrefstyle=short].
