...
When a CHERI fault, or CHERI page fault, is taken into M-mode, <<mtval2>> is written
...
<<mtval2>> is written to zero for all non-CHERI exceptions, except as listed otherwise by
...
<<stval2>> is updated following the same rules as <<mtval2>> for exceptions
which are delegated to S-mode.
