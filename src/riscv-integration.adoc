...
address
* <<CSCC>>: tests whether the bounds and permissions of a capability are a
...
The <<JALR_MODE>> instruction allows access to standard RISC-V <<JALR>>, so that
the target register and link registers are both *x* registers, not *c* registers.
...
Additionally, <<JALR_MODE>> allows standard RISC-V <<JALR>> behaviour to be
available, with a zero offset. The target check on a minimum sized instruction
is still present as for all jumps and branches.
