...
with the MXLEN-bit address which caused the fault. This can be used together with the
CAP_IDX field of <<mtval>> to diagnose the authorising capability which caused the
exception.
...
TYPE is a CHERI-specific fault type that caused the exception while CAUSE
is the cause of the fault. The possible CHERI types and causes are encoded as
shown in xref:mtval-cheri-type[xrefstyle=short], xref:mtval-cheri-cap-idx[xrefstyle=short] and
xref:mtval-cheri-causes[xrefstyle=short] respectively.
