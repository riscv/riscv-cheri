...
The current CHERI execution mode is Legacy if CHERI register access is disabled
at the current privilege level as described in
xref:section_cheri_disable[xrefstyle=short], and Capability otherwise.
...
Legacy by setting CRE=0 in <<mseccfg>>, <<menvcfg>>, or <<senvcfg>> as
required, and the <<infinite-cap>> capability is installed in the <<pcc>> and
<<ddc>> such that:
...
* XLEN-bit aliases are only accessible in Legacy mode. Using an XLEN-bit alias with
  a CSR instruction in Capability mode results in an illegal instruction exception.
...
Accessing a CLEN-bit alias or a CLEN-bit CSR with no XLEN-bit name in Legacy
mode results in an illegal instruction exception.

[#section_legacy_debug_integration]
=== Integrating Zcheri_legacy with Sdext

In Debug mode, CHERI register access is always enabled and the execution mode
is always Capability.
...
* The CHERI execution mode is Legacy

CHERI register access is disabled if XLEN in the current mode is less than
XLENMAX or if CRE active at the current mode (<<mseccfg>>.CRE for M-mode,
<<menvcfg>>.CRE for S-mode, or <<senvcfg>>.CRE for U-mode) is 0.
...
The CHERI Register Enable (CRE) bit controls whether explicit access can be
performed to CHERI registers in any privilege level.  When CRE=1, CHERI
registers can be read and written.  When CRE=0, CHERI registers are disabled as
described in xref:section_cheri_disable[xrefstyle=short].
...
xref:section_cheri_disable[xrefstyle=short].  CRE is read-only zero if
<<mseccfg>>.CRE=0.
