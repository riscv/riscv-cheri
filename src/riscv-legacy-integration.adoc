...
respectively.  It is overriden to Legacy if CHERI register access is disabled
...
Legacy by setting CRE=0 in <<mseccfg>>, CME=0 in <<menvcfg>>, or CME=0 in
<<senvcfg>> as required, and the <<infinite-cap>> capability is installed in
the <<pcc>> and <<ddc>> such that:
...
The CME bits in <<menvcfg>> and <<senvcfg>> have no effect whilst CHERI
