...
== "Zcherilegacy" Extension for CHERI Legacy Mode
...
indicates Legacy Mode and M=1 indicates Capability Mode.
...
* The mode is Legacy when the effective <<CRE>>=0 for the current privilege level or the M field of the <<pcc>> is Legacy

When MXLEN=32, the M field is Legacy Mode in the <<infinite-cap>> capability,
...
=== Zcherilegacy Instructions

{cheri_legacy_ext_name} introduces a small number of new mode-switching instructions to the base RISC-V integer ISA, as shown in <<Zcheri_legacy_instruction_extension>>.
...
=== Integrating Zcherilegacy with Sdext
