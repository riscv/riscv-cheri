...
== "Zcherihybrid" Extension for CHERI Default Mode
...
There are two execution modes: _Capability_ and _Default__. Additionally, there
...
In Default Mode, <<ddc>> is implicitly used. In Capability Mode, the authorising
...
are available in Default Mode but not Capability Mode and vice-versa (see
...
{cheri_legacy_ext_name} provided that the <<infinite-cap>> capability is installed in <<ddc>> and <<pcc>> (with <<section-cheri-execution-mode,M=0>>, i.e. CHERI execution mode set to Default).
...
{cheri_legacy_ext_name} adds a new CHERI execution mode field (M) to
...
indicates Default Mode and M=1 indicates Capability Mode.
...
* The mode is Default when the effective CRE=0 for the current privilege level or the M field of the <<pcc>> is Default

When MXLEN=32, the M field is Default Mode in the <<infinite-cap>> capability,
...
{cheri_legacy_ext_name} introduces a small number of new mode-switching instructions to the base RISC-V integer ISA, as shown in <<Zcheri_hybrid_instruction_extension>>.
...
In Default Mode, the capability authorising the memory access
...
<<pcc>> is Default, then the mode after executing <<MODESW>> is Capability
...
When in Default Mode, the address input or output operands are *x/f/v* registers;
...
When the mode is Default. In this case, the address of the instruction
...
xref:section_existing_riscv_insns[xrefstyle=short].
When the mode is Default. In this case, the target address is obtained by adding
...
In Default Mode, the instructions behave as described in
...
where the address field is visible in Default Mode (e.g. <<mtvec>>) and the full
...
When <<CSRRW>> is executed on an extended CSR in Default Mode:
...
XLEN write.
* In Default Mode, XLEN bits are read from the capability address field and
...
Accessing a capability CSR other than an extended CSR in Default Mode results in
...
* Accessing the added CLEN-wide CSRs in <<default-csrnames-added>> cause illegal instruction exceptions
* Only XLEN-wide access is available to the CLEN-wide CSRs in <<aliased_CSRs>>
* All allowed instructions execute as if in Default Mode.
...
NOTE: Disabling CHERI register access prevents a low-privileged Default Mode
from interfering with the correct operation of higher-privileged Default Modes
...
Default. This register must be readable in any implementation. Its reset value
