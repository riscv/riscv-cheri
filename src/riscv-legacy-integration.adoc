...
capabilities. When MXLEN=32, the mode is encoded in the permissions field as
described in xref:cap_permissions_encoding[xrefstyle=short]. When MXLEN=64, a
new M bit field is added to the capability format as shown in
xref:cap_encoding_xlen64_mode[xrefstyle=short]; M=0 indicates Legacy Mode and
M=1 indicates Capability Mode.
...
so its AP field is 8. The <<null-cap>> capability does not grant <<x_perm>>, so
the M field is superfluous in this case.

When MXLEN=64, the M bit can be set to 1 when the capability does not grant <<x_perm>>.
In this case, the M bit is superfluous, so the encoding may be used to support additional features in future extensions. The M bit is 0 in both the <<null-cap>> and <<infinite-cap>> capabilities.
