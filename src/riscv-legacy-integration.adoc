...
{cheri_default_ext_name} provided that the <<infinite-cap>> capability is installed in <<ddc>> and <<pcc>> (with <<section-cheri-execution-mode,M=0>>, i.e. CHERI execution mode set to Default).
...
{cheri_default_ext_name} adds two new enable bits to <<menvcfg>> as shown in
...
Specification. {cheri_default_ext_name} adds two new enable bits as shown in
