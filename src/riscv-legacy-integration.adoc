...
== "Zcherihybrid" Extension for CHERI Hybrid Mode

{cheri_hybrid_ext_name} is an optional extension to {cheri_base_ext_name}.
Implementations that support {cheri_base_ext_name} and {cheri_hybrid_ext_name}
define a variant of the CHERI ISA that is fully binary compatible with
existing RISC-V code.

Key features in {cheri_hybrid_ext_name} include a definition of a CHERI execution mode, a new
...
{cheri_hybrid_ext_name} adds CHERI execution modes to ensure backwards
compatibility with the base RISC-V ISA while saving instruction encoding space.
There are two execution modes: _Capability_ and _hybrid_. Additionally, there
...
In Hybrid Mode, <<ddc>> is implicitly used. In Capability Mode, the authorising
...
are available in Hybrid Mode but not Capability Mode and vice-versa (see
...
{cheri_hybrid_ext_name} provided that the <<infinite-cap>> capability is installed in <<ddc>> and <<pcc>> (with <<section-cheri-execution-mode,M=0>>, i.e. CHERI execution mode set to Hybrid).
...
{cheri_hybrid_ext_name} adds a new CHERI execution mode field (M) to
...
indicates Hybrid Mode and M=1 indicates Capability Mode.
...
* The mode is Capability when the M field of the <<pcc>> is Capability and the effective
<<CRE>>=1 for the current privilege level
* The mode is Hybrid when the effective CRE=0 for the current privilege level or the M field of the <<pcc>> is Hybrid

When MXLEN=32, the M field is Hybrid Mode in the <<infinite-cap>> capability,
...
{cheri_hybrid_ext_name} introduces a small number of new mode-switching instructions to the base RISC-V integer ISA, as shown in <<Zcheri_hybrid_instruction_extension>>.
...
In Hybrid Mode, the capability authorising the memory access
...
<<pcc>> is Hybrid, then the mode after executing <<MODESW>> is Capability
...
The CHERI execution mode introduced in {cheri_hybrid_ext_name} affects the
...
When in Hybrid Mode, the address input or output operands are *x/f/v* registers;
...
When the mode is Hybrid. In this case, the address of the instruction
...
xref:section_existing_riscv_insns[xrefstyle=short].
When the mode is Hybrid. In this case, the target address is obtained by adding
...
{cheri_hybrid_ext_name} allows changing the current CHERI execution mode when
...
xref:section_existing_riscv_insns[xrefstyle=short] when in Capability Mode
In Hybrid Mode, the instructions behave as described in
...
{cheri_hybrid_ext_name} adds the concept of CSRs which contain a capability
where the address field is visible in Hybrid Mode (e.g. <<mtvec>>) and the full
...
When <<CSRRW>> is executed on an extended CSR in Hybrid Mode:
...
XLEN write.
* In Hybrid Mode, XLEN bits are read from the capability address field and
...
Accessing a capability CSR other than an extended CSR in Hybrid Mode results in
...
{cheri_hybrid_ext_name} optionally allows <<MODESW>> to execute in debug mode.
...
{cheri_hybrid_ext_name} includes functions to disable explicit access to CHERI
registers.  The following occurs when executing code in a privilege mode that
has CHERI register access disabled:

* The CHERI instructions in xref:section_cap_instructions[xrefstyle=short],
xref:section_legacy_ext_instructions[xrefstyle=short]
cause illegal instruction exceptions.
* Accessing the added CLEN-wide CSRs in <<legacy-csrnames-added>> cause illegal instruction exceptions
* Only XLEN-wide access is available to the CLEN-wide CSRs in <<aliased_CSRs>>
* All allowed instructions execute as if in Hybrid Mode.
...
* <<mseccfg>>.CRE=0 for M-mode
* <<menvcfg>>.CRE=0 for S-mode
* <<senvcfg>>.CRE=0 for U-mode

CHERI register access is always enabled in debug mode.
...
[[xenvcfg-warl-field]]
.mseccfg, Xenvcfg joint WARL field
[float="center",align="center",cols="1,1,1,3",options="header"]
|===
...
The WARL programming nature is extended to include UXLEN, SXLEN and <<mstatus>>.MBE.
In _all_ cases the CRE enable for the current mode is read-only zero if all the conditions for
enabling CHERi register state are not met for each privilege mode.
Therefore the relevant CRE bit accurately shows whether CHERI register
state is enabled for the current mode.

The full equations per mode are:

* <<mseccfg>>.CRE is read-only-zero if:
** <<mstatus>>.MBE is not the reset value

* <<menvcfg>>.CRE is read-only-zero if:
...
NOTE: Disabling CHERI register access prevents a low-privileged Hybrid Mode
from interfering with the correct operation of higher-privileged Hybrid Modes
that do not perform <<ddc>> switches on trap entry and return.

=== Added CLEN-wide CSRs

{cheri_hybrid_ext_name} adds the CLEN-wide CSRs shown in
xref:legacy-csrnames-added[xrefstyle=short].

[[legacy-csrnames-added]]
.CLEN-wide CSRs added in {cheri_hybrid_ext_name}
...
{cheri_hybrid_ext_name} eliminates some restrictions for SXL and UXL imposed in
...
xref:CRE[xrefstyle=short], while in that privilege mode.
...
{cheri_hybrid_ext_name} eliminates some restrictions for MBE, SBE, and UBE
...
{cheri_hybrid_ext_name} adds a new enable bit to <<mseccfg>> as shown in
...
When <<CRE>>=0, CHERI register and instruction access is prohibited for M-mode and lower privilege levels.

<<mseccfg>>.<<CRE>> is read-only-zero if <<mstatus>>.MBE is not the reset value.
...
{cheri_hybrid_ext_name} adds two new enable bits to <<menvcfg>> as shown in
...
Specification. {cheri_hybrid_ext_name} adds two new enable bits as shown in
...
Hybrid. This register must be readable in any implementation. Its reset value
