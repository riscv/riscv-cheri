...
Load Mutable Permission (LM):: Allow loads of capabilities with <<w_perm>> via a read-only authorizing capability.
Clearing this permission allows sharing a read-only version of e.g. a tree structure without making a copy.
If this permission is not set and the loaded capability is tagged and not sealed, capability loads authorized by this capability will have <<w_perm>> and <<lm_perm>> removed. The rules specified by <<ACPERM>> are followed which means that additional permissions may also be removed as a result.
...
|==============================================================================
| Encoding[2:0] | R | W | C | LM | X | ASR | Mode | Notes
...
| 4-6   8+| reserved
| 7       | ✔ | ✔ | ✔ |   |   |     | N/A | Data & Cap RW
9+| *Quadrant 3: Capability data read/write (with <<lm_perm>>)*
9+| bit[2] - write. R and C implicitly granted.
9+| _Reserved bits for future extensions must be 1 so they are implicitly granted_
| 0-2   8+| reserved
| 3       | ✔ |   | ✔ | ✔ |   |     | N/A | Data & Cap RO
...
A {cap_rv64_perms_width}-bit vector encodes the permissions and <<m_bit>> when MXLEN=64. In
...
implementation supports {cheri_legacy_ext_name}, otherwise the bit is reserved
...
| AP (MXLEN=64) | 0x5F (see xref:cap_perms_encoding64[xrefstyle=short]) | Grants all permissions
| S       | zero  | Unsealed
| EF      | zero  | Internal exponent format
| T~8~    | zeros | Top address bit (MXLEN=32 only)
| T       | zeros | Top address bits
