...
instruction automatically unseals the target capability and installs it to the
program counter capability (see
xref:section_riscv_programmers_model[xrefstyle=short]). The jump instructions
also seal the return address capability (if any) since it is the entry point
