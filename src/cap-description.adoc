...
NOTE: Despite being encoded with the permissions here, the <<m_bit>> is not a permission.
Permissions decrease monotonically; you cannot create a capability with more
permissions than its progenitor. In contrast the <<m_bit>> can be toggled
using <<SCMODE>>.
...
NOTE: The AP fields both set pass:attributes,quotes[{cheri_int_mode_name}] (if {cheri_default_ext_name} is supported)
