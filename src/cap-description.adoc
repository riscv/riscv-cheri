...
NOTE: When MXLEN=32 the hart's CHERI execution mode is always Capability (see
xref:m_bit[xrefstyle=short]), so the encodings for Legacy Mode are reserved,
and must not appear in valid capabilities, if only {cheri_base_ext_name} is
supported.
...
.. MXLEN=32 goes further here and denies <<w_perm>> and <<c_perm>> without <<r_perm>>.
 Leaving this combination will clear _all_ permissions.
...
[#null-cap,reftext="NULL",options=header,align=center,width="90%",cols="20,17,63"]
|==============================================================================
| Field   | Value | Comment
...
[options=header,align=center,width="90%",cols="20,17,63"]
|==============================================================================
| Field   | Value | Comment
| Tag     | one   | Capability is valid
| SDP     | ones  | Grants all permissions
| AP (MXLEN=64) | ones  | Grants all permissions
| AP (MXLEN=32) | 9 (see xref:cap_perms_encoding32[xrefstyle=short]) | Grants all permissions (and Capability Mode; see xref:m_bit[xrefstyle=short])
| S       | zero  | Unsealed
| EF      | zero  | Internal exponent format
| T~8~    | zeros | Top address bit (MXLEN=32 only)
| T       | zeros | Top address bits
