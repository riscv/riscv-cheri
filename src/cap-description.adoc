...
otype no longer exists. Please note that this bit indicates the result of what
was done in two instructions in CHERI v9: CSeal for sealed capabilities and
CSealEntry for sealed entry capabilties.
endif::[]
...
creating sealed entry (sentry) capabilities.
...
Sealed capabilities might be useful to software as tokens that can be passed
around. The only way of removing the seal bit of a capability is by rebuilding
it via a superset capability with <<CBUILDCAP>>. {cheri_base_ext_name} does not offer
...
sealed code capability to begin executing the instructions it references. The jump
