...
Load Mutable Permission (LM):: Allow loads of capabilities with <<w_perm>> via a read-only authorizing capability one.
Clearing this permission allows sharing a read-only version of e.g. a tree structure without making a copy.
If this permission is not set, capability loads authorized by this capability will have resulting permissions as if an implicit <<ACPERM>> clearing <<w_perm>> and <<lm_perm>> was executed.
