...
[#em_perm,reftext="EM-permission"]
Elevate Mutable Permission (EM):: Allow preserving the <<w_perm>> of capabilities loaded from memory when the load is authorized via a read-only capability (with <<r_perm>> but no <<w_perm>>).
If a capability grants <<r_perm>>, but neither <<w_perm>> nor <<em_perm>>, then a capability loaded via this authorizing capability will have <<w_perm>> and <<em_perm>> removed provided that the loaded capability has its tag set and is not sealed; loaded capabilities that are sealed or untagged do not have their permissions changed.
The rules specified by <<ACPERM>> are followed when <<w_perm>> and <<em_perm>> are removed, so additional permissions may also be removed.
Clearing a capability's <<em_perm>> and <<w_perm>> allows sharing a read-only version of a data structure (e.g. a tree or linked list) without making a copy.
...
|==============================================================================
| Encoding[2:0] | R | W | C | EM | X | ASR | Mode^1^ | Notes
9+| *Quadrant 0: Non-capability data read/write*
9+| bit[2] - W, bit[1] - reserved (0), bit[0] - R
...
9+| *Quadrant 2: Capability data read/write (without <<em_perm>>)*
9+| bit[2] - W. R and C implicitly granted.
...
9+| bit[2] - W. R, C and EM implicitly granted.
9+| _Reserved bits for future extensions must be 1 so they are implicitly granted_
| 0-2   8+| reserved
| 3       | ✔ |   | ✔ | ✔ |   |     | N/A | Data & Cap RO
| 4-6   8+| reserved
