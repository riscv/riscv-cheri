...
^| 32    | {cap_rv32_perms_width} | Encodes some combinations of 5 permission bits, including the <<m_bit>> if {cheri_default_ext_name} is supported.
...
A {cap_rv64_perms_width}-bit vector encodes the permissions MXLEN=64. In
