...
Executing <<ACPERM>> can result in sets of permissions which cannot be represented when XLENMAX=32
(see <<cap_perms_encoding32>>) or permission combinations which are not useful for XLENMAX=64, such as
...
.. XLENMAX=32 goes further here and denies <<w_perm>> and <<c_perm>> without <<r_perm>>.
