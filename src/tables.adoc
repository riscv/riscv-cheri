...
{cheri_hybrid_ext_name} defines the set of instructions added by the Hybrid Mode, in addition to {cheri_base_ext_name}.

NOTE: {cheri_hybrid_ext_name} implies {cheri_base_ext_name}

.{cheri_hybrid_ext_name} instruction extension - Hybrid Mode instructions
...
.Instructions valid for execution in Hybrid Mode only
[#legacy_mode_insns]
[width="100%",options=header,cols="3,1,1,3"]
|==============================================================================
...
.Instructions valid for execution in both capability and Hybrid Modes
[#both_mode_insns]
[width="100%",options=header,cols="3,1,1,3"]
|==============================================================================
...
NOTE: _XLEN writing_ is only available if {cheri_hybrid_ext_name} is implemented.
...
.Mnemonics with the same encoding but mapped to different instructions in Hybrid and Capability Mode
[#legacy_mnemonics]
...
NOTE: <<MODESW>> and <<SCMODE>> only exist in Capability Mode if Hybrid Mode is _also_ present. A purecap core does not implement the mode bit in the capability.
