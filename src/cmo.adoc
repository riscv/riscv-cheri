...
When the base architecture is {cheri_base_ext_name}, all CBO and prefetch instructions are authorized by the capability in `{creg_pref}s1`.

In common with load, stores and prefetches, the encodings are _reserved_ if `{creg_pref}s1={creg}0`.
