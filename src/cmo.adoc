...
When the base architecture is {cheri_base_ext_name}, all CBO and prefetch instructions are authorized by the capability in `ys1`.

In common with load, stores and prefetches, the encodings are _reserved_ if `ys1=c0`.
...
When CBO.ZERO clears the cache block, it must also clear the associated valid tags.
