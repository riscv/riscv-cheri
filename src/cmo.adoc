...
For CBO instructions other than <<CBO_ZERO_CHERI>>, and for all prefetches, at least one byte of the cache block must be in bounds for the access to the authorised.

For <<CBO_ZERO_CHERI>> the entire cache block must in bounds for the access to be authorized, and the associated valid tags are also cleared.
