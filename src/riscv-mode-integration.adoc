...
A new <<CSETMODE>> instruction allows setting a capability's CHERI execution
mode to the indicated value. The output is written to an unprivileged *c*
register, not <<pcc>>.

==== Mode Change Instructions

A new CHERI execution mode switch (<<CMODESWITCH>>) instruction allows software
to toggle the hart's current CHERI execution mode. If the current mode in the
<<pcc>> is Legacy, then the mode after executing <<CMODESWITCH>> is Capability
...
executing <<CJALR>> or <<JALR_CAP>>.

=== Integrating Zcheri_mode with Sdext

ifdef::cheri_v9_annotations[]
NOTE: *CHERI v9 Note:* The mode change instruction <<CMODESWITCH>> is new
...
allows <<CMODESWITCH>> to act as an illegal instruction when it is executed
while in debug mode.
