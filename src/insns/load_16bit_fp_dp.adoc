...
Standard floating point stack pointer relative load instructions, authorised by the capability in <<ddc>>. Note that these instructions are not available in pass:attributes,quotes[{cheri_cap_mode_name}], as they have been remapped to <<C.LC>>, <<C.LCSP>>.
