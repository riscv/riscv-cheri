...
.. If {cheri_levels_ext_name} is implemented then also set <<sl_perm>> to 0 (_local_).
...
|  5   | <<x_perm>>   | <<w_perm>> or (<<c_perm>> and <<lm_perm>> and <<el_perm>>)
| 11   | <<asr_perm>> | All other permissions are set.
|===
