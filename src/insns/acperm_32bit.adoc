...
. Convert the AP and SDP fields of capability `cs1` into a bit field. The bit
field format is shown in xref:acperm_bit_field_clen64[xrefstyle=short] when
CLEN=64, otherwise it is as shown in
xref:acperm_bit_field_clen128[xrefstyle=short] when CLEN=128.
