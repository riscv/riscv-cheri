...
NOTE: If {cheri_levels_ext_name} is _not_ implemented then when applying the rules below
 <<el_perm>> is initialized to 1, and <<sl_perm>> is initialized to the maximum value.
 They are updated as required by the rules, and then discarded at the end.
