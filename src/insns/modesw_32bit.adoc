...
NOTE: The effective CHERI execution mode is given by the value of some CSRs
(see xref:section_cheri_disable[xrefstyle=short]) and
the <<pcc>>'s <<m_bit>>, so executing <<MODESW_CAP>> does not necessarily change
