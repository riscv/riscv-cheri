...
If the base architecture is {cheri_base_ext_name}, then an additional AMOSWAP encoding (<<AMOSWAP_CAP>>) is available to access capability types in memory.
...
Therefore _all_ atomic operations are _reserved_ if `{creg_pref}s1={creg}0`.
