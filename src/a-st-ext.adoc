...
If the base architecture is {cheri_base_ext_name}, then additional LR/SC encodings are available in <<rvyalrsc_ext_name>>
to access capability types in memory.
...
[#zaamo_cheri]
=== {rvyaamo_ext_name} ("Zaamo" for {cheri_base_ext_name})

If the base architecture is {cheri_base_ext_name}, then {rvyaamo_ext_name} provides an additional AMOSWAP encoding (<<AMOSWAP_CAP>>) to access capability types in memory.
...
arithmetic to be performed. For example, AMOADD would need <<CADD>> functionality.
