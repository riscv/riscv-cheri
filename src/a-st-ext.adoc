...
If the base architecture is {cheri_base_ext_name}, then additional LR/SC encodings are available
to access capability types in memory (the {lr_sc_bh_ext_name} extension, see <<abhlrsc_ext>>).
...
If the base architecture is {cheri_base_ext_name}, then an additional AMOSWAP encoding (<<AMOSWAP_C>>) is available to access capability types in memory.

All atomic operations follow the rules in xref:section_int_load_store_insns[xrefstyle=short],
where the atomic operation is considered to be a load followed by a store for the
capability rules.

Therefore _all_ atomic operations are _reserved_ if `cs1=c0`.
