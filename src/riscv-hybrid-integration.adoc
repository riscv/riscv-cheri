...
[#m_bit_observe,reftext="Observing the M-bit"]
==== Observing the M-bit

The <<m_bit>> can be observed as follows:

. outside debug mode <<AUIPC>> followed by <<GCTAG>> will return 1 for pass:attributes,quotes[{cheri_cap_mode_name}] and 0 for pass:attributes,quotes[{cheri_int_mode_name}]
. in debug mode <<CSRRW>> of <<dinfc>> followed by <<GCTAG>> will return 1 for pass:attributes,quotes[{cheri_cap_mode_name}] and 0 for pass:attributes,quotes[{cheri_int_mode_name}]
