...
== "Zcherihybrid" Extension for CHERI {cheri_int_mode_name}
...
There are two execution modes: {cheri_cap_mode_name} and {cheri_int_mode_name}. Additionally, there
...
In {cheri_int_mode_name}, <<ddc>> is implicitly used. In {cheri_cap_mode_name}, the authorising
...
(with <<section-cheri-execution-mode,M=0>>, i.e. in {cheri_int_mode_name}).
...
indicates {cheri_int_mode_name} and M=1 indicates {cheri_cap_mode_name}.
...
* The mode is {cheri_cap_mode_name} when the M field of the <<pcc>> is {cheri_cap_mode_name} and the effective
CRE=1 for the current privilege level
* The mode is {cheri_int_mode_name} when the effective CRE=0 for the current privilege level or the M field of the <<pcc>> is {cheri_int_mode_name}

When MXLEN=32, the M field is {cheri_int_mode_name} in the <<infinite-cap>> capability,
...
In {cheri_int_mode_name}, the capability authorising the memory access
...
When <<CSRRW>> is executed on an extended CSR in {cheri_cap_mode_name}:
...
XLEN write.
* In {cheri_int_mode_name}, XLEN bits are read from the capability address field and
written to an output *x* register. In {cheri_cap_mode_name}, CLEN bits are read from
...
Accessing a capability CSR other than an extended CSR in {cheri_int_mode_name} results in
...
{cheri_cap_mode_name}. The valid configurations are shown in
...
The <<mtdc>> register is {cheri_cap_mode_name} width read/write register dedicated
...
The <<stdc>> register is {cheri_cap_mode_name} width read/write register dedicated
