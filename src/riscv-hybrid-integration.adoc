...
There are two execution modes: _Capability_ and _Default_. Additionally, there
...
{cheri_legacy_ext_name} provided that the <<infinite-cap>> capability is installed in <<ddc>> and <<pcc>>
(with <<section-cheri-execution-mode,M=0>>, i.e. in Default Mode).
...
* All allowed instructions execute as if the CHERI execution mode is Default.
