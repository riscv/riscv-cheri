...
A new CHERI execution mode switch (<<MODESW_CAP>>/<<MODESW_INT>>) instruction allows software
...
{cheri_default_ext_name} optionally allows <<MODESW_CAP>>/<<MODESW_INT>> to execute in debug mode.
...
executing <<MODESW_CAP>>/<<MODESW_INT>> from the program buffer.

The current mode can be read from <<dinfc>>.

ifdef::cheri_v9_annotations[]
NOTE: *CHERI v9 Note:* The mode change instructions <<MODESW_CAP>>/<<MODESW_INT>> are new
