...
{cheri_hybrid_ext_name} provided that the <<infinite-cap>> capability is installed in <<ddc>> and <<pcc>>
(with <<section-cheri-execution-mode,M=0>>, i.e. in Hybrid Mode).
...
* All allowed instructions execute as if the CHERI execution mode is Hybrid.
