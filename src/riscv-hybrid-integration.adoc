...
the capability format, which is only valid for executable capabilities, i.e.
...
It is _only_ the <<m_bit>> for code capabilities, otherwise the field is reserved.

NOTE: Mode is similar to a permission, but is not considered one as it can varied
arbitrarily using <<SCMODE>>.

In both encodings, Mode (M)=0
indicates pass:attributes,quotes[{cheri_int_mode_name}] and Mode (M)=1 indicates pass:attributes,quotes[{cheri_cap_mode_name}].
