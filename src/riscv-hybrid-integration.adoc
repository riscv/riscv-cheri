...
== "Zcherihybrid" Extension for CHERI pass:attributes,quotes[pass:attributes,quotes[{cheri_int_mode_name}]]
...
There are two execution modes: pass:attributes,quotes[pass:attributes,quotes[{cheri_cap_mode_name}]] and pass:attributes,quotes[pass:attributes,quotes[{cheri_int_mode_name}]]. Additionally, there
is a new unprivileged register: the default data capability, <<ddc>>, that is
used to authorise all data memory accesses when in
pass:attributes,quotes[pass:attributes,quotes[{cheri_int_mode_name}]].
...
In pass:attributes,quotes[pass:attributes,quotes[{cheri_int_mode_name}]], <<ddc>> is implicitly used. In pass:attributes,quotes[pass:attributes,quotes[{cheri_cap_mode_name}]], the authorising
...
are available in pass:attributes,quotes[pass:attributes,quotes[{cheri_int_mode_name}]] but not pass:attributes,quotes[pass:attributes,quotes[{cheri_cap_mode_name}]] and vice-versa (see
...
is the register operand `cs1` (pass:attributes,quotes[pass:attributes,quotes[{cheri_cap_mode_name}]]). The mode is shown in the assembly
syntax.
...
(with <<section-cheri-execution-mode,M=0>>, i.e. in pass:attributes,quotes[pass:attributes,quotes[{cheri_int_mode_name}]]).
...
When MXLEN=64, the mode is encoded in bit 0 of the AP field: a new M bit field is
added to the capability format as shown in
xref:cap_perms_encoding64[xrefstyle=short]. In both encodings, M=0
indicates pass:attributes,quotes[pass:attributes,quotes[{cheri_int_mode_name}]] and M=1 indicates pass:attributes,quotes[pass:attributes,quotes[{cheri_cap_mode_name}]].

The current CHERI execution mode is given by the M field of the <<pcc>> and the
CRE bits in <<mseccfg>>, <<menvcfg>>, and <<senvcfg>> as follows:

* The mode is pass:attributes,quotes[pass:attributes,quotes[{cheri_cap_mode_name}]] when the M field of the <<pcc>> is pass:attributes,quotes[pass:attributes,quotes[{cheri_cap_mode_name}]] and the effective
CRE=1 for the current privilege level
* The mode is pass:attributes,quotes[pass:attributes,quotes[{cheri_int_mode_name}]] when the effective CRE=0 for the current privilege level or the M field of the <<pcc>> is pass:attributes,quotes[pass:attributes,quotes[{cheri_int_mode_name}]]

When MXLEN=32, the M field is pass:attributes,quotes[pass:attributes,quotes[{cheri_int_mode_name}]] in the <<infinite-cap>> capability,
so its AP field is 8; furthermore, the <<null-cap>> capability does not grant
<<x_perm>>, so the M field is superfluous in this case.

When MXLEN=64, the <<m_bit>> can be set to 1 when the capability does not grant <<x_perm>>.
In this case, the <<m_bit>> is superfluous, so the encoding may be used to support additional features in future extensions;
furthermore, the <<m_bit>> is 0 in both the <<null-cap>> and <<infinite-cap>> capabilities.
...
In pass:attributes,quotes[pass:attributes,quotes[{cheri_int_mode_name}]], the capability authorising the memory access
...
<<pcc>> is pass:attributes,quotes[pass:attributes,quotes[{cheri_int_mode_name}]], then the mode after executing <<MODESW>> is pass:attributes,quotes[pass:attributes,quotes[{cheri_cap_mode_name}]]
...
in pass:attributes,quotes[pass:attributes,quotes[{cheri_cap_mode_name}]], the address input or output operands may include *c* registers.
When in pass:attributes,quotes[pass:attributes,quotes[{cheri_int_mode_name}]], the address input or output operands are *x/f/v* registers;
...
When the mode is pass:attributes,quotes[pass:attributes,quotes[{cheri_int_mode_name}]]. In this case, the address of the instruction
...
is pass:attributes,quotes[pass:attributes,quotes[{cheri_cap_mode_name}]]; behaves as described in
xref:section_existing_riscv_insns[xrefstyle=short].
When the mode is pass:attributes,quotes[pass:attributes,quotes[{cheri_int_mode_name}]]. In this case, the target address is obtained by adding
...
executing <<JALR>> from pass:attributes,quotes[pass:attributes,quotes[{cheri_cap_mode_name}]].
...
xref:section_existing_riscv_insns[xrefstyle=short] when in pass:attributes,quotes[pass:attributes,quotes[{cheri_cap_mode_name}]]
In pass:attributes,quotes[pass:attributes,quotes[{cheri_int_mode_name}]], the instructions behave as described in
...
where the address field is visible in pass:attributes,quotes[pass:attributes,quotes[{cheri_int_mode_name}]] (e.g. <<mtvec>>) and the full
capability is visible in pass:attributes,quotes[pass:attributes,quotes[{cheri_cap_mode_name}]] through a different name (e.g.
<<mtvecc>>). These are referred to as _extended CSRs_.

Extended CSRs have only one address; the access width is determined by the
execution mode.

When <<CSRRW>> is executed on an extended CSR in pass:attributes,quotes[pass:attributes,quotes[{cheri_int_mode_name}]]:
...
When <<CSRRW>> is executed on an extended CSR in pass:attributes,quotes[pass:attributes,quotes[{cheri_cap_mode_name}]]:
...
XLEN write.
* In pass:attributes,quotes[pass:attributes,quotes[{cheri_int_mode_name}]], XLEN bits are read from the capability address field and
written to an output *x* register. In pass:attributes,quotes[pass:attributes,quotes[{cheri_cap_mode_name}]], CLEN bits are read from
...
Accessing a capability CSR other than an extended CSR in pass:attributes,quotes[pass:attributes,quotes[{cheri_int_mode_name}]] results in
...
When entering debug mode, the core always enters pass:attributes,quotes[pass:attributes,quotes[{cheri_cap_mode_name}]].
...
* All allowed instructions execute as if the CHERI execution mode is pass:attributes,quotes[pass:attributes,quotes[{cheri_int_mode_name}]].
...
pass:attributes,quotes[pass:attributes,quotes[{cheri_cap_mode_name}]]. The valid configurations are shown in
...
NOTE: Disabling CHERI register access prevents low-privileged pass:attributes,quotes[pass:attributes,quotes[{cheri_int_mode_name}]] software
from interfering with the correct operation of higher-privileged pass:attributes,quotes[pass:attributes,quotes[{cheri_int_mode_name}]] software
...
The <<mtdc>> register is pass:attributes,quotes[pass:attributes,quotes[{cheri_cap_mode_name}]] width read/write register dedicated
...
The <<stdc>> register is pass:attributes,quotes[pass:attributes,quotes[{cheri_cap_mode_name}]] width read/write register dedicated
...
pass:attributes,quotes[pass:attributes,quotes[{cheri_int_mode_name}]]. This register must be readable in any implementation. Its reset value
