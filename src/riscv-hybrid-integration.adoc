...
(with <<section-cheri-execution-mode,M=0>>, i.e. in pass:attributes,quotes[{cheri_int_mode_name}]).
...
* The mode is pass:attributes,quotes[{cheri_cap_mode_name}] when the M field of the <<pcc>> is pass:attributes,quotes[{cheri_cap_mode_name}] and the effective
CRE=1 for the current privilege level
* The mode is pass:attributes,quotes[{cheri_int_mode_name}] when the effective CRE=0 for the current privilege level or the M field of the <<pcc>> is pass:attributes,quotes[{cheri_int_mode_name}]

When MXLEN=32, the M field is pass:attributes,quotes[{cheri_int_mode_name}] in the <<infinite-cap>> capability,
